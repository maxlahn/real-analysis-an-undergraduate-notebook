...
},
{
  ...
  "type": "Problem",
  "number": "1.1.5.1",
  "title": "Distributivity of set operations.",
  "body": " Distributivity of set operations  Distributive property    Prove that for all sets , , and .    Let . Then and , and so or . If , then , and so . On the other hand, if , then , and so .  In summary, we have shown that for all elements , and so .  Conversely, now let . Then or . If , then and . In particular, , and so .  In summary, we have shown that for all , and so ; we now conclude that .      Prove that for all sets , , and .    Let . Then or . If , then and , and so . On the other hand, if , then and . In this case, and , and so .  In summary, we have shown that for all elements , and so .  Conversely, now let . Then and . In particular, if , then and , and so . We conclude that .  In summary, we have shown that for all , and so ; we now conclude that .     Taken together, these results are called the distributive properties of the set operations.   "
},
{
...
},
{
  ...
  "body": " Orders from pullbacks    Prove that the pullback of a preorder is a preorder.    You may find it useful to review the proof of     Consider the pullback of a preorder on a set along a map from a set to . For all elements , the reflexivity of implies that , and so ; that is, is reflexive.  For all elements , suppose that and . Then and , and so the transitivity of implies that ; that is, , and so is transitive.  Since the binary relation is reflexive and transitive, it is a preorder on .      Prove that the pullback of a partial order on a set along a map is a partial order on if and only if is injective.    Since is a partial order on , it is a preorder on , and thus is a preorder on by (a) above. Thus it suffices to show that is anti-symmetric if and only if is injective. To that end, suppose first that is not anti-symmetric. Then and for some elements so that . We observe that and , and so the anti-symmetry of implies that . Since but , is not injective.  Conversely, suppose that is not injective. Then for some elements so that . We see that and , so that and . That now implies that is not anti-symmetric.      Is the pullback of a total order along an injective map necessarily a total order? If so, prove it. If not, provide a counterexample.    Yes, the pullback of a total order along an injective map is necessarily a total order. Indeed, consider the pullback of a total order on a set along an injective map from a set to . Since is a total order, implies that it is a partial order, and so is a partial order by (b) above. Thus it suffices to show that must be total.  To that end, let , and note that the totality of implies that either or (or both); that is, either or (or both). Thus is total, and hence a total order on .    "
},
{
...
},
{
  ...
  "body": " Infima and Dedekind completeness   Let be an ordered field, and consider the map defined by the formula ; that is, sends an input to its additive inverse .     Show that for a subset , is bounded from above if and only if is bounded from below, and is bounded from below if and only if is bounded from above.    If is bounded from above, then there is some element so that for all . We observe that (b) in implies that for all , and so is a lower bound for .  Similarly, if is bounded from below, then there is some element so that for all . We observe that (b) in implies that for all , and so is an upper bound for .  The reverse implications follow from the observation that is its own inverse and then applying the above arguments to : if is bounded from above (resp. below), then is bounded from below (resp. above).      Show that a subset has a supremum if and only if has an infimum, and in this case . Similarly, show that has an infimum if and only if has a supremum, and in this case     The argument in the above proof of (a) shows that interchanges upper and lower bounds. Specifically, sends the set of upper bounds (resp. lower bounds) of a set to the set of lower bounds (resp. upper bounds) of . Moreover, by (b) of , if , then . As such, sends a least upper bound (resp. greatest lower bound) for to a greatest lower bound (resp. least upper bound) for . In particular, if has a supremum (resp. infimum), then has an infimum (resp. supremum). The converse holds by applying the same result to and observing that is an involution; that is, that .      Show that the ordered field is Dedekind complete if and only if every non-empty subset of which is bounded from below has an infimum.    If is Dedekind complete, then every non-empty subset of which is bounded from above has a supremum. Let be a non-empty subset of which is bounded from below. By (a) above, is a non-empty subset of which is bounded from above and hence has a supremum. Now (b) implies that has an infimum.  Conversely, now suppose that every non-empty subset of which is bounded from below has an infimum, and let be a non-empty subset of which is bounded from above. By (a) above, is a non-empty subset of which is bounded from below and hence has a supremum. Now (b) implies that has a supremum. Thus is Dedekind complete.    "
},
{
...
{
  "id": "exercise-verifying-metric-properties-3",
  "level": "2",
  "url": "section-introduction-to-metric-geometry.html#exercise-verifying-metric-properties-3",
  "type": "Exercise",
  "number": "3.1.4.5",
  "title": "",
  "body": "  Let be the Euclidean metric on the real numbers ; that is, for all real numbers . Verify that is a metric on .    For all real numbers , if , then . Conversely, if , then . Since if and only if , is non-degenerate.  Since for all real numbers , is symmetric.  Finally, consider three points . If , then . If , then . If , then . If , then . If , then . Finally, if , then . In any case, since , satisfies the triangle inequality.  Since is non-degenerate, symmetric, and satisfies the triangle inequality, it is a metric on the real numbers .   "
},
{
  "id": "exercise-verifying-metric-properties-4",
  "level": "2",
  "url": "section-introduction-to-metric-geometry.html#exercise-verifying-metric-properties-4",
  "type": "Exercise",
  "number": "3.1.4.6",
  "title": "",
  "body": "  Let be the discrete metric on a set ; that is, for all points . Verify that is a metric on .    For all points , if , then . Conversely, if , then . Since if and only if , is non-degenerate.  Consider two points . If , then , and so . Conversely, if , then , and so . In either case, since , is symmetric.  Finally, consider three points . If , then , since takes only non-negative values. On the other hand, suppose that . Then it is not the case that and , and so at least one of and is . We conclude that . In either case, since , satisfies the triangle inequality.  Since is non-degenerate, symmetric, and satisfies the triangle inequality, it is a metric on .   "
},
{
  "id": "exercise-verifying-metric-properties-5",
  "level": "2",
  "url": "section-introduction-to-metric-geometry.html#exercise-verifying-metric-properties-5",
  "type": "Exercise",
  "number": "3.1.4.7",
  "title": "",
  "body": "  Let be the real-valued function defined by the formula . Determine whether or not is a metric on the real numbers .    We note that , and so is not symmetric. Since is not symmetric, it is not a metric on the real numbers .   "
},
{
  "id": "exercise-subspaces-of-discrete-metric-spaces",
  "level": "2",
  "url": "section-introduction-to-metric-geometry.html#exercise-subspaces-of-discrete-metric-spaces",
  "type": "Exercise",
  "number": "3.1.4.8",
  "title": "Subspaces of discrete metric spaces.",
  "body": " Subspaces of discrete metric spaces   Let be a subset of a discrete metric space . Verify that the metric induced on by is the discrete metric on .    Let be the inclusion map of into , and consider points . If , then , and so . On the other hand, since is injective, if , then , and so . In either case, , and so .   "
},
{
  "id": "exercise-scalar-multiples-of-metrics",
  "level": "2",
  "url": "section-introduction-to-metric-geometry.html#exercise-scalar-multiples-of-metrics",
  "type": "Exercise",
  "number": "3.1.4.9",
  "title": "Scalar multiples of metrics.",
  "body": " Scalar multiples of metrics   Give an example of a real number and a metric so that is not a metric.    Any non-positive real number and any metric on a set with at least two distinct points are correct answers.    Since implies that metrics take only non-negative values, any non-positive real number and any metric on a set with at least two distinct points are correct answers.   "
},
{
...
},
{
  ...
  "body": " Norm equivalence  norm equivalence   Two norms on a real or complex vector space are called equivalent if there are positive real numbers so that for all vectors .     Prove that norm equivalence is an equivalence relation on the set of norms on .    First let be a norm on , and note that for all vectors . Thus and are equivalent, and so norm equivalence is reflexive.  Now let be norms on , and suppose that and are equivalent. Then there are positive real numbers so that for all vectors , and so for all vectors . Thus and are equivalent, and so norm equivalence is symmetric.  Finally, let be norms on , and suppose that and are equivalent and that and are equivalent. Then there are positive real numbers so that and for all vectors , and so for all vectors . Thus and are equivalent, and so norm equivalence is transitive.  Since norm equivalence is reflexive, symmetric, and transitive, it is an equivalence relation on the set of norms on .      Prove that for a given natural number , any two -norms on -dimensional real coordinate space for are all equivalent.    We will show that any -norm on with is equivalent to the -norm; (a) above implies that this suffices. To that end, let , and first note that for all indices , and so .  On the other hand, we note that . Since for all vectors , and are equivalent.    "
},
{
...
{
  "id": "problem-reverse-triangle-inequality-for-norms",
  "level": "2",
  "url": "section-introduction-to-metric-geometry.html#problem-reverse-triangle-inequality-for-norms",
  "type": "Problem",
  "number": "3.1.5.4",
  "title": "Reverse triangle inequality for norms.",
  "body": " Reverse triangle inequality for norms   Prove that for all vectors in a normed vector space .    The absolute homogeneity and subadditivity of the norm on implies that and . Thus .   "
},
{
...
{
  "id": "exercise-convergence-in-your-own-words-3",
  "level": "2",
  "url": "section-convergence-and-limits.html#exercise-convergence-in-your-own-words-3",
  "type": "Exercise",
  "number": "3.2.4.1",
  "title": "",
...
{
  "id": "exercise-convergence-in-your-own-words-4",
  "level": "2",
  "url": "section-convergence-and-limits.html#exercise-convergence-in-your-own-words-4",
  "type": "Exercise",
  "number": "3.2.4.2",
  "title": "",
...
{
  "id": "exercise-convergence-in-your-own-words-5",
  "level": "2",
  "url": "section-convergence-and-limits.html#exercise-convergence-in-your-own-words-5",
  "type": "Exercise",
  "number": "3.2.4.3",
  "title": "",
...
{
  "id": "exercise-convergence-in-your-own-words-6",
  "level": "2",
  "url": "section-convergence-and-limits.html#exercise-convergence-in-your-own-words-6",
  "type": "Exercise",
  "number": "3.2.4.4",
  "title": "",
...
},
{
  ...
  "number": "3.2.4.5",
  "title": "",
  "body": "  Consider the sequence of real numbers defined by the formula . Determine whether the sequence converges or diverges in the Euclidean metric on the real numbers . If it converges, find the limit, and express your answer in limit notation.    The sequence converges to ; that is, .    Let be a positive real number. The Archimedean principle implies that for some natural number . We observe that for all indices . Thus the sequence converges to ; that is, .   "
},
{
  ...
  "number": "3.2.4.6",
  "title": "",
  "body": "  Consider the sequence of real numbers defined by the formula . Determine whether the sequence converges or diverges in the Euclidean metric on the real numbers . If it converges, find the limit, and express your answer in limit notation.    The sequence diverges.    Let be a real number. Then for all even indices , and so does not converge to . Since was chosen to be an arbitrary real number, the sequence diverges.   "
},
{
  ...
  "number": "3.2.4.7",
  "title": "",
  "body": "  Consider the sequence of real numbers defined by the formula . Determine whether the sequence converges or diverges in the Euclidean metric on the real numbers . If it converges, find the limit, and express your answer in limit notation.    The sequence converges to ; that is, .    Let be a positive real number, and observe that for all indices . Thus the sequence converges to ; that is, .   "
},
{
...
},
{
  ...
  "number": "3.2.4.10",
  "title": "",
  "body": "  Consider the sequence of real numbers defined by the formula . Determine whether the sequence converges or diverges in the Euclidean metric on the real numbers . If it converges, find the limit, and express your answer in limit notation.    The sequence converges to ; that is, .    Let be a positive real number. The argument above in (5) implies that there is a natural number so that for all indices . We observe that for all indices , and so for all indices . Thus the sequence converges to ; that is, .   "
},
{
...
},
{
  ...
  "number": "3.2.4.12",
  "title": "",
  "body": "  Consider the sequence of real-valued functions defined by the formulae .     Determine whether the sequence converges or diverges pointwise in the Euclidean metric on the real numbers . If it converges pointwise, find the pointwise limit. Express your answer using limit notation.    Let be the function defined by the formula . Then the sequence converges pointwise to ; that is, .    Let be the function defined by the formula . Fix a point , and let be a positive real number. We observe that if , then . Similarly, we observe that if , then . The Archimedean principle implies that for some natural number . We observe that for all indices . Thus the sequence converges to . Since this holds for all points , the sequence converges pointwise to ; that is, .      Determine whether the sequence converges or diverges uniformly in the Euclidean metric on the real numbers . If it converges uniformly, find the uniform limit.    The sequence does not converge uniformly.    The dependence of on in the above solution for (a) is a good sign that the sequence does not converge uniformly. Indeed, we observe that for all positive integers . Thus the sequence does not converge uniformly to . Since converges pointwise to , this implies that the sequence does not converge uniformly.    "
},
{
  ...
  "number": "3.2.4.13",
  "title": "",
  "body": "  Consider the sequence of real-valued functions defined by the formulae .     Determine whether the sequence converges or diverges pointwise in the Euclidean metric on the real numbers . If it converges pointwise, find the pointwise limit. Express your answer using limit notation.    Let be the constant function defined by the formula . Then the sequence converges pointwise to ; that is, .    Let be the constant function defined by the formula . Fix a point , and let be a positive real number. The Archimedean principle implies that for some natural number . We observe that for all indices . Thus the sequence converges to . Since this holds for all points , the sequence converges pointwise to ; that is, .      Determine whether the sequence converges or diverges uniformly in the Euclidean metric on the real numbers . If it converges uniformly, find the uniform limit.    Let be the constant function defined by the formula . Then the sequence converges uniformly to ; that is, .    The dependence of on in the above solution for (a) is a good sign that the sequence does not converge uniformly. However, the bounds on the input allow us to obtain uniform convergence.  Let be a positive real number. The Archimedean principle implies that for some natural number . We observe that for all indices and points . Thus the sequence converges uniformly to ; that is, .    "
},
{
...
},
{
  ...
  "number": "3.2.4.22",
  "title": "",
  "body": "  Consider the real-valued function defined piecewise by the formula . Determine whether or not converges at . If converges at , find the limit and express your answer using limit notation.     does not converge at .    We observe that and . If converged at , then the above limits to have the same value. Since they do not have the same value, does not converge at .   "
},
{
  ...
  "number": "3.2.4.23",
  "title": "",
  "body": "  Consider the real-valued function defined by the formula . Determine whether or not converges at . If converges at , find the limit and express your answer using limit notation.     converges at to ; that is, .    Let be a positive real number, and let . We observe that for all so that , and so converges to at . Since is a limit point of , this limit is unique; that is, .   "
},
{
  ...
  "number": "3.2.4.24",
  "title": "",
  "body": "  Consider the real-valued function defined by the formula . Determine whether or not converges at . If converges at , find the limit and express your answer using limit notation.    Use the fact that for all non-negative real numbers .     .    Let be a positive real number, and let . We observe that if , then , and so . We conclude that for all so that , and so converges to at . Since is a limit point of , this limit is unique; that is, .   "
},
{
...
{
  "id": "problem-alternative-characterization-of-uniform-convergence",
  "level": "2",
  "url": "section-convergence-and-limits.html#problem-alternative-characterization-of-uniform-convergence",
  "type": "Problem",
  "number": "3.2.5.3",
  "title": "Alternative characterization of uniform convergence.",
  "body": " Alternative characterization of uniform convergence   Let be a sequence of maps from a non-empty set to a metric space . Prove that converges to a map from to if and only if for all positive real numbers , there is a natural number so that for all indices .    We observe that for a given index , if and only if for all inputs . Thus the sequence converges uniformly to if and only if there is a natural number so that either (and therefore both) of the above conditions holds for all indices .   "
},
{
  "id": "problem-uniqueness-of-map-limits",
  "level": "2",
  "url": "section-convergence-and-limits.html#problem-uniqueness-of-map-limits",
  "type": "Problem",
  "number": "3.2.5.4",
  "title": "Uniqueness of map limits.",
  "body": " Uniqueness of map limits   Let be a map from a metric space to a metric space which has at least two distinct points. Prove the following partial converse to : If converges at a point to a unique point , then is a limit point of .    By contraposition. Suppose that is an isolated point of . Then there are no sequences of points which converge to , and so for each point , implies that converges at to . Since has at least two distinct points, then does not converge to a unique limit at .  In summary, we have shown that if is an isolated point, then does not converge to a unique limit at . Equivalently, if converges at to a unique limit, then is a limit point of .   "
},
{
...
{
  "id": "exercise-limits-and-arithmetic-5",
  "level": "2",
  "url": "section-limits-in-the-euclidean-metric.html#exercise-limits-and-arithmetic-5",
  "type": "Exercise",
  "number": "3.3.4.3",
  "title": "",
  "body": "  Compute .     .    We compute .   "
...
},
{
  ...
  "body": " Limits and scalar multiplication   Let be a norm on a real or complex vector space . Prove that for any convergent sequences and of scalars and vectors .    Let and . Let be a positive real number. . and together imply that .  In summary, we have shown that and that . implies that , and so by .   "
},
{
  ...
  "body": " Limits and division   Prove that for every sequence of non-zero real numbers which converges to a non-zero real number limit .    Let . Since converges to and , there is a natural number so that for all indices . We observe that if , then . In particular, for all indices . Thus .   "
},
{
  ...
  "body": " Squeezing to infinity   Let and be sequences of real numbers .     Prove that if for all indices and diverges to , then diverges to .    Let be a real number. Since diverges to , there is a natural number so that for all indices . We observe that for all indices . Thus the sequence diverges to .      Prove that if for all indices and diverges to , then diverges to .    Let be a real number. Since diverges to , there is a natural number so that for all indices . We observe that for all indices . Thus the sequence diverges to .     We can interpret the above results as versions of the for divergence to infinity.   "
},
{
...
},
{
  ...
  "body": " Divergence and upper\/lower limits   In this problem, you will complete the proof of . Let be a sequence of real numbers .     Prove that diverges to if and only if .    We note that for a natural number and a real number , for all indices if and only if . First suppose that the sequence diverges to . Then for all real numbers , there is a natural number so that for all indices . We observe that for all indices , and so the sequence diverges to ; that is, .  Conversely, now suppose that , and let be a real number. Then there is a natural number so that for all indices . In particular, for all indices . Thus the sequence diverges to .      Prove that diverges to if and only if .    We note that for a natural number and a real number , for all indices if and only if . First suppose that the sequence diverges to . Then for all real numbers , there is a natural number so that for all indices . We observe that for all indices , and so the sequence diverges to ; that is, .  Conversely, now suppose that , and let be a real number. Then there is a natural number so that for all indices . In particular, for all indices . Thus the sequence diverges to .    "
},
{
  ...
  "body": " Open and Closed Sets   In this section, we generalize the notion of open and closed intervals to subsets of arbitrary metric spaces. Recall that the open interval between two real numbers is the subset of the real numbers defined as follows. . Similarly, the closed interval between two real numbers is the subset of the real numbers defined as follows. . Soon, we will develop the more general notion of the open and closed subsets of a metric space, which will play a crucial role in the remainder of this text.    Neighborhoods in Metric Spaces   Informally, a subset of a metric space is open if it contains all points which are close to its elements and closed if it contains all of its limit points. These are one of many provably equivalent definitions of openness and closedness. In order to formalize this, we make the following preliminary definitions.    Open ball; closed disk; sphere  open ball  closed disk  sphere  radius  center  open metric ball  closed metric disk  metric sphere       the open metric ball of radius and center          the closed metric disk of radius and center          the metric sphere of radius and center      Fix a positive real number and a point in a metric space .   Open metric ball  The open (metric) ball  in of radius  and center  is the subset of defined by .    Closed metric disk  The closed (metric) disk  in of radius  and center  is the subset of defined by .    Open metric ball  The (metric) sphere  in of radius  and center  is the subset of defined by .       So an open ball contains precisely the points of distance strictly less than its radius from its center; a closed disk contains precisely the points of distance at most its radius from its center; and a sphere contains precisely the points of distance precisely its radius from its center. Just as the difference between an open and a closed interval is the question of the inclusion of its endpoints, the distinction between open balls and closed disks is that a closed disk contains all points of distance precisely its radius from its center, while an open ball does not.  We will see that in many ways, this section is an investigation of the properties of subsets of metric spaces which contain or are contained in certain open balls. The following definition, that of a neighborhood , will be a useful notion for both the rest of this chapter and the next chapter as well. Informally, a subset of a metric space is a neighborhood of a point if it contains all points which are very close to that point.   Neighborhood  neighborhood   A subset is a neighborhood of a point in a metric space if contains an open ball centered at ; that is, is a neighborhood of if for some positive real number .     Neighborhoods   Every open ball and closed disk in a metric space is a neighborhood of its center.     Locality  locality  The notion of a neighborhood is our latest exposure to the conceptual metaphor of locality . We conceive of metric spaces by spatial reasoning, saying for example that for points in a metric space , if is a small positive number, then and are close .  This idea is pervasive throughout point-set topology, and it implicitly informs much of the relevant terminology. Just as your neighborhood contains all of the people closest to where you live, a neighborhood in a metric space contains all nearby points.   We now have the terminology to make the crucial and titular definitions of this section; that of open and closed subsets of a metric space. A subset of a metric space is open if it is a neighborhood of all of its points, and is closed if its complement is open.   Open\/closed subset  open subset  closed subset   Let be a subset of a metric space .   Open subset   is open in if it is a neighborhood of each of its points; that is, is open if for each point , there is a positive real number so that .    Closed subset   is closed in if its complement is open in ; that is, is closed if each point has a neighborhood in which is disjoint from .        Open balls and closed disks   Open balls in metric spaces are open, and closed disks are closed.    Fix a positive real number and a point in a metric space . Let and be the open ball and closed disk of radius and center , respectively. We will show that and are open and closed in , respectively.  To that end, first let , so that . Let . We note that for a point , if , then , and so . Thus . Since contains an open ball centered at each of its points, it is a neighborhood of each of its points, and so is open in .  On the other hand, now suppose that , so that . Let . We note that for a point , if , then . We conclude that , and so . Since contains an open ball centered at each of its points, it is a neighborhood of each of its points, and so is open in ; that is, is closed in .     illustrates that the names open and closed for balls and disks are well-deserved; open balls and closed disks in a metric space are open and closed, respectively. This immediately implies that open intervals and closed intervals of real numbers (with finite endpoints) are open and closed, respectively, in the Euclidean metric, since any such interval is either a open ball or a closed disk (depending on whether or not it includes its endpoints) around its center, which is the arithmetic mean of its endpoints.  We next develop a series of point-set topological definitions based on the notion of neighborhoods which will play an important role in both this and the next chapter. Given a subset of a metric space, we may consider open or closed sets which approximate the set as well as possible from the inside or outside, respectively.   Interior; closure; boundary  interior point  interior  point of closure  closure  boundary point  boundary       the interior of          the closure of          the boundary of      Fix a subset of a metric space .   Interior  A point is an interior point of if is a neighborhood of . The set of interior points of is called the interior of and is denoted or .    Closure  A point is called a point of closure of in if for all neighborhoods of in . The set of points of closure of in is called the closure of in and is denoted or .    Boundary  A point is called a boundary point of in if and for all neighborhoods of in . The set of all boundary points of in is called the boundary of in and is denoted or .       The above definitions are equivalent if we quantify over open balls centered at the point in question rather than over neighborhoods of said point. However, the notion of neighborhood will extend into our discussion in , so we will continue in this spirit of generality, which we hope will ease our generalization of the above notions.   Interior, closure, and boundary   Consider the half-closed, half-open interval of real numbers between and .   Interior   has interior the open interval .    Closure   has closure the closed interval .    Boundary   has boundary the set .       Clearly, a subset of a metric space is open if and only if it is its own interior. Perhaps less obvious, however, is the second half of the following result; a subset of a metric space is closed if and only if it is its own closure.   Interior\/closure   Let be a subset of a metric space .   Interior   is open in if and only if .    Closure   is closed in if and only if .         We first note that for any point , there is some neighborhood of so that . Since is a neighborhood of , it contains some open ball . We conclude that , and so . We conclude that . Therefore, it suffices to show that is open in if and only if .  To that end, we first suppose that is open, and let . Then is a neighborhood of in , and of course . Thus is an interior point of . We conclude that .  Conversely, now suppose that , and let . Then , and so for some neighborhood of in . Since is a neighborhood of , it contains some open ball . We conclude that , and so is a neighborhood of in . Since this holds for all , is open in .    Let be a neighborhood of a point in . Then contains some open ball , and so . In particular, , and so is non-empty. Thus is a point of closure of in . We conclude that . Therefore, it suffices to show that is closed in if and only if .  To that end, we first suppose that is closed, and let . If , then our hypothesis that is closed implies that has a neighborhood in so that . This implies that . In summary, we have shown that , and so .  Conversely, now suppose that , and let . Then is not a point of closure of in , and so for some neighborhood of in . This implies that , and so is a neighborhood of in . Since this holds for all points , we conclude that is open in ; that is, is closed in .      So a subset of a metric space is open if and only if it is its own interior, and it is closed if and only if it is its own closure. One consequence of this is that the interior of a subset is open, and the closure of a set is closed. In order to show this, we make the following observation about the behavior of the openness and closedness conditions under the set operations.   Topology of a metric space   Let be a metric space.   The empty set and the ground set are both open and closed in .    The union of any collection of open subsets of is itself open in .    The intersection of any collection of closed subsets of is itself closed in .    The intersection of finitely many open subsets of is itself open in .    The union of finitely many closed subsets of is itself closed in .         The empty set is vacuously a neighborhood of each of its points, and so is open in . On the other hand, let . Then , and so is a neighborhood of in . We conclude that is open in .  Since the empty set and the ground set are both open in , their complements and are closed in .    Let be an indexed family of open sets in , and consider the union . Let . Then for some index . Since is open in , it is a neighborhood of in , and so for some positive real number .  We observe that , and so is a neighborhood of in . Since this holds for all points , is open in .    Let be an indexed family of closed sets in , and consider the intersection . Since for each index , is open in , (b) above implies that is open in . We conclude that is closed in .    Let be a finite sequence of open sets in , and consider the intersection . Let , so that for all indices . For each index , is open in , and so is a neighborhood of in ; that is, .  Let , and note that for all indices . We conclude that , so that is a neighborhood of in . Since this holds for all points , is open in .    Let be a finite sequence of closed sets in , and consider the union . Since for each index , is open in , (d) above implies that is open in . We conclude that is closed in .      Along with , the following result implies that the interior of a subset of a metric space is the largest open set which is contained in it, the closure of such a subset is the smallest closed set in which it is contained.   Interior\/closure   Let be a subset of a metric space .   Interior   has interior , where is the collection of open subsets of contained in .    Closure   has closure , where is the collection of closed subsets of which contain .         Let , and first suppose that is an interior point to in . Then is a neighborhood of in , and so for some positive real number . implies that is open in , and so . We conclude that .  Conversely, now suppose that . Then for some . Since is open in , it is a neighborhood of in , and so for some positive real number . Since , is a neighborhood of in ; that is, is an interior point of in .  In summary, we have shown that if and only if , and so .    Consider an element , and let . Since is closed in , is open in , and so is a neighborhood of each of its points. Moreover, since , , and so none of the points of are points of closure of in ; that is, . We conclude that . Since this holds for all , .  Conversely, now let . Then for some neighborhood of in . In particular, for some positive real number . Note that . However, , and so . We conclude that , and so .       Idempotence of interior\/closure   Let be a metric space.   Idempotence of interior   for all subsets .    Idempotence of closure   for all subsets .         (1) of implies that is a union of open sets in , and so is itself open in by (2) of . Thus by (1) of .    (2) of implies that is an intersection of closed sets in , and so is itself closed in by (3) of . Thus by (2) of .       For a subset of a metric space , the property of being a neighborhood of a point is the existence of a certain kind lower bound on in the power set of . We now examine the dual condition, that of metric boundedness .     Metrics and Boundedness   In this section, we have introduced open balls and closed disks of positive radius. While the notion of radius does not easily generalize to arbitrary subsets of metric spaces (even for just open balls and closed disks), the related concept of diameter is easily generalized from its meaning for Euclidean circles and spheres of higher dimension. Informally, the diameter of a subset of a metric space is the largest possible value taken by the metric when considering only points in that subset.    Diameter  diameter       the diameter of      The diameter of a subset of a metric space is the extended real number defined by .     Diameter and radius   Let be a positive real number and be a point in a metric space . The open ball , closed disk , and sphere of radius and center have diameter at most .    Since the open ball is non-empty. Since for all points , .  Similarly, since the closed disk is non-empty. Since for all points , .  If the sphere is empty, then . On the other hand, if the sphere is non-empty, then we observe that for all points , .    So the diameter of an open ball, closed disk, or sphere in a metric space is bounded above by twice its radius. Note that, unlike in Euclidean spaces, the diameter of an open ball, closed disk, or sphere in arbitrary metric space need not actually reach this upper bound. This is because, unlike in Euclidean space, in which a given open ball or closed disk has a unique center and radius, balls and disks of different centers and radii may be equal. In particular, the geometry of open balls, closed disks, and spheres may be quite different in a given metric space than our intuition from Euclidean space might suggest.   Monotonicity of the diameter   Let be subsets of a metric space . If , then .    If , then as well, and so . So we may suppose that is non-empty. We observe that for all points , and so . Thus we may additionally suppose that is non-empty. In this case,     Much of this section thus far has been almost entirely concerned with the properties of subsets of metric spaces which contain certain open balls. We now consider the opposite case; what can we say about subsets which are themselves contained in open balls of finite radius? Such subsets of metric spaces are called bounded .   Boundedness  bounded  unbounded   A subset of a metric space is called bounded (resp. unbounded ) if (resp. ). A metric space is called bounded (resp. unbounded ) if its ground set is bounded (resp. unbounded).    A priori, the above definition of boundedness does not appear to be related to the definition introduced in . Namely, that kind of boundedness requires a partial order, and this one requires a metric. However, there is a way to reconcile these perspectives, and the two notions coincide in the real numbers, taken with their standard ordering and the Euclidean metric .   Boundedness   The following are examples of bounded and unbounded subsets of metric spaces:   The empty set is a bounded subset of every metric space.    Any discrete metric space is bounded, since its diameter is either or .    The -dimensional Euclidean space is bounded when and unbounded when .       Any subset of a bounded subset of a metric space is itself bounded, and any set which contains an unbounded set is itself unbounded. Moreover, any subset of a nonempty metric space is bounded if and only if it is contained in an open ball or closed disk of finite radius.   Metric characterization of boundedness   A subset of a nonempty metric space is bounded if and only if it is contained in an open ball of finite radius.    Let be a subset of a non-empty metric space . First suppose that is bounded, so that . Since is non-empty, it contains a point . If is empty, then for any positive real number .  On the other hand, if is non-empty, then it contains a point . Let , and note that for all points , and so .  Conversely, now suppose that for some point . Then and imply that , so that is bounded.    In fact, something stronger is true for normed vector spaces; a subset of a normed vector space is bounded if and only if it is contained in some open ball of finite radius centered at the origin. This kind of result is typical of analysis done on normed vector spaces; the interplay between the norm and the vector space structure implies that most point-set topological concerns can be reduced to questions which are answerable in a neighborhood of the origin.   Norm characterization of boundedness   A subset of a normed vector space is bounded if and only if it is contained in an open ball of finite radius centered at the origin.    Let be a normed vector space. By , it suffices to show that any bounded subset of is contained in an open ball centered at . To that end, let be a bounded subset of . implies that for some positive real number and some vector .  Let , and observe that for all . Thus .    Any finite subset of a metric space is bounded. In fact, any finite union of bounded subsets of a metric space is itself bounded. In order to see this, it suffices to show that the binary union of two bounded subsets of a metric space is itself bounded.   Unions of bounded sets   The union of two bounded sets in a metric space is itself bounded.    Let be bounded subsets of a metric space , and consider the union . If is empty, then , and so , so that and are bounded. On the other hand, if is non-empty, then implies that and for some positive real numbers and points . Let , and consider a point . If , then , and if , then . In either case, we conclude that , and so . now implies that is bounded.    The following corollary follows immediately from the above result by induction. Essentially, it asserts that the property of boundedness is stable under finite unions.   Unions of bounded sets   The union of any finite collection of bounded subsets of a metric space is itself bounded.    Before we move on to discuss the boundedness of maps between metric spaces, we will briefly mention one stronger kind of boundedness for subsets of metric spaces. A subset of a metric space is called totally bounded if it can be covered by finitely many open balls of arbitrarily small radius.   Total boundedness  totally bounded   A subset of a metric space is called totally bounded if for all positive real numbers , there is a finite list of points so that . A metric space is called totally bounded if its ground set is totally bounded.     Total boundedness   The following are examples of totally bounded and not totally bounded subsets of metric spaces:   The empty set is a totally bounded subset of any metric space. More generally, any finite set of a metric space is totally bounded.    Any infinite subset of a discrete metric space is bounded but not totally bounded.    A subset of -dimensional Euclidean space is totally bounded if and only if it is bounded.       The above examples illustrate that while boundedness and total boundedness are not in general equivalent for subsets of metric spaces, the two notions are also not necessarily entirely unrelated. In fact, total boundedness is a stronger condition than boundedness for subsets of a metric space.   Total boundedness implies boundedness   If a subset of a metric space is totally bounded, then it is bounded.    Let be a totally bounded subset of a metric space . Since is a positive real number, for some finite list of points . and together imply that is bounded.    We have seen that infinite discrete metric spaces provide a counterexample to the converse of . However, in Euclidean space, total boundedness and boundedness are equivalent. We offer this result without proof.   Total boundedness in Euclidean space   A subset of Euclidean space is totally bounded if and only if it is bounded.     One way to see why is true is that any open ball or closed disk of finite radius in Euclidean space is a subset of a cube, which may be subdivided into finitely many appropriately small cubes. While boundedness and total boundedness are equivalent in Euclidean spaces, they are not the same for metric spaces in general. In some of the generalizations of theorems about Euclidean space to results about metric spaces in general, total boundedness is a more natural notion to discuss. We will further discuss totally bounded sets in the , in which we investigate completeness and compactness for metric spaces. For now, however, we move on in favor of exploring boundedness for maps between metric spaces.     Bounded maps between metric spaces   In , we defined boundedness for subsets of partially ordered sets, and then we defined a map from a set to a partially ordered set to be bounded if and only if its image was a bounded subset of the codomain. In this section, we have introduced a different notion of boundedness for subsets of a metric space, but an there is an analogous way to extend this notion to maps to metric spaces. A map to a metric space is bounded if its image is bounded in the codomain, and unbounded otherwise.  We note that this is a different notion than a bounded linear transformation between normed vector spaces, which is not bounded in this sense. The homogeneity of a norm on a vector space implies that no nonzero linear transformation to a normed vector space can be bounded in the above sense.}    Map boundedness  bounded  unbounded            Let be a map from a set to a metric space .   Boundedness  The map is called bounded if its image is a bounded subset of .    Unboundedness  The map is called unbounded if its image is an unbounded subset of .   The set of bounded maps from a set to a metric space is denoted . In the special case , the set of bounded real-valued functions on is denoted .     Boundedness for linear transformations  We remark that the above definition of boundedness is different than the usual definition of boundedness for linear maps between normed vector spaces. Indeed, in the setting of normed vector spaces, the absolute homogeneity of the norm implies that no non-zero linear transformation to a normed vector space can be bounded in the above sense. Instead, a linear transformation is called bounded if its restriction to the unit sphere around the origin is bounded in the above sense.    Bounded functions   Let be the real-valued function defined by the formula . Then is a bounded function on the real line .    Just as in the cases of sequential and map convergence, in attempting to understand boundedness in arbitrary metric spaces, it suffices to investigate boundedness in the real line . These links between arbitrary metric spaces and the metric space of the real numbers taken with the Euclidean metric stem from the definition of a metric; the structure of a real-valued function such as a metric can be seen to pull back some of the properties of its codomain to its domain. In order to check that a metric space is bounded, it is both necessary and sufficient for the metric itself to be a bounded function.   Map boundedness   A metric space is bounded if and only if the metric is a bounded function.    First suppose that is bounded, so that . If , then , and so is bounded. Thus we may suppose that is non-empty. In particular, for all points , which implies that for all points . In particular, this implies that . Now and imply that , so that is bounded.  Conversely, now suppose that is bounded. Then implies that for some positive real number . This implies that for all , so that ; that is, is bounded.    The set of bounded maps to a metric space may inherit some of the structure of the codomain. We will see some examples of this shortly. First, however, a useful observation about the relationship between the boundedness of maps and the boundedness of their codomains.   Maps to bounded metric spaces  Let be a metric space. If is bounded, then implies that for any set , any map is bounded; that is, .   There is a canonical way of constructing a metric on the set of bounded maps to a metric space.   Uniform metric  uniform metric       the uniform metric     Let be a set and be a metric space. The uniform metric  on the set of bounded maps from to is the map defined by the formula .     Uniform metric   The uniform metric is a metric.    Let be a set and be a metric space. If , then there is exactly one map from to , and this map is bounded. The uniform metric is identically zero in this case, and so is a metric on .  Thus we may suppose that is non-empty. Let be a bounded map from to , and note that if , then . Conversely, if , then for all inputs , so that . The non-degeneracy of the metric implies that for all inputs , and so . We conclude that is non-degenerate.  We observe that the symmetry of the metric implies that for all bounded maps , and so is symmetric.  Finally, we observe that the triangle inequality for the metric implies that for all bounded maps , and so is satisfies the triangle inequality.  Since is reflexive, symmetric, and satisfies the triangle inequality, it is a metric on .    You will have an opportunity to explore the uniform metric in the setting of normed vector spaces in the problems for this section. In particular, you will see that in this setting, the uniform metric is induced by a norm.  The similarity in the names of the uniform metric and uniformly convergent sequences of functions is no coincidence. In fact, it turns out that for bounded maps to a metric space, these two notions coincide.   Convergence in the uniform metric   Let be a sequence of bounded maps from a set to a metric space . For a bounded map , converges uniformly to if and only if it converges to in the uniform metric.      In the next section, we reintroduce one of the fundamental ideas in analysis and topology: that of continuity .    Exercises   The following exercises are designed to help you practice basic techniques and verify\/solidify your retention of the core ideas presented in each section. Answering these questions will typically involve applying known ideas in familiar ways, recalling information presented in this section, and building fluency with the basic tools of the course. You should approach these exercises as opportunities to make sure you can recognize, recall, and use the fundamental concepts accurately and efficiently before you are asked to answer questions which require complex problem-solving strategies.     Neighborhoods   Determine whether or not the given set is a neighborhood of the given point.     Let be the open interval between and . Determine whether or not is a neighborhood of the point in the metric space .     is a neighborhood of in .    Since contains the open ball of radius and center , is a neighborhood of in .      Let be the closed interval between and . Determine whether or not is a neighborhood of the point in the metric space .     is a neighborhood of in .    Since contains the open ball of radius and center , is a neighborhood of in .      Let be the half-closed, half-open interval between and . Determine whether or not is a neighborhood of the point in the metric space .     is not a neighborhood of in .    We note that for any positive real number , the open ball contains negative numbers. Thus any neighborhood of in contains negative numbers. Since every element of is non-negative, is not a neighborhood of in .      Let be the set of even integers. Determine whether or not is a neighborhood of the point in the discrete metric space .     is a neighborhood of in .    We note that since the discrete metric takes only the values and , . Since , , and so is a neighborhood of in .      Let be the closed interval between and . Determine whether or not is a neighborhood of the point in the metric subspace of the real line .     is a neighborhood of in .    Since contains the open ball of radius and center , is a neighborhood of in .      Open and closed sets   Determine whether the given subset of a metric space is open, closed, both open and closed, or neither open nor closed.     Let be the open interval between and . Determine whether or not is open or closed in the metric space .     is open in but not closed.      Let be the closed interval between and . Determine whether or not is open or closed in the metric space .     is closed in but not open.      Let be the half-closed, half-open interval between and . Determine whether or not is open or closed in the metric space .     is neither open nor closed in .      Let be the set of even integers. Determine whether or not is open or closed in the discrete metric space .     both open and closed in .      Let be the closed interval between and . Determine whether or not is open or closed in the metric subspace of the real line .     is closed in but not open.      Let be the closed interval between and . Determine whether or not is open or closed in the metric subspace of the real line .     is both open and closed in .       Diameter   Compute the diameter of the given subsets of metric spaces.     Let . Compute the diameter of as a subset of the metric space .     .      Let . Compute the diameter of as a subset of the discrete metric space .     .      Let . Compute the diameter of as a subset of the metric space .     .      Let be the set of integers which are divisible by . Compute the diameter of as a subset of the metric space .     .      Let . Compute the diameter of as a subset of the metric space .     .      Let . Compute the diameter of as a subset of the discrete metric space .     .      Bounded and unbounded subsets of metric spaces   Let be the open interval between and . Determine whether is a bounded or unbounded subset of the given metric space.     Determine whether is bounded or unbounded in the metric space .     is bounded in       Determine whether is bounded or unbounded in the subspace of the metric space .     is bounded in       Determine whether is bounded or unbounded in the metric space , where is the metric on defined by the formula .     is unbounded in .      Determine whether is bounded or unbounded in the discrete metric space .     is bounded in        The uniform metric   Compute the distance between the given bounded maps in the uniform metric.     Let be the constant real-valued functions defined by the formulae and . Compute the uniform distance .     .    We compute .      Let be the real-valued functions defined by the formulae and . Compute the uniform distance .     .    We compute .      Let be the real-valued functions defined by the formulae and . Compute the uniform distance .     .    We compute .      Short answers and\/or in-depth solutions to these exercises are available in . However, these are provided in order to check your work. If you are unable to answer the above questions, revisit the content of this section rather than spoiling the answers.     Problems   In contrast with the exercises, solving the following problems will require you to engage more deeply with the course material. These questions may require you to analyze a situation, connect multiple ideas, or develop a logical argument from first principles. Many problems will involve writing proofs, explaining your reasoning clearly, and justifying each step. You should expect these to be less routine and more exploratory: there may not be an obvious starting point, and persistence is part of the task .    Open and closed sets in metric subspaces   Let be a subset of a metric space .     Prove that a subset is open in the metric subspace if and only if for some open set in .       Prove that if a subset is open in , then it is open in .       Construct an explicit counterexample to the converse of (b) above.       Interior, closure, and boundary of complements   Let be a subset of a metric space .     Prove that .       Prove that .       Prove that .       Prove that .       Closure and limit points   Let be a subset of a metric space . Prove that .    Recall that the derived set  of a subset of a metric space is the set of all limit points of in .      Open balls in ultrametric spaces   Prove that any point in an open ball in an ultrametric space is its center.    You may find (c) of to be of use.      Diameter in the real line   Let be a non-empty subset of the real line . Prove that has diameter         In-depth solutions to these problems are available in . However, these are provided in order to check your work. If you are unable to solve the above problems, discuss your questions with classmates and\/or instructor rather than spoiling the answers.    "
},
{
...
},
{
  ...
  "body": " Interior, closure, and boundary   Consider the half-closed, half-open interval of real numbers between and .   Interior   has interior the open interval .    Closure   has closure the closed interval .    Boundary   has boundary the set .      "
},
{
...
{
  "id": "proposition-topology-of-a-metric-space",
  "level": "2",
  "url": "section-open-and-closed-sets.html#proposition-topology-of-a-metric-space",
  "type": "Proposition",
  "number": "3.4.10",
  "title": "Topology of a metric space.",
  "body": " Topology of a metric space   Let be a metric space.   The empty set and the ground set are both open and closed in .    The union of any collection of open subsets of is itself open in .    The intersection of any collection of closed subsets of is itself closed in .    The intersection of finitely many open subsets of is itself open in .    The union of finitely many closed subsets of is itself closed in .         The empty set is vacuously a neighborhood of each of its points, and so is open in . On the other hand, let . Then , and so is a neighborhood of in . We conclude that is open in .  Since the empty set and the ground set are both open in , their complements and are closed in .    Let be an indexed family of open sets in , and consider the union . Let . Then for some index . Since is open in , it is a neighborhood of in , and so for some positive real number .  We observe that , and so is a neighborhood of in . Since this holds for all points , is open in .    Let be an indexed family of closed sets in , and consider the intersection . Since for each index , is open in , (b) above implies that is open in . We conclude that is closed in .    Let be a finite sequence of open sets in , and consider the intersection . Let , so that for all indices . For each index , is open in , and so is a neighborhood of in ; that is, .  Let , and note that for all indices . We conclude that , so that is a neighborhood of in . Since this holds for all points , is open in .    Let be a finite sequence of closed sets in , and consider the union . Since for each index , is open in , (d) above implies that is open in . We conclude that is closed in .     "
},
{
  ...
  "number": "3.4.11",
  "title": "Interior\/closure.",
  "body": " Interior\/closure   Let be a subset of a metric space .   Interior   has interior , where is the collection of open subsets of contained in .    Closure   has closure , where is the collection of closed subsets of which contain .         Let , and first suppose that is an interior point to in . Then is a neighborhood of in , and so for some positive real number . implies that is open in , and so . We conclude that .  Conversely, now suppose that . Then for some . Since is open in , it is a neighborhood of in , and so for some positive real number . Since , is a neighborhood of in ; that is, is an interior point of in .  In summary, we have shown that if and only if , and so .    Consider an element , and let . Since is closed in , is open in , and so is a neighborhood of each of its points. Moreover, since , , and so none of the points of are points of closure of in ; that is, . We conclude that . Since this holds for all , .  Conversely, now let . Then for some neighborhood of in . In particular, for some positive real number . Note that . However, , and so . We conclude that , and so .     "
},
{
  "id": "corollary-idempotence-of-interior-closure",
  "level": "2",
  "url": "section-open-and-closed-sets.html#corollary-idempotence-of-interior-closure",
  "type": "Corollary",
  "number": "3.4.12",
  "title": "Idempotence of interior\/closure.",
  "body": " Idempotence of interior\/closure   Let be a metric space.   Idempotence of interior   for all subsets .    Idempotence of closure   for all subsets .         (1) of implies that is a union of open sets in , and so is itself open in by (2) of . Thus by (1) of .    (2) of implies that is an intersection of closed sets in , and so is itself closed in by (3) of . Thus by (2) of .     "
},
{
  "id": "subsection-neighborhoods-in-metric-spaces-24-1",
  "level": "2",
  "url": "section-open-and-closed-sets.html#subsection-neighborhoods-in-metric-spaces-24-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "metric boundedness "
},
{
  "id": "subsection-metrics-and-boundedness-2-1",
  "level": "2",
  "url": "section-open-and-closed-sets.html#subsection-metrics-and-boundedness-2-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "diameter "
...
{
  "id": "definition-diameter",
  "level": "2",
  "url": "section-open-and-closed-sets.html#definition-diameter",
  "type": "Definition",
  "number": "3.4.13",
  "title": "Diameter.",
  "body": " Diameter  diameter       the diameter of      The diameter of a subset of a metric space is the extended real number defined by .   "
},
{
  "id": "lemma-diameter-and-radius",
  "level": "2",
  "url": "section-open-and-closed-sets.html#lemma-diameter-and-radius",
  "type": "Lemma",
  "number": "3.4.14",
  "title": "Diameter and radius.",
  "body": " Diameter and radius   Let be a positive real number and be a point in a metric space . The open ball , closed disk , and sphere of radius and center have diameter at most .    Since the open ball is non-empty. Since for all points , .  Similarly, since the closed disk is non-empty. Since for all points , .  If the sphere is empty, then . On the other hand, if the sphere is non-empty, then we observe that for all points , .   "
},
{
  "id": "lemma-monotonicity-of-the-diameter",
  "level": "2",
  "url": "section-open-and-closed-sets.html#lemma-monotonicity-of-the-diameter",
  "type": "Lemma",
  "number": "3.4.15",
  "title": "Monotonicity of the diameter.",
  "body": " Monotonicity of the diameter   Let be subsets of a metric space . If , then .    If , then as well, and so . So we may suppose that is non-empty. We observe that for all points , and so . Thus we may additionally suppose that is non-empty. In this case,    "
},
{
  "id": "subsection-metrics-and-boundedness-7",
  "level": "2",
  "url": "section-open-and-closed-sets.html#subsection-metrics-and-boundedness-7",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "bounded "
...
{
  "id": "definition-metric-boundedness",
  "level": "2",
  "url": "section-open-and-closed-sets.html#definition-metric-boundedness",
  "type": "Definition",
  "number": "3.4.16",
  "title": "Boundedness.",
  "body": " Boundedness  bounded  unbounded   A subset of a metric space is called bounded (resp. unbounded ) if (resp. ). A metric space is called bounded (resp. unbounded ) if its ground set is bounded (resp. unbounded).   "
},
{
  "id": "example-boundedness",
  "level": "2",
  "url": "section-open-and-closed-sets.html#example-boundedness",
  "type": "Example",
  "number": "3.4.17",
  "title": "Boundedness.",
  "body": " Boundedness   The following are examples of bounded and unbounded subsets of metric spaces:   The empty set is a bounded subset of every metric space.    Any discrete metric space is bounded, since its diameter is either or .    The -dimensional Euclidean space is bounded when and unbounded when .      "
},
{
  "id": "proposition-metric-characterization-of-boundedness",
  "level": "2",
  "url": "section-open-and-closed-sets.html#proposition-metric-characterization-of-boundedness",
  "type": "Proposition",
  "number": "3.4.18",
  "title": "Metric characterization of boundedness.",
  "body": " Metric characterization of boundedness   A subset of a nonempty metric space is bounded if and only if it is contained in an open ball of finite radius.    Let be a subset of a non-empty metric space . First suppose that is bounded, so that . Since is non-empty, it contains a point . If is empty, then for any positive real number .  On the other hand, if is non-empty, then it contains a point . Let , and note that for all points , and so .  Conversely, now suppose that for some point . Then and imply that , so that is bounded.   "
},
{
  "id": "corollary-norm-characterization-of-boundedness",
  "level": "2",
  "url": "section-open-and-closed-sets.html#corollary-norm-characterization-of-boundedness",
  "type": "Corollary",
  "number": "3.4.19",
  "title": "Norm characterization of boundedness.",
  "body": " Norm characterization of boundedness   A subset of a normed vector space is bounded if and only if it is contained in an open ball of finite radius centered at the origin.    Let be a normed vector space. By , it suffices to show that any bounded subset of is contained in an open ball centered at . To that end, let be a bounded subset of . implies that for some positive real number and some vector .  Let , and observe that for all . Thus .   "
},
{
  "id": "proposition-unions-of-bounded-sets",
  "level": "2",
  "url": "section-open-and-closed-sets.html#proposition-unions-of-bounded-sets",
  "type": "Proposition",
  "number": "3.4.20",
  "title": "Unions of bounded sets.",
  "body": " Unions of bounded sets   The union of two bounded sets in a metric space is itself bounded.    Let be bounded subsets of a metric space , and consider the union . If is empty, then , and so , so that and are bounded. On the other hand, if is non-empty, then implies that and for some positive real numbers and points . Let , and consider a point . If , then , and if , then . In either case, we conclude that , and so . now implies that is bounded.   "
},
{
  "id": "corollary-unions-of-bounded-sets",
  "level": "2",
  "url": "section-open-and-closed-sets.html#corollary-unions-of-bounded-sets",
  "type": "Corollary",
  "number": "3.4.21",
  "title": "Unions of bounded sets.",
  "body": " Unions of bounded sets   The union of any finite collection of bounded subsets of a metric space is itself bounded.   "
},
{
  "id": "subsection-metrics-and-boundedness-19",
  "level": "2",
  "url": "section-open-and-closed-sets.html#subsection-metrics-and-boundedness-19",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "totally bounded "
},
{
  "id": "definition-total-boundedness",
  "level": "2",
  "url": "section-open-and-closed-sets.html#definition-total-boundedness",
  "type": "Definition",
  "number": "3.4.22",
  "title": "Total boundedness.",
  "body": " Total boundedness  totally bounded   A subset of a metric space is called totally bounded if for all positive real numbers , there is a finite list of points so that . A metric space is called totally bounded if its ground set is totally bounded.   "
},
{
  "id": "example-total-boundedness",
  "level": "2",
  "url": "section-open-and-closed-sets.html#example-total-boundedness",
  "type": "Example",
  "number": "3.4.23",
  "title": "Total boundedness.",
  "body": " Total boundedness   The following are examples of totally bounded and not totally bounded subsets of metric spaces:   The empty set is a totally bounded subset of any metric space. More generally, any finite set of a metric space is totally bounded.    Any infinite subset of a discrete metric space is bounded but not totally bounded.    A subset of -dimensional Euclidean space is totally bounded if and only if it is bounded.      "
},
{
  "id": "proposition-total-boundedness-implies-boundedness",
  "level": "2",
  "url": "section-open-and-closed-sets.html#proposition-total-boundedness-implies-boundedness",
  "type": "Proposition",
  "number": "3.4.24",
  "title": "Total boundedness implies boundedness.",
  "body": " Total boundedness implies boundedness   If a subset of a metric space is totally bounded, then it is bounded.    Let be a totally bounded subset of a metric space . Since is a positive real number, for some finite list of points . and together imply that is bounded.   "
},
{
  "id": "proposition-total-boundedness-in-euclidean-space",
  "level": "2",
  "url": "section-open-and-closed-sets.html#proposition-total-boundedness-in-euclidean-space",
  "type": "Proposition",
  "number": "3.4.25",
  "title": "Total boundedness in Euclidean space.",
  "body": " Total boundedness in Euclidean space   A subset of Euclidean space is totally bounded if and only if it is bounded.   "
},
{
  "id": "subsection-metrics-and-boundedness-26-1",
  "level": "2",
  "url": "section-open-and-closed-sets.html#subsection-metrics-and-boundedness-26-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "completeness compactness "
},
{
  "id": "subsection-bounded-maps-between-metric-spaces-2-1",
  "level": "2",
  "url": "section-open-and-closed-sets.html#subsection-bounded-maps-between-metric-spaces-2-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "bounded "
...
{
  "id": "definition-metric-map-boundedness",
  "level": "2",
  "url": "section-open-and-closed-sets.html#definition-metric-map-boundedness",
  "type": "Definition",
  "number": "3.4.26",
  "title": "Map boundedness.",
  "body": " Map boundedness  bounded  unbounded            Let be a map from a set to a metric space .   Boundedness  The map is called bounded if its image is a bounded subset of .    Unboundedness  The map is called unbounded if its image is an unbounded subset of .   The set of bounded maps from a set to a metric space is denoted . In the special case , the set of bounded real-valued functions on is denoted .   "
},
{
  "id": "remark-boundedness-for-linear-transformations",
  "level": "2",
  "url": "section-open-and-closed-sets.html#remark-boundedness-for-linear-transformations",
  "type": "Remark",
  "number": "3.4.27",
  "title": "Boundedness for linear transformations.",
  "body": " Boundedness for linear transformations  We remark that the above definition of boundedness is different than the usual definition of boundedness for linear maps between normed vector spaces. Indeed, in the setting of normed vector spaces, the absolute homogeneity of the norm implies that no non-zero linear transformation to a normed vector space can be bounded in the above sense. Instead, a linear transformation is called bounded if its restriction to the unit sphere around the origin is bounded in the above sense.  "
},
{
  "id": "example-bounded-functions",
  "level": "2",
  "url": "section-open-and-closed-sets.html#example-bounded-functions",
  "type": "Example",
  "number": "3.4.28",
  "title": "Bounded functions.",
  "body": " Bounded functions   Let be the real-valued function defined by the formula . Then is a bounded function on the real line .   "
},
{
  "id": "lemma-map-boundedness",
  "level": "2",
  "url": "section-open-and-closed-sets.html#lemma-map-boundedness",
  "type": "Lemma",
  "number": "3.4.29",
  "title": "Map boundedness.",
  "body": " Map boundedness   A metric space is bounded if and only if the metric is a bounded function.    First suppose that is bounded, so that . If , then , and so is bounded. Thus we may suppose that is non-empty. In particular, for all points , which implies that for all points . In particular, this implies that . Now and imply that , so that is bounded.  Conversely, now suppose that is bounded. Then implies that for some positive real number . This implies that for all , so that ; that is, is bounded.   "
},
{
  "id": "remark-maps-to-bounded-metric-spaces",
  "level": "2",
  "url": "section-open-and-closed-sets.html#remark-maps-to-bounded-metric-spaces",
  "type": "Remark",
  "number": "3.4.30",
  "title": "Maps to bounded metric spaces.",
  "body": " Maps to bounded metric spaces  Let be a metric space. If is bounded, then implies that for any set , any map is bounded; that is, .  "
},
{
  "id": "definition-uniform-metric",
  "level": "2",
  "url": "section-open-and-closed-sets.html#definition-uniform-metric",
  "type": "Definition",
  "number": "3.4.31",
  "title": "Uniform metric.",
  "body": " Uniform metric  uniform metric       the uniform metric     Let be a set and be a metric space. The uniform metric  on the set of bounded maps from to is the map defined by the formula .   "
},
{
  "id": "proposition-uniform-metric",
  "level": "2",
  "url": "section-open-and-closed-sets.html#proposition-uniform-metric",
  "type": "Proposition",
  "number": "3.4.32",
  "title": "Uniform metric.",
  "body": " Uniform metric   The uniform metric is a metric.    Let be a set and be a metric space. If , then there is exactly one map from to , and this map is bounded. The uniform metric is identically zero in this case, and so is a metric on .  Thus we may suppose that is non-empty. Let be a bounded map from to , and note that if , then . Conversely, if , then for all inputs , so that . The non-degeneracy of the metric implies that for all inputs , and so . We conclude that is non-degenerate.  We observe that the symmetry of the metric implies that for all bounded maps , and so is symmetric.  Finally, we observe that the triangle inequality for the metric implies that for all bounded maps , and so is satisfies the triangle inequality.  Since is reflexive, symmetric, and satisfies the triangle inequality, it is a metric on .   "
},
{
  "id": "theorem-convergence-in-the-uniform-metric",
  "level": "2",
  "url": "section-open-and-closed-sets.html#theorem-convergence-in-the-uniform-metric",
  "type": "Theorem",
  "number": "3.4.33",
  "title": "Convergence in the uniform metric.",
  "body": " Convergence in the uniform metric   Let be a sequence of bounded maps from a set to a metric space . For a bounded map , converges uniformly to if and only if it converges to in the uniform metric.   "
},
{
  "id": "section-open-and-closed-sets-6-1",
  "level": "2",
  "url": "section-open-and-closed-sets.html#section-open-and-closed-sets-6-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "continuity "
...
{
  "id": "exercise-neighborhoods-3",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-neighborhoods-3",
  "type": "Exercise",
  "number": "3.4.4.1",
  "title": "",
  "body": "  Let be the open interval between and . Determine whether or not is a neighborhood of the point in the metric space .     is a neighborhood of in .    Since contains the open ball of radius and center , is a neighborhood of in .   "
},
{
  "id": "exercise-neighborhoods-4",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-neighborhoods-4",
  "type": "Exercise",
  "number": "3.4.4.2",
  "title": "",
  "body": "  Let be the closed interval between and . Determine whether or not is a neighborhood of the point in the metric space .     is a neighborhood of in .    Since contains the open ball of radius and center , is a neighborhood of in .   "
},
{
  "id": "exercise-neighborhoods-5",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-neighborhoods-5",
  "type": "Exercise",
  "number": "3.4.4.3",
  "title": "",
  "body": "  Let be the half-closed, half-open interval between and . Determine whether or not is a neighborhood of the point in the metric space .     is not a neighborhood of in .    We note that for any positive real number , the open ball contains negative numbers. Thus any neighborhood of in contains negative numbers. Since every element of is non-negative, is not a neighborhood of in .   "
},
{
  "id": "exercise-neighborhoods-6",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-neighborhoods-6",
  "type": "Exercise",
  "number": "3.4.4.4",
  "title": "",
  "body": "  Let be the set of even integers. Determine whether or not is a neighborhood of the point in the discrete metric space .     is a neighborhood of in .    We note that since the discrete metric takes only the values and , . Since , , and so is a neighborhood of in .   "
},
{
  "id": "exercise-neighborhoods-7",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-neighborhoods-7",
  "type": "Exercise",
  "number": "3.4.4.5",
  "title": "",
  "body": "  Let be the closed interval between and . Determine whether or not is a neighborhood of the point in the metric subspace of the real line .     is a neighborhood of in .    Since contains the open ball of radius and center , is a neighborhood of in .   "
},
{
  "id": "exercise-open-and-closed-sets-3",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-open-and-closed-sets-3",
  "type": "Exercise",
  "number": "3.4.4.6",
  "title": "",
  "body": "  Let be the open interval between and . Determine whether or not is open or closed in the metric space .     is open in but not closed.   "
},
{
  "id": "exercise-open-and-closed-sets-4",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-open-and-closed-sets-4",
  "type": "Exercise",
  "number": "3.4.4.7",
  "title": "",
  "body": "  Let be the closed interval between and . Determine whether or not is open or closed in the metric space .     is closed in but not open.   "
},
{
  "id": "exercise-open-and-closed-sets-5",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-open-and-closed-sets-5",
  "type": "Exercise",
  "number": "3.4.4.8",
  "title": "",
  "body": "  Let be the half-closed, half-open interval between and . Determine whether or not is open or closed in the metric space .     is neither open nor closed in .   "
},
{
  "id": "exercise-open-and-closed-sets-6",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-open-and-closed-sets-6",
  "type": "Exercise",
  "number": "3.4.4.9",
  "title": "",
  "body": "  Let be the set of even integers. Determine whether or not is open or closed in the discrete metric space .     both open and closed in .   "
},
{
  "id": "exercise-open-and-closed-sets-7",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-open-and-closed-sets-7",
  "type": "Exercise",
  "number": "3.4.4.10",
  "title": "",
  "body": "  Let be the closed interval between and . Determine whether or not is open or closed in the metric subspace of the real line .     is closed in but not open.   "
},
{
  "id": "exercise-open-and-closed-sets-8",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-open-and-closed-sets-8",
  "type": "Exercise",
  "number": "3.4.4.11",
  "title": "",
  "body": "  Let be the closed interval between and . Determine whether or not is open or closed in the metric subspace of the real line .     is both open and closed in .   "
},
{
  "id": "exercise-diameter-3",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-diameter-3",
  "type": "Exercise",
  "number": "3.4.4.12",
  "title": "",
  "body": "  Let . Compute the diameter of as a subset of the metric space .     .   "
},
{
  "id": "exercise-diameter-4",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-diameter-4",
  "type": "Exercise",
  "number": "3.4.4.13",
  "title": "",
  "body": "  Let . Compute the diameter of as a subset of the discrete metric space .     .   "
},
{
  "id": "exercise-diameter-5",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-diameter-5",
  "type": "Exercise",
  "number": "3.4.4.14",
  "title": "",
  "body": "  Let . Compute the diameter of as a subset of the metric space .     .   "
...
{
  "id": "exercise-diameter-6",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-diameter-6",
  "type": "Exercise",
  "number": "3.4.4.15",
  "title": "",
  "body": "  Let be the set of integers which are divisible by . Compute the diameter of as a subset of the metric space .     .   "
},
{
  "id": "exercise-diameter-7",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-diameter-7",
  "type": "Exercise",
  "number": "3.4.4.16",
  "title": "",
  "body": "  Let . Compute the diameter of as a subset of the metric space .     .   "
...
{
  "id": "exercise-diameter-8",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-diameter-8",
  "type": "Exercise",
  "number": "3.4.4.17",
  "title": "",
  "body": "  Let . Compute the diameter of as a subset of the discrete metric space .     .   "
...
{
  "id": "exercise-bounded-and-unbounded-subsets-of-metric-spaces-3",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-bounded-and-unbounded-subsets-of-metric-spaces-3",
  "type": "Exercise",
  "number": "3.4.4.18",
  "title": "",
  "body": "  Determine whether is bounded or unbounded in the metric space .     is bounded in    "
},
{
  "id": "exercise-bounded-and-unbounded-subsets-of-metric-spaces-4",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-bounded-and-unbounded-subsets-of-metric-spaces-4",
  "type": "Exercise",
  "number": "3.4.4.19",
  "title": "",
  "body": "  Determine whether is bounded or unbounded in the subspace of the metric space .     is bounded in    "
},
{
  "id": "exercise-bounded-and-unbounded-subsets-of-metric-spaces-5",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-bounded-and-unbounded-subsets-of-metric-spaces-5",
  "type": "Exercise",
  "number": "3.4.4.20",
  "title": "",
  "body": "  Determine whether is bounded or unbounded in the metric space , where is the metric on defined by the formula .     is unbounded in .   "
},
{
  "id": "exercise-bounded-and-unbounded-subsets-of-metric-spaces-6",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-bounded-and-unbounded-subsets-of-metric-spaces-6",
  "type": "Exercise",
  "number": "3.4.4.21",
  "title": "",
  "body": "  Determine whether is bounded or unbounded in the discrete metric space .     is bounded in    "
},
{
  "id": "exercise-the-uniform-metric-3",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-the-uniform-metric-3",
  "type": "Exercise",
  "number": "3.4.4.22",
  "title": "",
  "body": "  Let be the constant real-valued functions defined by the formulae and . Compute the uniform distance .     .    We compute .   "
},
{
  "id": "exercise-the-uniform-metric-4",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-the-uniform-metric-4",
  "type": "Exercise",
  "number": "3.4.4.23",
  "title": "",
  "body": "  Let be the real-valued functions defined by the formulae and . Compute the uniform distance .     .    We compute .   "
},
{
  "id": "exercise-the-uniform-metric-5",
  "level": "2",
  "url": "section-open-and-closed-sets.html#exercise-the-uniform-metric-5",
  "type": "Exercise",
  "number": "3.4.4.24",
  "title": "",
  "body": "  Let be the real-valued functions defined by the formulae and . Compute the uniform distance .     .    We compute .   "
...
{
  "id": "problem-open-and-closed-sets-in-metric-subspaces",
  "level": "2",
  "url": "section-open-and-closed-sets.html#problem-open-and-closed-sets-in-metric-subspaces",
  "type": "Problem",
  "number": "3.4.5.1",
  "title": "Open and closed sets in metric subspaces.",
  "body": " Open and closed sets in metric subspaces   Let be a subset of a metric space .     Prove that a subset is open in the metric subspace if and only if for some open set in .       Prove that if a subset is open in , then it is open in .       Construct an explicit counterexample to the converse of (b) above.     "
},
{
  "id": "problem-interior-closure-and-boundary-of-complements",
  "level": "2",
  "url": "section-open-and-closed-sets.html#problem-interior-closure-and-boundary-of-complements",
  "type": "Problem",
  "number": "3.4.5.2",
  "title": "Interior, closure, and boundary of complements.",
  "body": " Interior, closure, and boundary of complements   Let be a subset of a metric space .     Prove that .       Prove that .       Prove that .       Prove that .     "
},
{
  "id": "problem-closure-and-limit-points",
  "level": "2",
  "url": "section-open-and-closed-sets.html#problem-closure-and-limit-points",
  "type": "Problem",
  "number": "3.4.5.3",
  "title": "Closure and limit points.",
  "body": " Closure and limit points   Let be a subset of a metric space . Prove that .    Recall that the derived set  of a subset of a metric space is the set of all limit points of in .    "
},
{
  "id": "problem-open-balls-in-ultrametric-spaces",
  "level": "2",
  "url": "section-open-and-closed-sets.html#problem-open-balls-in-ultrametric-spaces",
  "type": "Problem",
  "number": "3.4.5.4",
  "title": "Open balls in ultrametric spaces.",
  "body": " Open balls in ultrametric spaces   Prove that any point in an open ball in an ultrametric space is its center.    You may find (c) of to be of use.    "
},
{
  "id": "problem-diameter-in-the-real-line",
  "level": "2",
  "url": "section-open-and-closed-sets.html#problem-diameter-in-the-real-line",
  "type": "Problem",
  "number": "3.4.5.5",
  "title": "Diameter in the real line.",
  "body": " Diameter in the real line   Let be a non-empty subset of the real line . Prove that has diameter     "
},
{
  ...
  "body": " Notions of Metric Continuity   In this section, we present three distinct but related notions of continuity on a metric space. Informally, these forms of continuity are conditions related to the nature of the convergence of a function to its value at a point. We will in this section make this precise, and investigate the structure of continuous maps between metric spaces.    Continuous Maps and Functions   In calculus, you may have learned that a function of the real numbers is continuous if the function commutes with limits, if limits of the function agree with the values of the function. We will see that this is also the case for maps of metric spaces. More precisely, map of metric spaces is continuous at a point in its domain if its values can be made arbitrarily close to the image of that point just by choosing points close to that point.    Characterizations of metric continuity   Let be a map from a metric space to a metric space . For a point , the following are equivalent:   Convergence characterization of continuity  The map converges at to .     - characterization of continuity  For all positive real numbers , there is a positive real number so that for all points , if , then .    Neighborhood characterization of continuity  The pre-image of any neighborhood of in is a neighborhood of in .    Sequential characterization of continuity  For every sequence of points , if , then .       First suppose that converges at to , and let . Then there exists a positive real number so that for all points , if , then . We note that if , then the non-degeneracy of implies that . In this case, , and so .  Now suppose that for all positive real numbers , there is a positive real number so that for all points , if , then , and let be a neighborhood of in . Then for some positive real number .  Since is positive, there is a positive real number so that for all points , if , then . In particular, , and so ; that is, is a neighborhood of in .  Now suppose that the pre-image of any neighborhood of in is a neighborhood of in , and let be a sequence of points so that . Let be a positive real number. Then is a neighborhood of in , and so is a neighborhood of in . Thus for some positive real number . Since converges to , there is some natural number so that for all indices . This implies that for all indices , and so for all such indices. We conclude that .  Finally, suppose that for every sequence of points , if , then . Then implies that converges at to .     Continuity  continuity  discontinuity       the space of continuous maps from to          the space of continuous real-valued functions on      Let be a map from a metric space to a metric space .   Continuity   is continuous at a point if it satisfies any (and therefore all) of the conditions in . is continuous if it is continuous at every point .    Discontinuity   is discontinuous at a point if it does not satisfies any of the conditions in . is discontinuous if it is discontinuous at some point .   The set of continuous maps from to is denoted . In the special case , the set of continuous, real-valued functions on a metric space is denoted .     Continuity   The following are examples of continuous and discontinuous maps.   Any constant map between metric spaces is continuous.    The identity map on a metric space is continuous. More generally, the inclusion map of a metric subspace is continuous.    Any map from a discrete metric space to a metric space is continuous.    Let be the real-valued function defined by the formula . Then is continuous. In particular, we note that is neither continuous nor discontinuous at , since is not in the domain of .  You may have been taught that has a discontinuity at the point , but we will require more specific language. We can describe the behavior of near by saying that does not have a continuous extension to .       A map of metric spaces is continuous at a point if and only if the outputs of the map can be made close to the image of that point solely by choosing inputs close to the point. Perhaps unsurprising in light of is the fact that continuity is totally determined by the behavior of the map on convergent sequences.In fact, unlike the more general spaces we will investigate in , the answers to all point-set topological questions about metric spaces are captured by the behavior of sequences. Perhaps more surprising is the following characterization of continuity in terms of open sets.   Open set characterization of metric continuity   A map from a metric space to a metric space is continuous if and only if the pre-image of any open subset of is an open subset of .    Suppose first that is continuous, and let be an open subset of . Let . Since is open, it is a neighborhood of , and so the neighborhood characterization of continuity in implies that is a neighborhood of . Since this is true of every point , is open in .  Conversely, now suppose that the pre-image of every open subset of is an open subset of , and consider a point and a neighborhood of in . Then for some positive real number . Since is open in , is open in and is therefore a neighborhood of each of its points.  In particular, , and so . We conclude that is a neighborhood of . The neighborhood characterization of continuity in now implies that is continuous at . Since this holds for all points , is continuous.    In general, proving that a given map between metric spaces is continuous can be annoyingly difficult. For example, if you want to use the - characterization of continuity, then finding a correct positive real number for each positive real number is not necessarily an easy task. The following results allow us to decompose such a problem into smaller parts, which may be much easier to solve than taking on the entire problem at once.   Composition of continuous maps   Let and be maps between metric spaces , , and . If is continuous at a point and is continuous at , then the composition is continuous at .    Let be a sequence of points , and suppose that . Since is continuous at , . Since is continuous at , . Thus is continuous at .     Vector arithmetic of continuous maps   Let be a metric space and be a normed vector space.   Vector addition  Let be maps from to . If and are continuous at a point , then their pointwise sum is continuous at .    Scalar multiplication  Let be a scalar and be a map from to . If is continuous at a point , then the scalar multiplication is continuous at .    Norm  Let be a map from to . If is continuous at a point , then the norm is continuous at .    Inner product  Let be maps from to . If the norm on is induced by an inner product and and are continuous at a point , then the pointwise inner product is continuous at .         Let be a positive real number. Since is continuous at , there is a positive real number so that for all points , if , then . Similarly, since is continuous at , there is a positive real number so that for all points , if , then .  Let , and note that for all points , if , then . We conclude that is continuous at .    Let be a sequence of points , and suppose that . Since is continuous at , we observe that , and so is continuous at .    By , it suffices to show that is a continuous real-valued function on . This follows directly from (3) of .    Let be a sequence of points , and suppose that . Since and are continuous at , we observe that , and so is continuous at .       Arithmetic of continuous functions   Let be real-valued functions on a metric space . If and are continuous at a point , then , , and are continuous at .  Moreover, if and are continuous at at point and for all points , then is continuous at .    Having introduced the notion of continuity for maps between metric spaces, one natural question is to describe the relationship between continuity and the limiting behavior of sequences of maps. At first glance, it may seem plausible that the limit of a sequence of continuous functions which converges pointwise must also be continuous. This, however, is not the case.   Discontinuous limits of sequences of continuous functions   Consider the sequence of continuous real-valued functions defined by the formulae . We recall that converges pointwise to the function defined piecewise by the formula . Even though each function is continuous, the pointwise limit is not continuous at .    So the pointwise limit of a sequence of continuous maps between metric spaces need not be continuous. However, if such a sequence of continuous maps converges uniformly, then the limit is a continuous map.   Uniform limits of continuous maps   The limit of a uniformly convergent sequence of continuous maps between metric spaces is continuous.    Let be a sequence of continuous maps from a metric space to a metric space , and suppose that converges uniformly to a map .  Fix a point , and let . Since converges uniformly to , there is a natural number so that for all indices and points . Fix one such index . Since is continuous, there is a positive real number so that for all points so that . We observe that for all points so that , and so is continuous at . Since this holds for all points , we conclude that is continuous.     So, while the pointwise limit of a sequence of continuous maps between metric spaces need not in general be continuous, any such sequence which converges uniformly must converge to a continuous limit. Uniformity, or the relative independence of the size of neighborhoods of points in a space from position, is a useful property which will allow us to reconcile complex limiting behavior where several limits interact. We now turn to a strengthening of the notion of continuity based on this idea of uniformity.     Stronger Forms of Metric Continuity   Continuity is a fundamental and useful tool for mathematical analysis of maps between metric spaces. However, there are many pathological examples of maps which are continuous and yet do not behave as well as we might like. Therefore, we now turn our attention to a stronger notion of continuity, that of uniform continuity. At the beginning of this section, we saw a definition for metric continuity which posited the existence of a positive real number for each point in the domain and each positive real number . Informally, a map between metric spaces is uniformly continuous if the associated to a given may be chosen independent of position in the domain.    Uniform continuity  uniform continuity       the set of uniformly continuous maps from to          the set of uniformly continuous real-valued functions on      A map from a metric space to a metric space is called uniformly continuous if for all positive real numbers , there is a positive real number so that for all points so that .  The set of uniformly continuous maps from to is denoted . In the special case , the set of uniformly continuous real-valued functions on is denoted .    Note that uniform continuity does not apply at a specific point, but rather over the entire domain of a map. It does not make sense to say that a map between metric spaces is uniformly continuous at a point.    Uniform continuity   The following are examples of uniformly continuous and not uniformly continuous maps between metric spaces:   Any constant map between metric spaces is uniformly continuous. Indeed, fix metric spaces and , and consider a point . The constant map from to defined by the formula satisfies the condition for all points . Thus for a given positive real number , we may choose any positive real number and check that this choice verifies the uniform continuity condition.    The inclusion map of a metric subspace is uniformly continuous. Indeed, fix a subset of a metric space , and note that the inclusion map satisfies the condition for all points . Thus for a given positive real number , we may choose and check that this choice verifies the uniform continuity condition.    The real-valued function defined by the formula is continuous but not uniformly continuous. Indeed, for any positive real number , let . We observe that , but , which implies that . In particular, for the positive real number , there is no positive real number for which the uniform continuity criterion is satisfied.       The definitions of continuity and uniform continuity are very similar at first glance. In fact, despite the above example of a continuous but not uniformly continuous function, it's not difficult to see that uniform continuity implies continuity.   Uniform continuity implies continuity   Any uniformly continuous map between metric spaces is continuous.    Let be a uniformly continuous map from a metric space to a metric space . Consider a point , and let be a positive real number. Since is uniformly continuous, there is a positive real number so that for all points so that . In particular, we observe that for all points so that , and so is continuous at . Since was chosen to be an arbitrary point in , is continuous.    Continuous maps between metric spaces (or other more general spaces) are fundamental objects in point-set topology. This is because lots of topological properties of spaces, which we will examine in the following section and chapter, are preserved under continuous maps. However, some of the more geometric properties, which start to approach notions of size, are not preserved under all continuous maps in general, but only under uniformly continuous maps. In the previous section, we introduced a notion of boundedness for subsets. While the image of a bounded subset of a metric space under a continuous map is not necessarily bounded, the image of a totally bounded subset of a metric space under a uniformly continuous map is totally bounded.   Uniform continuity and total boundedness   The image of a totally bounded subset of a metric space under a uniformly continuous map between metric spaces is totally bounded.    Let be a uniformly continuous map from a metric space to a metric space , and consider a totally bounded subset . We wish to show that is totally bounded in .  To that end, let be a positive real number. Since is uniformly continuous, there is a positive real number so that for all points so that . In particular, this implies that for all points .  Since is totally bounded in , there is a finite list so that . We observe that , and so is totally bounded in .    So the uniformly continuous image of a totally bounded subset of a metric space is totally bounded. Just as in the case of plain continuity, the set of uniformly continuous maps from a metric space to a normed vector space itself forms a vector space under pointwise addition and scalar multiplication.   Vector arithmetic and uniform continuity   Let be a metric space and be a normed vector space.   Vector addition  If two maps from to are uniformly continuous, then their pointwise sum is also uniformly continuous.    Scalar multiplication  If a map is uniformly continuous, then any pointwise scalar multiple is also uniformly continuous.    Norm  If a map is uniformly continuous, then its pointwise norm is also uniformly continuous.         Let be a positive real number. Since and are continuous, there are positive real numbers so that for all points so that and for all points so that . Let . We observe that for all points so that , and so is uniformly continuous.    Let be a positive real number. If , then is a constant map and therefore is uniformly continuous. We may therefore suppose that is non-zero.  Let be a positive real number. Since is uniformly continuous, there is a positive real number so that for all points so that . We observe that for all points so that , and so is uniformly continuous.    Let be a positive real number. Since is uniformly continuous, there is a positive real number so that for all points so that . We observe that implies that for all points so that , so that is uniformly continuous.      A natural question to ask is whether the same holds for the pointwise product of uniformly continuous real-valued functions on a metric space. In fact, we have seen that this is not necessarily the case: the real-valued function defined by the formula is uniformly continuous, but the pointwise product is not uniformly continuous. However, since uniformly continuous maps are continuous and because the product of two continuous, real-valued functions is continuous, the pointwise product of two uniformly continuous, real-valued functions on a metric space is continuous.  Just as in the case of continuous maps, the composition of uniformly continuous maps is uniformly continuous. This allows us to simplify the proofs of uniform continuity for a vast class of maps.   Composition of uniformly continuous maps   The composition of uniformly continuous maps between metric spaces is uniformly continuous.    Let and be uniformly continuous maps between metric spaces , , and . Let be a positive real number. Since is uniformly continuous, there is a positive real number so that for all points so that . Moreover, since is uniformly continuous, there is a positive real number so that for all points so that .  We observe that for all points if , then , and so . We conclude that is uniformly continuous.     Uniform continuity of a map has strong implications for its properties. However, there are even stronger notions of continuity.     Even Stronger Forms of Metric Continuity   We conclude this section with a look at a stronger form of continuity than uniform continuity. A map between metric spaces is called Lipschitz continuous if it changes the distance between points by at most some positive scalar multiple.    Lipschitz continuity  lipschitz continuity       the set of Lipschitz continuous maps from to          the set of Lipschitz continuous real-valued functions on      A map from a metric space to a metric space is called Lipschitz continuous if there is a non-negative real number so that for all points .  The set of Lipschitz continuous maps from a metric space to a metric space is denoted , and the set of Lipschitz continuous real-valued functions on is denoted .    As with uniform continuity, Lipschitz continuity does not apply at a specific point, but rather over the entire domain of a map. It does not make sense to say that a map between metric spaces is Lipschitz continuous at a point.    Norms are Lipschitz continuous   Let be a normed vector space. The implies that for all vectors , and so the norm is Lipschitz continuous.    For any Lipschitz continuous map between metric spaces, there is a unique smallest non-negative real number such that the above condition is satisfied. This unique smallest constant is called the Lipschitz constant for the map.   Lipschitz constant  Lipschitz constant       the Lipschitz constant of      The Lipschitz constant  of a map from a metric space to a metric space is the infimum .     Lipschitz constants and Lipschitz continuity   A map from a metric space to a metric space is Lipschitz continuous if and only if , in which case for all points .    We observe that is a subset of the real numbers, and so if and only if is non-empty. That is non-empty is precisely the Lipschitz continuity condition for , and so is Lipschitz continuous if and only if .  If is non-empty, let be a sequence of elements which converge to . We observe that for all points .    A map is Lipschitz continuous if it increases the distance between points by at most some nonnegative constant multiplier. This implies that Lipschitz continuity is a stronger condition than uniform continuity, and therefore is also a stronger condition than continuity.   Lipschitz continuity implies uniform continuity   If a map between metric spaces is Lipschitz continuous, then it is uniformly continuous and continuous.    Let be a map from a metric space to a metric space , and consider a positive real number . There are two cases; either , or .  If , then implies that for all points , and so for all points . In particular, this implies that is constant and therefore uniformly continuous.  If on the other hand , let , and note that implies that for all points so that . Thus is uniformly continuous. We conclude that is continuous by .     Uniform continuity does not imply Lipschitz continuity   Consider the real-valued function defined by the formula . For each positive real number , let . We observe that for all points , and so for all points so that . In particular, for all points so that , and so is uniformly continuous.  On the other hand, we observe that for any non-negative real number , so that . Thus . Since this holds for each non-negative real number , and so is not Lipschitz continuous by .    Just as in the case of continuity and uniform continuity, Lipschitz continuity is preserved by composition, although possibly at the cost of increasing the Lipschitz constant.   Lipschitz continuity and composition   Let and be maps between metric spaces , , and . If and are Lipschitz continuous, then so is their composition , and .    We observe that for all points . We conclude that the composition is Lipschitz continuous and .    Just as in the case of continuity and uniform continuity, Lipschitz continuous maps from a metric space to a normed vector space form a vector space under the pointwise operations of addition and scalar multiplication, although possibly at the cost of increasing the Lipschitz constant.   Vector arithmetic and Lipschitz continuity   Let be a metric space and be a normed vector space.   Vector addition  If two maps from to are Lipschitz continuous, then their pointwise sum is also Lipschitz continuous, and .    Scalar multiplication  If a map is Lipschitz continuous, then any pointwise scalar multiple is also Lipschitz continuous, and .    Norm  If a map is uniformly continuous, then its pointwise norm is also uniformly continuous, and .         We observe that for all points . We conclude that the composition is Lipschitz continuous and .    We observe that for all points . We conclude that the pointwise scalar multiple is Lipschitz continuous and .    Recall from that the norm is Lipschitz continuous with Lipschitz constant . Thus implies that the composition is Lipschitz continuous, with Lipschitz constant .       You will have several opportunities to explore applications of Lipschitz continuity in the problems for both this section and .     In this section, we have examined several nonequivalent notions of continuity for maps between metric spaces. In the next and final section of this chapter, we will introduce the notions of completeness and compactness for metric spaces, which both have to do with the convergence of sequences which morally should have a limit, but might not.    Exercises   The following exercises are designed to help you practice basic techniques and verify\/solidify your retention of the core ideas presented in each section. Answering these questions will typically involve applying known ideas in familiar ways, recalling information presented in this section, and building fluency with the basic tools of the course. You should approach these exercises as opportunities to make sure you can recognize, recall, and use the fundamental concepts accurately and efficiently before you are asked to answer questions which require complex problem-solving strategies.    Continuity at a point   Determine whether or not the given map between metric spaces is continuous at the given point.     Let be the linear function defined by the formula . Determine whether or not is continuous at .     is continuous at .    Let be a positive real number, and let . We observe that for all points so that . Thus is continuous at .      Let be the linear function defined by the formula . Determine whether or not is continuous at .     is continuous at .    Let be a positive real number, and let . We observe that for all points so that . Thus is continuous at .      Let be the linear function defined by the formula . Determine whether or not is continuous at .     is continuous at .    Let be a positive real number, and let . We observe that for all points so that . Thus is continuous at .      Uniform and Lipschitz continuity   Let be real numbers, and consider the real-valued function defined by the formula .     Verify that is not uniformly continuous.    We observe that for any positive real number , is the absolute value of a quadratic function of and is therefore unbounded from above. This implies that for any positive real number , there is some real number so that , and so is not uniformly continuous.      Without appealing to , verify that is not Lipschitz continuous.    We observe that for any points . Since is a quadratic polynomial in and , it is not bounded in absolute value, and so cannot be Lipschitz continuous.      Verify that the restriction of to a closed interval is uniformly continuous.    Let be a positive real number, and consider the positive real number . We observe that for all points so that . Thus the restriction of to is uniformly continuous.      Verify that the restriction of to a closed interval is Lipschitz continuous.    We observe that for all points . Thus the restriction of to is Lipschitz continuous, with Lipschitz constant .      Short answers and\/or in-depth solutions to these exercises are available in . However, these are provided in order to check your work. If you are unable to answer the above questions, revisit the content of this section rather than spoiling the answers.     Problems   In contrast with the exercises, solving the following problems will require you to engage more deeply with the course material. These questions may require you to analyze a situation, connect multiple ideas, or develop a logical argument from first principles. Many problems will involve writing proofs, explaining your reasoning clearly, and justifying each step. You should expect these to be less routine and more exploratory: there may not be an obvious starting point, and persistence is part of the task .    Continuity of rational functions    Prove using one or more characterizations of continuity that the real-valued function defined by the formula is continuous.       Argue that any polynomial function is continuous.       Argue that any rational function is continuous on its implicit domain.       Uniform limits of uniformly continuous maps   Let be a sequence of uniformly continuous maps from a metric space to a metric space . Prove that if converges uniformly, the uniform limit is also uniformly continuous.      Continuous pointwise limits of discontinuous maps   We have seen in and that uniform limits preserve continuity and uniform continuity. In this problem, we will construct counterexamples to the converses of these results.     Give an example of a sequence of continuous maps from a metric space to a metric space which converges pointwise but not uniformly to a continuous map from to .       Give an example of a sequence of discontinuous maps from a metric space to a metric space which converges uniformly to a continuous map from to .       Uniform limits of Lipschitz continuous maps   Consider the sequence of real-valued functions defined by the formula .     Show that is Lipschitz continuous for each index .       Show that the sequence converges uniformly, but the uniform limit is not Lipschitz continuous.       Prove that the limit of a uniformly convergent sequence of Lipschitz continuous maps from a metric space to a metric space is Lipschitz continuous if the sequence of Lipschitz constants is bounded from above.        In-depth solutions to these problems are available in . However, these are provided in order to check your work. If you are unable to solve the above problems, discuss your questions with classmates and\/or instructor rather than spoiling the answers.    "
},
{
  "id": "subsection-continuous-maps-and-functions-2-1",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#subsection-continuous-maps-and-functions-2-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "continuous "
...
{
  "id": "proposition-characterizations-of-metric-continuity",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#proposition-characterizations-of-metric-continuity",
  "type": "Proposition",
  "number": "3.5.1",
  "title": "Characterizations of metric continuity.",
  "body": " Characterizations of metric continuity   Let be a map from a metric space to a metric space . For a point , the following are equivalent:   Convergence characterization of continuity  The map converges at to .     - characterization of continuity  For all positive real numbers , there is a positive real number so that for all points , if , then .    Neighborhood characterization of continuity  The pre-image of any neighborhood of in is a neighborhood of in .    Sequential characterization of continuity  For every sequence of points , if , then .       First suppose that converges at to , and let . Then there exists a positive real number so that for all points , if , then . We note that if , then the non-degeneracy of implies that . In this case, , and so .  Now suppose that for all positive real numbers , there is a positive real number so that for all points , if , then , and let be a neighborhood of in . Then for some positive real number .  Since is positive, there is a positive real number so that for all points , if , then . In particular, , and so ; that is, is a neighborhood of in .  Now suppose that the pre-image of any neighborhood of in is a neighborhood of in , and let be a sequence of points so that . Let be a positive real number. Then is a neighborhood of in , and so is a neighborhood of in . Thus for some positive real number . Since converges to , there is some natural number so that for all indices . This implies that for all indices , and so for all such indices. We conclude that .  Finally, suppose that for every sequence of points , if , then . Then implies that converges at to .   "
},
{
  "id": "definition-metric-continuity",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#definition-metric-continuity",
  "type": "Definition",
  "number": "3.5.2",
  "title": "Continuity.",
  "body": " Continuity  continuity  discontinuity       the space of continuous maps from to          the space of continuous real-valued functions on      Let be a map from a metric space to a metric space .   Continuity   is continuous at a point if it satisfies any (and therefore all) of the conditions in . is continuous if it is continuous at every point .    Discontinuity   is discontinuous at a point if it does not satisfies any of the conditions in . is discontinuous if it is discontinuous at some point .   The set of continuous maps from to is denoted . In the special case , the set of continuous, real-valued functions on a metric space is denoted .   "
},
{
  "id": "example-metric-continuity",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#example-metric-continuity",
  "type": "Example",
  "number": "3.5.3",
  "title": "Continuity.",
  "body": " Continuity   The following are examples of continuous and discontinuous maps.   Any constant map between metric spaces is continuous.    The identity map on a metric space is continuous. More generally, the inclusion map of a metric subspace is continuous.    Any map from a discrete metric space to a metric space is continuous.    Let be the real-valued function defined by the formula . Then is continuous. In particular, we note that is neither continuous nor discontinuous at , since is not in the domain of .  You may have been taught that has a discontinuity at the point , but we will require more specific language. We can describe the behavior of near by saying that does not have a continuous extension to .      "
},
{
  "id": "corollary-open-set-characterization-of-metric-continuity",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#corollary-open-set-characterization-of-metric-continuity",
  "type": "Corollary",
  "number": "3.5.4",
  "title": "Open set characterization of metric continuity.",
  "body": " Open set characterization of metric continuity   A map from a metric space to a metric space is continuous if and only if the pre-image of any open subset of is an open subset of .    Suppose first that is continuous, and let be an open subset of . Let . Since is open, it is a neighborhood of , and so the neighborhood characterization of continuity in implies that is a neighborhood of . Since this is true of every point , is open in .  Conversely, now suppose that the pre-image of every open subset of is an open subset of , and consider a point and a neighborhood of in . Then for some positive real number . Since is open in , is open in and is therefore a neighborhood of each of its points.  In particular, , and so . We conclude that is a neighborhood of . The neighborhood characterization of continuity in now implies that is continuous at . Since this holds for all points , is continuous.   "
},
{
  "id": "proposition-composition-of-continuous-maps",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#proposition-composition-of-continuous-maps",
  "type": "Proposition",
  "number": "3.5.5",
  "title": "Composition of continuous maps.",
  "body": " Composition of continuous maps   Let and be maps between metric spaces , , and . If is continuous at a point and is continuous at , then the composition is continuous at .    Let be a sequence of points , and suppose that . Since is continuous at , . Since is continuous at , . Thus is continuous at .   "
},
{
  "id": "proposition-vector-arithmetic-of-continuous-maps",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#proposition-vector-arithmetic-of-continuous-maps",
  "type": "Proposition",
  "number": "3.5.6",
  "title": "Vector arithmetic of continuous maps.",
  "body": " Vector arithmetic of continuous maps   Let be a metric space and be a normed vector space.   Vector addition  Let be maps from to . If and are continuous at a point , then their pointwise sum is continuous at .    Scalar multiplication  Let be a scalar and be a map from to . If is continuous at a point , then the scalar multiplication is continuous at .    Norm  Let be a map from to . If is continuous at a point , then the norm is continuous at .    Inner product  Let be maps from to . If the norm on is induced by an inner product and and are continuous at a point , then the pointwise inner product is continuous at .         Let be a positive real number. Since is continuous at , there is a positive real number so that for all points , if , then . Similarly, since is continuous at , there is a positive real number so that for all points , if , then .  Let , and note that for all points , if , then . We conclude that is continuous at .    Let be a sequence of points , and suppose that . Since is continuous at , we observe that , and so is continuous at .    By , it suffices to show that is a continuous real-valued function on . This follows directly from (3) of .    Let be a sequence of points , and suppose that . Since and are continuous at , we observe that , and so is continuous at .     "
},
{
  "id": "corollary-arithmetic-of-continuous-functions",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#corollary-arithmetic-of-continuous-functions",
  "type": "Corollary",
  "number": "3.5.7",
  "title": "Arithmetic of continuous functions.",
  "body": " Arithmetic of continuous functions   Let be real-valued functions on a metric space . If and are continuous at a point , then , , and are continuous at .  Moreover, if and are continuous at at point and for all points , then is continuous at .   "
},
{
  "id": "example-discontinuous-limits-of-sequences-of-continuous-functions",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#example-discontinuous-limits-of-sequences-of-continuous-functions",
  "type": "Example",
  "number": "3.5.8",
  "title": "Discontinuous limits of sequences of continuous functions.",
  "body": " Discontinuous limits of sequences of continuous functions   Consider the sequence of continuous real-valued functions defined by the formulae . We recall that converges pointwise to the function defined piecewise by the formula . Even though each function is continuous, the pointwise limit is not continuous at .   "
},
{
  "id": "theorem-uniform-limits-of-continuous-maps",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#theorem-uniform-limits-of-continuous-maps",
  "type": "Theorem",
  "number": "3.5.9",
  "title": "Uniform limits of continuous maps.",
  "body": " Uniform limits of continuous maps   The limit of a uniformly convergent sequence of continuous maps between metric spaces is continuous.    Let be a sequence of continuous maps from a metric space to a metric space , and suppose that converges uniformly to a map .  Fix a point , and let . Since converges uniformly to , there is a natural number so that for all indices and points . Fix one such index . Since is continuous, there is a positive real number so that for all points so that . We observe that for all points so that , and so is continuous at . Since this holds for all points , we conclude that is continuous.   "
},
{
  "id": "subsection-stronger-forms-of-metric-continuity-2-1",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#subsection-stronger-forms-of-metric-continuity-2-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "uniformly continuous "
},
{
  "id": "definition-uniform-continuity",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#definition-uniform-continuity",
  "type": "Definition",
  "number": "3.5.10",
  "title": "Uniform continuity.",
  "body": " Uniform continuity  uniform continuity       the set of uniformly continuous maps from to          the set of uniformly continuous real-valued functions on      A map from a metric space to a metric space is called uniformly continuous if for all positive real numbers , there is a positive real number so that for all points so that .  The set of uniformly continuous maps from to is denoted . In the special case , the set of uniformly continuous real-valued functions on is denoted .   "
},
{
  "id": "example-uniform-continuity",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#example-uniform-continuity",
  "type": "Example",
  "number": "3.5.11",
  "title": "Uniform continuity.",
  "body": " Uniform continuity   The following are examples of uniformly continuous and not uniformly continuous maps between metric spaces:   Any constant map between metric spaces is uniformly continuous. Indeed, fix metric spaces and , and consider a point . The constant map from to defined by the formula satisfies the condition for all points . Thus for a given positive real number , we may choose any positive real number and check that this choice verifies the uniform continuity condition.    The inclusion map of a metric subspace is uniformly continuous. Indeed, fix a subset of a metric space , and note that the inclusion map satisfies the condition for all points . Thus for a given positive real number , we may choose and check that this choice verifies the uniform continuity condition.    The real-valued function defined by the formula is continuous but not uniformly continuous. Indeed, for any positive real number , let . We observe that , but , which implies that . In particular, for the positive real number , there is no positive real number for which the uniform continuity criterion is satisfied.      "
},
{
  "id": "proposition-uniform-continuity-implies-continuity",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#proposition-uniform-continuity-implies-continuity",
  "type": "Proposition",
  "number": "3.5.12",
  "title": "Uniform continuity implies continuity.",
  "body": " Uniform continuity implies continuity   Any uniformly continuous map between metric spaces is continuous.    Let be a uniformly continuous map from a metric space to a metric space . Consider a point , and let be a positive real number. Since is uniformly continuous, there is a positive real number so that for all points so that . In particular, we observe that for all points so that , and so is continuous at . Since was chosen to be an arbitrary point in , is continuous.   "
},
{
  "id": "proposition-uniform-continuity-and-total-boundedness",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#proposition-uniform-continuity-and-total-boundedness",
  "type": "Proposition",
  "number": "3.5.13",
  "title": "Uniform continuity and total boundedness.",
  "body": " Uniform continuity and total boundedness   The image of a totally bounded subset of a metric space under a uniformly continuous map between metric spaces is totally bounded.    Let be a uniformly continuous map from a metric space to a metric space , and consider a totally bounded subset . We wish to show that is totally bounded in .  To that end, let be a positive real number. Since is uniformly continuous, there is a positive real number so that for all points so that . In particular, this implies that for all points .  Since is totally bounded in , there is a finite list so that . We observe that , and so is totally bounded in .   "
},
{
  "id": "proposition-vector-arithmetic-and-uniform-continuity",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#proposition-vector-arithmetic-and-uniform-continuity",
  "type": "Proposition",
  "number": "3.5.14",
  "title": "Vector arithmetic and uniform continuity.",
  "body": " Vector arithmetic and uniform continuity   Let be a metric space and be a normed vector space.   Vector addition  If two maps from to are uniformly continuous, then their pointwise sum is also uniformly continuous.    Scalar multiplication  If a map is uniformly continuous, then any pointwise scalar multiple is also uniformly continuous.    Norm  If a map is uniformly continuous, then its pointwise norm is also uniformly continuous.         Let be a positive real number. Since and are continuous, there are positive real numbers so that for all points so that and for all points so that . Let . We observe that for all points so that , and so is uniformly continuous.    Let be a positive real number. If , then is a constant map and therefore is uniformly continuous. We may therefore suppose that is non-zero.  Let be a positive real number. Since is uniformly continuous, there is a positive real number so that for all points so that . We observe that for all points so that , and so is uniformly continuous.    Let be a positive real number. Since is uniformly continuous, there is a positive real number so that for all points so that . We observe that implies that for all points so that , so that is uniformly continuous.     "
},
{
  "id": "proposition-composition-of-uniformly-continuous-maps",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#proposition-composition-of-uniformly-continuous-maps",
  "type": "Proposition",
  "number": "3.5.15",
  "title": "Composition of uniformly continuous maps.",
  "body": " Composition of uniformly continuous maps   The composition of uniformly continuous maps between metric spaces is uniformly continuous.    Let and be uniformly continuous maps between metric spaces , , and . Let be a positive real number. Since is uniformly continuous, there is a positive real number so that for all points so that . Moreover, since is uniformly continuous, there is a positive real number so that for all points so that .  We observe that for all points if , then , and so . We conclude that is uniformly continuous.   "
},
{
  "id": "subsection-even-stronger-forms-of-metric-continuity-2-1",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#subsection-even-stronger-forms-of-metric-continuity-2-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
{
  "id": "definition-lipschitz-continuity",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#definition-lipschitz-continuity",
  "type": "Definition",
  "number": "3.5.16",
  "title": "Lipschitz continuity.",
  "body": " Lipschitz continuity  lipschitz continuity       the set of Lipschitz continuous maps from to          the set of Lipschitz continuous real-valued functions on      A map from a metric space to a metric space is called Lipschitz continuous if there is a non-negative real number so that for all points .  The set of Lipschitz continuous maps from a metric space to a metric space is denoted , and the set of Lipschitz continuous real-valued functions on is denoted .   "
},
{
  "id": "example-norms-are-Lipschitz-continuous",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#example-norms-are-Lipschitz-continuous",
  "type": "Example",
  "number": "3.5.17",
  "title": "Norms are Lipschitz continuous.",
  "body": " Norms are Lipschitz continuous   Let be a normed vector space. The implies that for all vectors , and so the norm is Lipschitz continuous.   "
},
{
  "id": "subsection-even-stronger-forms-of-metric-continuity-6",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#subsection-even-stronger-forms-of-metric-continuity-6",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "Lipschitz constant "
},
{
  "id": "definition-lipschitz-constant",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#definition-lipschitz-constant",
  "type": "Definition",
  "number": "3.5.18",
  "title": "Lipschitz constant.",
  "body": " Lipschitz constant  Lipschitz constant       the Lipschitz constant of      The Lipschitz constant  of a map from a metric space to a metric space is the infimum .   "
},
{
  "id": "proposition-lipschitz-constants-and-lipschitz-continuity",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#proposition-lipschitz-constants-and-lipschitz-continuity",
  "type": "Proposition",
  "number": "3.5.19",
  "title": "Lipschitz constants and Lipschitz continuity.",
  "body": " Lipschitz constants and Lipschitz continuity   A map from a metric space to a metric space is Lipschitz continuous if and only if , in which case for all points .    We observe that is a subset of the real numbers, and so if and only if is non-empty. That is non-empty is precisely the Lipschitz continuity condition for , and so is Lipschitz continuous if and only if .  If is non-empty, let be a sequence of elements which converge to . We observe that for all points .   "
},
{
  "id": "proposition-lipschitz-continuity-implies-uniform-continuity",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#proposition-lipschitz-continuity-implies-uniform-continuity",
  "type": "Proposition",
  "number": "3.5.20",
  "title": "Lipschitz continuity implies uniform continuity.",
  "body": " Lipschitz continuity implies uniform continuity   If a map between metric spaces is Lipschitz continuous, then it is uniformly continuous and continuous.    Let be a map from a metric space to a metric space , and consider a positive real number . There are two cases; either , or .  If , then implies that for all points , and so for all points . In particular, this implies that is constant and therefore uniformly continuous.  If on the other hand , let , and note that implies that for all points so that . Thus is uniformly continuous. We conclude that is continuous by .   "
},
{
  "id": "example-uniform-continuity-does-not-imply-lipschitz-continuity",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#example-uniform-continuity-does-not-imply-lipschitz-continuity",
  "type": "Example",
  "number": "3.5.21",
  "title": "Uniform continuity does not imply Lipschitz continuity.",
  "body": " Uniform continuity does not imply Lipschitz continuity   Consider the real-valued function defined by the formula . For each positive real number , let . We observe that for all points , and so for all points so that . In particular, for all points so that , and so is uniformly continuous.  On the other hand, we observe that for any non-negative real number , so that . Thus . Since this holds for each non-negative real number , and so is not Lipschitz continuous by .   "
},
{
  "id": "lemma-lipschitz-continuity-and-composition",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#lemma-lipschitz-continuity-and-composition",
  "type": "Lemma",
  "number": "3.5.22",
  "title": "Lipschitz continuity and composition.",
  "body": " Lipschitz continuity and composition   Let and be maps between metric spaces , , and . If and are Lipschitz continuous, then so is their composition , and .    We observe that for all points . We conclude that the composition is Lipschitz continuous and .   "
},
{
  "id": "proposition-vector-arithmetic-and-lipschitz-continuity",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#proposition-vector-arithmetic-and-lipschitz-continuity",
  "type": "Proposition",
  "number": "3.5.23",
  "title": "Vector arithmetic and Lipschitz continuity.",
  "body": " Vector arithmetic and Lipschitz continuity   Let be a metric space and be a normed vector space.   Vector addition  If two maps from to are Lipschitz continuous, then their pointwise sum is also Lipschitz continuous, and .    Scalar multiplication  If a map is Lipschitz continuous, then any pointwise scalar multiple is also Lipschitz continuous, and .    Norm  If a map is uniformly continuous, then its pointwise norm is also uniformly continuous, and .         We observe that for all points . We conclude that the composition is Lipschitz continuous and .    We observe that for all points . We conclude that the pointwise scalar multiple is Lipschitz continuous and .    Recall from that the norm is Lipschitz continuous with Lipschitz constant . Thus implies that the composition is Lipschitz continuous, with Lipschitz constant .     "
},
{
  "id": "section-notions-of-metric-continuity-6-1",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#section-notions-of-metric-continuity-6-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "completeness compactness "
...
{
  "id": "exercise-continuity-at-a-point-3",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#exercise-continuity-at-a-point-3",
  "type": "Exercise",
  "number": "3.5.4.1",
  "title": "",
  "body": "  Let be the linear function defined by the formula . Determine whether or not is continuous at .     is continuous at .    Let be a positive real number, and let . We observe that for all points so that . Thus is continuous at .   "
},
{
  "id": "exercise-continuity-at-a-point-4",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#exercise-continuity-at-a-point-4",
  "type": "Exercise",
  "number": "3.5.4.2",
  "title": "",
  "body": "  Let be the linear function defined by the formula . Determine whether or not is continuous at .     is continuous at .    Let be a positive real number, and let . We observe that for all points so that . Thus is continuous at .   "
...
{
  "id": "exercise-continuity-at-a-point-5",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#exercise-continuity-at-a-point-5",
  "type": "Exercise",
  "number": "3.5.4.3",
  "title": "",
  "body": "  Let be the linear function defined by the formula . Determine whether or not is continuous at .     is continuous at .    Let be a positive real number, and let . We observe that for all points so that . Thus is continuous at .   "
...
{
  "id": "exercise-uniform-and-lipschitz-continuity",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#exercise-uniform-and-lipschitz-continuity",
  "type": "Exercise",
  "number": "3.5.4.4",
  "title": "Uniform and Lipschitz continuity.",
  "body": " Uniform and Lipschitz continuity   Let be real numbers, and consider the real-valued function defined by the formula .     Verify that is not uniformly continuous.    We observe that for any positive real number , is the absolute value of a quadratic function of and is therefore unbounded from above. This implies that for any positive real number , there is some real number so that , and so is not uniformly continuous.      Without appealing to , verify that is not Lipschitz continuous.    We observe that for any points . Since is a quadratic polynomial in and , it is not bounded in absolute value, and so cannot be Lipschitz continuous.      Verify that the restriction of to a closed interval is uniformly continuous.    Let be a positive real number, and consider the positive real number . We observe that for all points so that . Thus the restriction of to is uniformly continuous.      Verify that the restriction of to a closed interval is Lipschitz continuous.    We observe that for all points . Thus the restriction of to is Lipschitz continuous, with Lipschitz constant .    "
},
{
  "id": "problem-continuity-of-rational-functions",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#problem-continuity-of-rational-functions",
  "type": "Problem",
  "number": "3.5.5.1",
  "title": "Continuity of rational functions.",
  "body": " Continuity of rational functions    Prove using one or more characterizations of continuity that the real-valued function defined by the formula is continuous.       Argue that any polynomial function is continuous.       Argue that any rational function is continuous on its implicit domain.     "
},
{
  "id": "problem-uniform-limits-of-uniformly-continuous-maps",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#problem-uniform-limits-of-uniformly-continuous-maps",
  "type": "Problem",
  "number": "3.5.5.2",
  "title": "Uniform limits of uniformly continuous maps.",
  "body": " Uniform limits of uniformly continuous maps   Let be a sequence of uniformly continuous maps from a metric space to a metric space . Prove that if converges uniformly, the uniform limit is also uniformly continuous.    "
},
{
  "id": "problem-continuous-pointwise-limits-of-discontinuous-maps",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#problem-continuous-pointwise-limits-of-discontinuous-maps",
  "type": "Problem",
  "number": "3.5.5.3",
  "title": "Continuous pointwise limits of discontinuous maps.",
  "body": " Continuous pointwise limits of discontinuous maps   We have seen in and that uniform limits preserve continuity and uniform continuity. In this problem, we will construct counterexamples to the converses of these results.     Give an example of a sequence of continuous maps from a metric space to a metric space which converges pointwise but not uniformly to a continuous map from to .       Give an example of a sequence of discontinuous maps from a metric space to a metric space which converges uniformly to a continuous map from to .     "
},
{
  "id": "problem-uniform-limits-of-lipschitz-continuous-maps",
  "level": "2",
  "url": "section-notions-of-metric-continuity.html#problem-uniform-limits-of-lipschitz-continuous-maps",
  "type": "Problem",
  "number": "3.5.5.4",
  "title": "Uniform limits of Lipschitz continuous maps.",
  "body": " Uniform limits of Lipschitz continuous maps   Consider the sequence of real-valued functions defined by the formula .     Show that is Lipschitz continuous for each index .       Show that the sequence converges uniformly, but the uniform limit is not Lipschitz continuous.       Prove that the limit of a uniformly convergent sequence of Lipschitz continuous maps from a metric space to a metric space is Lipschitz continuous if the sequence of Lipschitz constants is bounded from above.     "
},
{
  ...
  "body": " Completeness and Compactness   In this section, we will investigate two related notions which, like much of the material of this chapter, have to do with the convergence of sequences in metric spaces. First, we will reexamine the limiting behavior of sequences from a more intrinsic perspective, drawing a distinction between convergent sequences and Cauchy sequences . We will then examine the properties of complete metric spaces , in which this distinction does not exist. Next, we will discuss compact metric spaces , which are those in which, even if a sequence diverges, it has a convergent subsequence . Finally, to conclude this chapter, we will use both of these notions to prove several powerful results about metric spaces.    Cauchy Sequences and Metric Completeness   Thus far, we have examined the limiting behavior of sequences through an extrinsic perspective, focusing on the distance between the values of a given sequence and the points of the metric space. We now turn our attention to a more broad class of sequences, which are called Cauchy sequences , and which are viewed in a more intrinsic way. While a sequence in a metric space is convergent if its values eventually grow close to a specific point, it is Cauchy if its values eventually grow close to each other.    Cauchy sequence  Cauchy sequence   A sequence of points in a metric space is called Cauchy if for all positive real numbers , there is a natural number so that for all indices .     Cauchy criterion The above requirement is sometimes called the Cauchy criterion , so a sequence in a metric space satisfies the Cauchy criterion if the distances between its values eventually becomes negligible. Note the distinction between this notion and that of convergence: The entries of a convergent sequence must approach a specific point but the entries of a Cauchy sequence must approach each other (while not necessarily approaching a point of the metric space). At first glance, these notions might seem to be equivalent characterizations of sequences. However, we will see that in some metric spaces, this is not the case.   Cauchy sequence   For each natural number , let denote the th digit in the decimal expansion of , so that . Consider the sequence of rational numbers defined by the formula . Let be a positive real number. The Archimedean principle implies that for some natural number . We observe that for all indices . Thus is Cauchy.    The above example of Cauchy sequences shows that Cauchy sequences in a metric space need not converge; the given Cauchy sequence of rational numbers converges to and thus diverges in the metric space . However, any convergent sequence is Cauchy.   Convergent sequences are Cauchy   Any convergent sequence of points in a metric space is Cauchy.    Let be a sequence of points in a metric space , and suppose that converges to a limit .  Let be a positive real number. Since , there is a natural number so that for all indices . In particular, we observe that for all indices . We conclude that the sequence is Cauchy.    So the convergence of a sequence implies that it satisfies the Cauchy criterion. A natural follow-up question to the above result is whether or not it is possible for the reverse implication holds. The answer to this question actually depends on the particulars of the metric space. Such a metric space in which Cauchy sequences converge is called complete .   Metric completeness  metric completeness  metric incompleteness  Banach space   Let be a subset of a metric space .   Completeness  The subset is called complete in if every Cauchy sequence of points in converges in to a limit in .    Incompleteness  The subset is called incomplete in if some Cauchy sequence of points in does not converge in to a limit in .   A metric space is called complete (resp. incomplete ) if is complete (resp. incomplete) in . A complete normed vector space is called a Banach space .    Complete metric spaces are ideal for performing mathematical analysis, as it is often much easier to verify that a given sequence is Cauchy (and therefore has a limit) than to actually compute its limiting value.   Metric completeness   The following are examples of complete and incomplete metric spaces.   Any discrete metric space or finite metric space is complete, since any Cauchy sequence in a discrete metric space is eventually constant.    The rational numbers are not complete in the real line . Indeed, we have already seen an example of a Cauchy sequence of rational numbers which converges in to an irrational number .       The above fact about the rational number system is one of the reasons why it doesn't serve as a good context in which to perform mathematical analysis. However, as we will shortly see, the real number system (and, more generally, Euclidean space of any dimension) is a complete metric space, and so there is a rich theory of real analysis. These facts are themselves intimately related to the fact that the rational number system is not Dedekind complete, while the real number system is. This relationship between Dedekind completeness and completeness as a metric space is true for subspaces of the real numbers because of the relationship between the standard ordering of the real numbers and the point-set topology of the Euclidean metric, which we will examine in more detail in . We will prove that the real numbers are complete in the Euclidean metric shortly.  First, however, let us examine some of the properties of complete subsets of metric spaces in general. The following two results will be useful in determining whether or not a subset of a given metric space is complete. First, notice that any complete subset of a metric space needs to contain all of its limit points, and so must be closed.   Complete subsets are closed   A complete subset of a metric space is closed.    By contraposition. Let be a subset of a metric space , and suppose that is not closed in . Then by (2) of and , and so ; that is, there is some limit point which is not an element of .  Since is a limit point of in , for some sequence of points . Since converges in , it is Cauchy. However, the limit of this Cauchy sequence of points in does not lie in . Thus is not complete in .    So any complete subset of a metric space is closed. The converse to this statement is not, in general, true, since any metric space is closed in the metric, but not every metric space is complete. However, there is a partial converse to the above result; any closed subset of a complete metric space is also complete.   Closed subsets of complete metric spaces   A subset of a complete metric space is complete if and only if it is closed.    Let be a subset of a complete metric space . We have already seen in that if is complete in , then it is closed in . It remains to prove the converse implication. To that end, suppose that is not complete in , so that some Cauchy sequence of points does not converge to a limit in .  However, since is a Cauchy sequence in a complete metric space , it converges to a limit . Since for all indices , we conclude that is a limit point of in which is not contained in . implies that is a point of closure of in , and so is a proper subset of its closure . (2) of now implies that is not closed in .    We now would like to prove the completeness of various metric spaces associated with the real number system . First, however, let us consider the notion of a subsequence . Informally, a subsequence of a sequence is a sequence which takes some, but not necessarily all, of the same entries, and these entries appear in the same relative order. We can form a subsequence of a sequence by selecting some infinite subset of indices to include in the subsequence. In fact, any subsequence can be formed in this manner. In order to formalize this notion, recall that a sequence is formally a map whose domain is the natural numbers or the positive integers, and that an increasing function is an injective and non-decreasing function.   Subsequence  subsequence       a subsequence of      A sequence of points in a metric space is a subsequence of a sequence of points in if for some increasing function . In this case, if and , then we write .     Subsequences   The following are examples of subsequences:   The sequences and are subsequences of the sequence .    The sequence is a subsequence of the sequence .    The convergent sequence is a subsequence of the divergent sequence .       The above example shows that divergent sequences may have convergent subsequences. In fact, much of this section will be devoted to examining the properties of metric spaces in which this is the only possible situation. First, however, note that any subsequence of a convergent sequence in a metric space converges to the same limit. In order to show this, it helps to see the following fact about strictly increasing functions on the natural numbers.   Increasing functions    for any increasing function .    By induction on . For the base case , we observe that is a natural number, and so .  For the inductive step, suppose for the inductive hypothesis that for some natural number . We observe that , and so , since is non-decreasing. Moreover, , and so , since is injective. In particular, by the inductive hypothesis, and so .    We are now ready to show that a subsequence of a convergent sequence in a metric space is itself convergent, and converges to the same limit as the larger sequence. So while a subsequence of a divergent sequence might converge, the reverse situation cannot occur.   Subsequences of convergent sequences   Let be a sequence of points in a metric space . If converges in , then any subsequence also converges in , and .    Let , and consider a positive real number . There is some natural number so that for all indices .  We observe that implies that for all indices , and so for all such indices. Thus .    An analogous argument can be used to show that any subsequence of a sequence of real numbers which diverges to infinity must itself diverge to infinity. Taken together with , these results state that a subsequence of a sequence in a metric space with a limit shares that limit. We have seen that the opposite is not in general true; divergent sequences which do not approach a limit may have subsequences which do. However, this cannot happen if the original sequence was Cauchy; a Cauchy sequence in a metric space with a convergent subsequence converges to the limit of this subsequence.   Cauchy sequences with convergent subsequences   If a Cauchy sequence of points in a metric space has a convergent subsequence, then it converges.    Let be a Cauchy sequence of points in a metric space , and suppose that some subsequence converges to a point . We will show that the entire sequence also converges to .  To that end, let be a positive real number. Since is Cauchy, there is a natural number so that for all indices . Since the subsequence converges to , there is a natural number so that for all indices . Let . We observe that implies that for all indices . In particular, for all indices . Thus the sequence converges to .    The above result will come in handy in proving the completeness of the real numbers in the Euclidean metric. So will the following foundational theorem, the content of which is that any bounded sequence of real numbers has a subsequence which is convergent in the Euclidean metric.   Bolzano-Weierstrass theorem  Bernard Bolzano; Karl Weierstrass   Any bounded sequence of real numbers has a convergent subsequence.    Let be a sequence of real numbers which is bounded in the Euclidean metric on . For each natural number , consider the set . There are two cases: either has a maximum for every index or does not have a maximum for some index . We will show that in either case, has a monotone subsequence.  First suppose that has a maximum for each index . Consider the sequence of natural numbers defined recursively by . Since , the sequence is increasing, and so is a subsequence of . Moreover, we observe that for all indices , and so is non-increasing.  On the other hand, now suppose that has no maximum for some index . Consider the sequence of natural numbers defined recursively by . Since , the sequence is increasing, and so is a subsequence of . Moreover, we observe that for all indices , and so is increasing.  In either case, has a monotone subsequence . Since is bounded, so too is , and so implies that converges.    So any bounded sequence of real numbers has a subsequence which converges in the Euclidean metric. We will see that this implies that the real line is complete, and so is a Banach space under the Euclidean norm.   Cauchy sequences are bounded   Any Cauchy sequence of points in a metric space is bounded.    Let be a Cauchy sequence of points in a metric space . Then there is a natural number so that for all indices . Let . For all indices , if , then . On the other hand, if , then . Thus , and so the sequence is bounded.     Completeness of the real line   The real line is complete, and so is a Banach space.    Let be a Cauchy sequence of real numbers . Then is bounded by , and so it has a convergent subsequence by the . Since is a Cauchy sequence with a convergent subsequence, it is itself convergent by . Since every Cauchy sequence in converges, this metric space is complete.    So the real line is a complete metric space. As the Euclidean metric on is induced by the Euclidean norm, this implies that is a Banach space. That this completeness result is dependent on the Dedekind completeness of the real number system is not immediately obvious. However, in our proof of the , we used the fact that a bounded monotone sequence of real numbers converges to a real number, which requires the existence of upper and lower limits and is not true in ordered number systems such at the rational numbers in which these upper and lower limits do not necessarily exist.  In fact, the completeness of the real line implies that the normed vector spaces associated to the -norms are all also Banach spaces.   Completeness of the -norm    is a Banach space for all natural numbers and extended real numbers .    Let be a Cauchy sequence of points in the metric space . Let be a positive real number. Then there is a natural number so that for all indices . We observe that for each index , for all indices , and so is a Cauchy sequence of real numbers.   now implies that this sequence converges to some real number . Let . and now imply that converges to .  Since every Cauchy sequence in converges, this metric space is complete. We conclude that is a Banach space.     It may appear from the above result that almost all commonly appearing metric spaces are complete, but this is certainly not the case. The rational numbers, , for example, form a metric subspace of the real line which are not complete. We showed this by exhibiting a Cauchy sequence of rational numbers which converge to an irrational number, but this could also be observed by noting that the rational numbers are not a closed subset of the real numbers.  As the properties of complete metric spaces are incredibly useful for performing analysis, it is almost always preferable to work in a complete metric space. Shortly, we will see that this is usually possible, by passing to the completion of an incomplete metric space.     Completion of a Metric Space [SKIP]   Text before the first subdivision.    Text after the last subdivision.     Metric Compactness   When is it the case that a sequence in a metric space has a convergent subsequence? This is a natural question to ask about metric spaces. We have seen so far that this occurs when the original sequence converges, and when the sequence is a bounded sequence of real numbers, but this phenomenon is not limited to the case of convergent sequences. In some metric spaces, all sequences have convergent subsequences. These metric spaces are called (sequentially) compact , and we will devote the rest of this section to investigating the properties of sequentially compact metric spaces.    Sequential compactness  sequential compactness   A subset of a metric space is called (sequentially) compact if every sequence of points has a subsequence which converges to a limit in , and (sequentially) non-compact otherwise. A metric space is called compact (resp. non-compact ) if is compact (resp. non-compact).     Sequential compactness   The following are examples of compact and non-compact metric spaces:   A discrete metric space is compact if and only if the ground set is finite. More generally, any finite metric space is compact.    The -dimensional Euclidean space is compact if and non-compact if . In particular, the real line is non-compact.       The compactness of a metric space has profound implications for the behavior of continuous maps on that space. We will see in the next chapter that much can be said about continuous maps with compact (in the more general sense which we will introduce later) domains. For example, a continuous map with domain a compact metric space is uniformly continuous.   Heine-Cantor theorem  Eduard Heine; Georg Cantor   A continuous map from a compact metric space to a metric space is uniformly continuous.    Let be a map from a compact metric space to a metric space , and suppose that is not uniformly continuous. Then there is a positive real number so that for all positive real numbers , for some points with .  Let be a sequence of positive real numbers so that . For each index , there are points so that but . Since is compact, the sequence has a convergent subsequence ; that is, for some point . We will argue that also converges to . Indeed, let be a positive real number. Since converges to , there is some natural number so that for all indices . Moreover, since converges to , there is a natural number so that for all indices . Let , and note that implies that whenever . In particular, this implies that for all indices . Thus also converges to .  If were continuous at , then both and would converge to . In this case, we would see that This contradiction implies that is not continuous at .  In summary, we have shown that if is not uniformly continuous, then it is not continuous. Equivalently, if is continuous, then it is uniformly continuous.    We observe that there is no version of the for Lipschitz continuity; indeed, the function defined by the formula is continuous but not Lipschitz continuous, even though we will see that the closed interval is compact.  Continuity of a map between metric spaces is a local property which places bounds on the amount the map can increase the distance between close points. Compactness allows us to port this local property of continuity to a global property of uniform continuity, so that any continuous map is uniformly continuous on a compact subset of the domain. It is for this reason (that we may transfer many local geometric and topological properties to global properties) that compact subsets of metric spaces are so useful. We will see that compact metric spaces are also complete, and so it is often the case that the most salient information about a metric space are its compact subspaces.  In fact, more can be said about this situation. The image of a compact subset of a metric space under a continuous map is a compact subset of the codomain.   Continuous images of compact subsets   Let be a continuous map from a metric space to a metric space . If a subset is compact, then its image is also compact.    Let be a sequence of points . For each index , there is some point so that . Since is compact, the sequence has a convergent subsequence . Since is continuous, ; that is, the subsequence of converges to a point in . Since every sequence in has a subsequence which converges to a limit in , we conclude that is compact.    The above result implies that sequential compactness is a topological invariant of metric spaces, since it is preserved under continuous maps between metric spaces. Intuitively, a compact metric space can be viewed as self-contained, since it is impossible for a divergent sequence to fully escape. We will end this section with an equivalent characterization of sequential compactness which may be surprising; a metric space is (sequentially) compact if and only if it is complete and totally bounded. In order to show this, we will require the following supporting lemma:   Sequences in totally bounded subsets   If a subset of a metric space is totally bounded, then every sequence of points in has a Cauchy subsequence.    If is empty, then the desired result holds vacuously. We may therefore suppose that is non-empty. Let be a sequence of points . We aim to show that this sequence has a Cauchy subsequence. To that end, we first observe that since is totally bounded in , it is bounded by , and so is finite. let be a sequence of positive real numbers so that and . We argue inductively that for each natural number , there is a subsequence of whose entries are all contained in an open ball of radius , and moreover that if then is a subsequence of .  Indeed, for the base case , we observe that is contained in an open ball of radius around any of its points, so we may take for all indices .  For the inductive step, suppose for the inductive hypothesis that for some natural number the sequence has a subsequence whose entries are all contained in an open ball of radius . Since is totally bounded, it is contained in a union of finitely many open balls of radius . The pigeonhole principle implies that infinitely many entries of lie in one of these open balls. Removing all other entries, we obtain a further subsequence whose entries all lie in a ball of radius .  Consider the subsequence of defined by the formula . We will show that this subsequence is Cauchy. Indeed, let be a positive real number. Since converges to , there is a natural number so that for all indices . Indeed, since is a subsequence of , its entries all lie in an open ball of radius . In particular, the entries of have distance strictly less than from each other. Thus is Cauchy.     Heine-Borel Theorem  Eduard Heine; Émile Borel   A subset of a metric space is compact if and only if it is complete and totally bounded.    Let be a subset of a metric space , and suppose first that is compact. If is a Cauchy sequence of points , then compactness implies that has a subsequence which converges to a limit in . implies that converges to this limit. Since every Cauchy sequence in converges to a limit in , is complete.  Now suppose for a contradiction that is not totally bounded in . Then there exists a positive real number so that is not contained in any finite union of open balls of radius . In particular, there is a sequence of points so that for all indices . Since is compact, this sequence has a subsequence which converges to some point . In particular, there is a natural number so that for all indices . We observe that , so that . This contradiction implies that is totally bounded in .  Conversely, now suppose that is complete and totally bounded in . Then implies that every sequence of points in has a Cauchy subsequence, and completeness implies that such a subsequence converges to a limit in . Thus every sequence of points in has a subsequence which converges to a limit in ; that is, is compact.    We may use equivalent characterizations of completeness and total boundedness in Euclidean space to reduce the above theorem into a result which makes it easier to determine if a subset of Euclidean space is compact. Since Euclidean space is complete, a subset is complete if and only if it is closed. Moreover, as we have previously shown, a subset of Euclidean space is totally bounded if and only if it is bounded. Therefore, a subset of Euclidean space is sequentially compact if and only if it is closed and bounded. This is the content of the following corollary to the , which is sometimes also called the Heine-Borel theorem.   Compact subsets of Euclidean space   A subset of Euclidean space is compact if and only if it is closed and bounded.    The implies that a subset of Euclidean space is compact if and only if it is complete and totally bounded. Since Euclidean space is complete by , implies that a subset of Euclidean space is complete if and only if it is closed. Moreover, implies that a subset of Euclidean space is totally bounded if and only if it is bounded.      allows our intuition about the geometry and point-set topology of Euclidean spaces to inform our understanding of compactness in arbitrary metric spaces.     Completeness and compactness are important geometric and topological properties for the study of metric geometry. While completeness is not easily generalizable to the broader spaces which we will study in , compactness remains a central idea in the study of topological spaces. We will see that in the absence of a metric, we will rely on an equivalent characterization of compactness derived from a similar idea to total boundedness.    Exercises   The following exercises are designed to help you practice basic techniques and verify\/solidify your retention of the core ideas presented in each section. Answering these questions will typically involve applying known ideas in familiar ways, recalling information presented in this section, and building fluency with the basic tools of the course. You should approach these exercises as opportunities to make sure you can recognize, recall, and use the fundamental concepts accurately and efficiently before you are asked to answer questions which require complex problem-solving strategies.    Cauchy sequences   Determine whether or not the given sequence of points in a metric space is Cauchy.     Consider the sequence of real numbers defined by the formula . Determine whether or not the sequence is a Cauchy sequence in the real line .    The sequence is a Cauchy sequence in the real line .    Let be a positive real number. The Archimedean principle implies that for some natural number . We observe that for all indices , and so the sequence is Cauchy.      Consider the sequence of real numbers defined by the formula . Determine whether or not the sequence is a Cauchy sequence in the discrete metric space .    The sequence is not a Cauchy sequence in the discrete metric space .    Since for all indices , for all such indices. In particular, there is no natural number so that for all indices . Thus the sequence is not a Cauchy sequence in the discrete metric space .      Consider the sequence of bounded real-valued functions defined by the formula . Determine whether or not the sequence is a Cauchy sequence in the metric space .    The sequence is a Cauchy sequence in the metric space .       Complete subspaces of Euclidean space   Determine whether the given subset of Euclidean space is complete or incomplete.     Let be the subset of the Euclidean plane defined by . Determine whether is complete or incomplete in .     is incomplete in .    For any vector , the sequence defined by the formula is a Cauchy sequence which does not converge to a limit in , and so is not complete.      Let be the subset of -dimensional Euclidean space defined by . Determine whether is complete or incomplete in .     is complete in .     is the pre-image of the closed set of under the continuous map , and so is closed. Since is a closed subset of a complete metric space, it is complete.      Determine whether the irrational numbers is complete or incomplete in the real line .    The irrational numbers are incomplete in the real line .    For any irrational number , the sequence defined by the formula is a Cauchy sequence of irrational numbers which does not converge to a limit in , and so the irrational numbers are incomplete in the real line .      Compact subspaces of Euclidean space   Determine whether or not the given subset of Euclidean space is compact.     Let be the subset of the Euclidean plane defined by . Determine whether or not is compact in .     is not compact.    As shown in , is not complete in . Thus the implies that is not compact.      Let be the subset of -dimensional Euclidean space defined by . Determine whether or not is compact.     is compact.    As shown in , is complete in . Moreover, is bounded. Thus implies that is compact.      Determine whether or not the irrational numbers are compact.    The irrational numbers are not compact.    As shown in , the irrational numbers are not complete in the real line . Thus the implies that is not compact.      Let be real numbers, and suppose that . Determine whether or not the closed interval is compact.    The closed interval is compact.    Since the interval is closed in the real line , implies that is complete in . Moreover, is bounded. Thus implies that is compact.      Short answers and\/or in-depth solutions to these exercises are available in . However, these are provided in order to check your work. If you are unable to answer the above questions, revisit the content of this section rather than spoiling the answers.     Problems   In contrast with the exercises, solving the following problems will require you to engage more deeply with the course material. These questions may require you to analyze a situation, connect multiple ideas, or develop a logical argument from first principles. Many problems will involve writing proofs, explaining your reasoning clearly, and justifying each step. You should expect these to be less routine and more exploratory: there may not be an obvious starting point, and persistence is part of the task .    Completeness of discrete metric spaces    Prove that any discrete metric space is complete.       Prove that any metric space with finite ground set is complete.       Consider the metric subspace of the real line with ground set . Argue that is a counterexample to the false claim that any metric space whose points are all isolated points is complete.       Incompleteness of the -adic metric   Fix a prime number , and recall that the  -adic valuation  is a rational-valued function defined by the formula , where is the unique integer so that for some integers which are not divisible by .  Moreover, recall from (b) of that the -adic valuation induces an ultrametric called the  -adic metric defined by the formula .     Consider the sequence of rational numbers defined by the formula . Show that is a Cauchy sequence in .       Consider the sequence of rational numbers defined by the formula . Show that is a Cauchy sequence in .      While the Cauchy sequence converges in the -adic metric to a rational number, the Cauchy sequence diverges and therefore is a witness to the incompleteness of .     In-depth solutions to these problems are available in . However, these are provided in order to check your work. If you are unable to solve the above problems, discuss your questions with classmates and\/or instructor rather than spoiling the answers.    "
},
{
  "id": "section-completeness-and-compactness-2-1",
  "level": "2",
  "url": "section-completeness-and-compactness.html#section-completeness-and-compactness-2-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "Cauchy sequences complete metric spaces compact metric spaces subsequence "
},
{
  "id": "subsection-cauchy-sequences-and-metric-completeness-2-1",
  "level": "2",
  "url": "section-completeness-and-compactness.html#subsection-cauchy-sequences-and-metric-completeness-2-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
},
{
  ...
  "level": "2",
  "url": "section-completeness-and-compactness.html#definition-cauchy-sequence",
  "type": "Definition",
  "number": "3.6.1",
  "title": "Cauchy sequence.",
  "body": " Cauchy sequence  Cauchy sequence   A sequence of points in a metric space is called Cauchy if for all positive real numbers , there is a natural number so that for all indices .   "
},
{
  "id": "subsection-cauchy-sequences-and-metric-completeness-4",
  "level": "2",
  "url": "section-completeness-and-compactness.html#subsection-cauchy-sequences-and-metric-completeness-4",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "Cauchy criterion "
},
{
  "id": "example-cauchy-sequence",
  "level": "2",
  "url": "section-completeness-and-compactness.html#example-cauchy-sequence",
  "type": "Example",
  "number": "3.6.2",
  "title": "Cauchy sequence.",
  "body": " Cauchy sequence   For each natural number , let denote the th digit in the decimal expansion of , so that . Consider the sequence of rational numbers defined by the formula . Let be a positive real number. The Archimedean principle implies that for some natural number . We observe that for all indices . Thus is Cauchy.   "
},
{
  "id": "proposition-convergent-sequences-are-cauchy",
  "level": "2",
  "url": "section-completeness-and-compactness.html#proposition-convergent-sequences-are-cauchy",
  "type": "Proposition",
  "number": "3.6.3",
  "title": "Convergent sequences are Cauchy.",
  "body": " Convergent sequences are Cauchy   Any convergent sequence of points in a metric space is Cauchy.    Let be a sequence of points in a metric space , and suppose that converges to a limit .  Let be a positive real number. Since , there is a natural number so that for all indices . In particular, we observe that for all indices . We conclude that the sequence is Cauchy.   "
},
{
  "id": "subsection-cauchy-sequences-and-metric-completeness-8",
  "level": "2",
  "url": "section-completeness-and-compactness.html#subsection-cauchy-sequences-and-metric-completeness-8",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "complete "
...
{
  "id": "definition-metric-completeness",
  "level": "2",
  "url": "section-completeness-and-compactness.html#definition-metric-completeness",
  "type": "Definition",
  "number": "3.6.4",
  "title": "Metric completeness.",
  "body": " Metric completeness  metric completeness  metric incompleteness  Banach space   Let be a subset of a metric space .   Completeness  The subset is called complete in if every Cauchy sequence of points in converges in to a limit in .    Incompleteness  The subset is called incomplete in if some Cauchy sequence of points in does not converge in to a limit in .   A metric space is called complete (resp. incomplete ) if is complete (resp. incomplete) in . A complete normed vector space is called a Banach space .   "
},
{
  "id": "example-metric-completeness",
  "level": "2",
  "url": "section-completeness-and-compactness.html#example-metric-completeness",
  "type": "Example",
  "number": "3.6.5",
  "title": "Metric completeness.",
  "body": " Metric completeness   The following are examples of complete and incomplete metric spaces.   Any discrete metric space or finite metric space is complete, since any Cauchy sequence in a discrete metric space is eventually constant.    The rational numbers are not complete in the real line . Indeed, we have already seen an example of a Cauchy sequence of rational numbers which converges in to an irrational number .      "
},
{
  "id": "proposition-complete-subsets-are-closed",
  "level": "2",
  "url": "section-completeness-and-compactness.html#proposition-complete-subsets-are-closed",
  "type": "Proposition",
  "number": "3.6.6",
  "title": "Complete subsets are closed.",
  "body": " Complete subsets are closed   A complete subset of a metric space is closed.    By contraposition. Let be a subset of a metric space , and suppose that is not closed in . Then by (2) of and , and so ; that is, there is some limit point which is not an element of .  Since is a limit point of in , for some sequence of points . Since converges in , it is Cauchy. However, the limit of this Cauchy sequence of points in does not lie in . Thus is not complete in .   "
},
{
  "id": "proposition-closed-subsets-of-complete-metric-spaces",
  "level": "2",
  "url": "section-completeness-and-compactness.html#proposition-closed-subsets-of-complete-metric-spaces",
  "type": "Proposition",
  "number": "3.6.7",
  "title": "Closed subsets of complete metric spaces.",
  "body": " Closed subsets of complete metric spaces   A subset of a complete metric space is complete if and only if it is closed.    Let be a subset of a complete metric space . We have already seen in that if is complete in , then it is closed in . It remains to prove the converse implication. To that end, suppose that is not complete in , so that some Cauchy sequence of points does not converge to a limit in .  However, since is a Cauchy sequence in a complete metric space , it converges to a limit . Since for all indices , we conclude that is a limit point of in which is not contained in . implies that is a point of closure of in , and so is a proper subset of its closure . (2) of now implies that is not closed in .   "
},
{
  "id": "subsection-cauchy-sequences-and-metric-completeness-17",
  "level": "2",
  "url": "section-completeness-and-compactness.html#subsection-cauchy-sequences-and-metric-completeness-17",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "subsequence "
...
{
  "id": "definition-subsequence",
  "level": "2",
  "url": "section-completeness-and-compactness.html#definition-subsequence",
  "type": "Definition",
  "number": "3.6.8",
  "title": "Subsequence.",
  "body": " Subsequence  subsequence       a subsequence of      A sequence of points in a metric space is a subsequence of a sequence of points in if for some increasing function . In this case, if and , then we write .   "
},
{
  "id": "example-subsequences",
  "level": "2",
  "url": "section-completeness-and-compactness.html#example-subsequences",
  "type": "Example",
  "number": "3.6.9",
  "title": "Subsequences.",
  "body": " Subsequences   The following are examples of subsequences:   The sequences and are subsequences of the sequence .    The sequence is a subsequence of the sequence .    The convergent sequence is a subsequence of the divergent sequence .      "
},
{
  "id": "lemma-increasing-functions",
  "level": "2",
  "url": "section-completeness-and-compactness.html#lemma-increasing-functions",
  "type": "Lemma",
  "number": "3.6.10",
  "title": "Increasing functions.",
  "body": " Increasing functions    for any increasing function .    By induction on . For the base case , we observe that is a natural number, and so .  For the inductive step, suppose for the inductive hypothesis that for some natural number . We observe that , and so , since is non-decreasing. Moreover, , and so , since is injective. In particular, by the inductive hypothesis, and so .   "
},
{
  "id": "proposition-subsequences-of-convergent-sequences",
  "level": "2",
  "url": "section-completeness-and-compactness.html#proposition-subsequences-of-convergent-sequences",
  "type": "Proposition",
  "number": "3.6.11",
  "title": "Subsequences of convergent sequences.",
  "body": " Subsequences of convergent sequences   Let be a sequence of points in a metric space . If converges in , then any subsequence also converges in , and .    Let , and consider a positive real number . There is some natural number so that for all indices .  We observe that implies that for all indices , and so for all such indices. Thus .   "
},
{
  "id": "proposition-cauchy-sequences-with-convergent-subsequences",
  "level": "2",
  "url": "section-completeness-and-compactness.html#proposition-cauchy-sequences-with-convergent-subsequences",
  "type": "Proposition",
  "number": "3.6.12",
  "title": "Cauchy sequences with convergent subsequences.",
  "body": " Cauchy sequences with convergent subsequences   If a Cauchy sequence of points in a metric space has a convergent subsequence, then it converges.    Let be a Cauchy sequence of points in a metric space , and suppose that some subsequence converges to a point . We will show that the entire sequence also converges to .  To that end, let be a positive real number. Since is Cauchy, there is a natural number so that for all indices . Since the subsequence converges to , there is a natural number so that for all indices . Let . We observe that implies that for all indices . In particular, for all indices . Thus the sequence converges to .   "
},
{
  "id": "theorem-bolzano-weierstrass-theorem",
  "level": "2",
  "url": "section-completeness-and-compactness.html#theorem-bolzano-weierstrass-theorem",
  "type": "Theorem",
  "number": "3.6.13",
  "title": "Bolzano-Weierstrass theorem.",
  "body": " Bolzano-Weierstrass theorem  Bernard Bolzano; Karl Weierstrass   Any bounded sequence of real numbers has a convergent subsequence.    Let be a sequence of real numbers which is bounded in the Euclidean metric on . For each natural number , consider the set . There are two cases: either has a maximum for every index or does not have a maximum for some index . We will show that in either case, has a monotone subsequence.  First suppose that has a maximum for each index . Consider the sequence of natural numbers defined recursively by . Since , the sequence is increasing, and so is a subsequence of . Moreover, we observe that for all indices , and so is non-increasing.  On the other hand, now suppose that has no maximum for some index . Consider the sequence of natural numbers defined recursively by . Since , the sequence is increasing, and so is a subsequence of . Moreover, we observe that for all indices , and so is increasing.  In either case, has a monotone subsequence . Since is bounded, so too is , and so implies that converges.   "
},
{
  "id": "lemma-cauchy-sequences-are-bounded",
  "level": "2",
  "url": "section-completeness-and-compactness.html#lemma-cauchy-sequences-are-bounded",
  "type": "Lemma",
  "number": "3.6.14",
  "title": "Cauchy sequences are bounded.",
  "body": " Cauchy sequences are bounded   Any Cauchy sequence of points in a metric space is bounded.    Let be a Cauchy sequence of points in a metric space . Then there is a natural number so that for all indices . Let . For all indices , if , then . On the other hand, if , then . Thus , and so the sequence is bounded.   "
},
{
  "id": "corollary-completeness-of-the-real-line",
  "level": "2",
  "url": "section-completeness-and-compactness.html#corollary-completeness-of-the-real-line",
  "type": "Corollary",
  "number": "3.6.15",
  "title": "Completeness of the real line.",
  "body": " Completeness of the real line   The real line is complete, and so is a Banach space.    Let be a Cauchy sequence of real numbers . Then is bounded by , and so it has a convergent subsequence by the . Since is a Cauchy sequence with a convergent subsequence, it is itself convergent by . Since every Cauchy sequence in converges, this metric space is complete.   "
},
{
  "id": "corollary-completeness-of-the-p-norm",
  "level": "2",
  "url": "section-completeness-and-compactness.html#corollary-completeness-of-the-p-norm",
  "type": "Corollary",
  "number": "3.6.16",
  "title": "Completeness of the <span class=\"process-math\">\\(p \\)<\/span>-norm.",
  "body": " Completeness of the -norm    is a Banach space for all natural numbers and extended real numbers .    Let be a Cauchy sequence of points in the metric space . Let be a positive real number. Then there is a natural number so that for all indices . We observe that for each index , for all indices , and so is a Cauchy sequence of real numbers.   now implies that this sequence converges to some real number . Let . and now imply that converges to .  Since every Cauchy sequence in converges, this metric space is complete. We conclude that is a Banach space.   "
},
{
  "id": "subsection-cauchy-sequences-and-metric-completeness-34-2",
  "level": "2",
  "url": "section-completeness-and-compactness.html#subsection-cauchy-sequences-and-metric-completeness-34-2",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "completion "
...
{
  "id": "subsection-metric-compactness-2-1",
  "level": "2",
  "url": "section-completeness-and-compactness.html#subsection-metric-compactness-2-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "(sequentially) compact "
},
{
  "id": "definition-sequential-compactness",
  "level": "2",
  "url": "section-completeness-and-compactness.html#definition-sequential-compactness",
  "type": "Definition",
  "number": "3.6.17",
  "title": "Sequential compactness.",
  "body": " Sequential compactness  sequential compactness   A subset of a metric space is called (sequentially) compact if every sequence of points has a subsequence which converges to a limit in , and (sequentially) non-compact otherwise. A metric space is called compact (resp. non-compact ) if is compact (resp. non-compact).   "
},
{
  "id": "example-sequential-compactness",
  "level": "2",
  "url": "section-completeness-and-compactness.html#example-sequential-compactness",
  "type": "Example",
  "number": "3.6.18",
  "title": "Sequential compactness.",
...
{
  "id": "theorem-heine-cantor-theorem",
  "level": "2",
  "url": "section-completeness-and-compactness.html#theorem-heine-cantor-theorem",
  "type": "Theorem",
  "number": "3.6.19",
  "title": "Heine-Cantor theorem.",
  "body": " Heine-Cantor theorem  Eduard Heine; Georg Cantor   A continuous map from a compact metric space to a metric space is uniformly continuous.    Let be a map from a compact metric space to a metric space , and suppose that is not uniformly continuous. Then there is a positive real number so that for all positive real numbers , for some points with .  Let be a sequence of positive real numbers so that . For each index , there are points so that but . Since is compact, the sequence has a convergent subsequence ; that is, for some point . We will argue that also converges to . Indeed, let be a positive real number. Since converges to , there is some natural number so that for all indices . Moreover, since converges to , there is a natural number so that for all indices . Let , and note that implies that whenever . In particular, this implies that for all indices . Thus also converges to .  If were continuous at , then both and would converge to . In this case, we would see that This contradiction implies that is not continuous at .  In summary, we have shown that if is not uniformly continuous, then it is not continuous. Equivalently, if is continuous, then it is uniformly continuous.   "
},
{
  "id": "proposition-continuous-images-of-compact-subsets",
  "level": "2",
  "url": "section-completeness-and-compactness.html#proposition-continuous-images-of-compact-subsets",
  "type": "Proposition",
  "number": "3.6.20",
  "title": "Continuous images of compact subsets.",
  "body": " Continuous images of compact subsets   Let be a continuous map from a metric space to a metric space . If a subset is compact, then its image is also compact.    Let be a sequence of points . For each index , there is some point so that . Since is compact, the sequence has a convergent subsequence . Since is continuous, ; that is, the subsequence of converges to a point in . Since every sequence in has a subsequence which converges to a limit in , we conclude that is compact.   "
},
{
  "id": "lemma-sequences-in-totally-bounded-subsets",
  "level": "2",
  "url": "section-completeness-and-compactness.html#lemma-sequences-in-totally-bounded-subsets",
  "type": "Lemma",
  "number": "3.6.21",
  "title": "Sequences in totally bounded subsets.",
  "body": " Sequences in totally bounded subsets   If a subset of a metric space is totally bounded, then every sequence of points in has a Cauchy subsequence.    If is empty, then the desired result holds vacuously. We may therefore suppose that is non-empty. Let be a sequence of points . We aim to show that this sequence has a Cauchy subsequence. To that end, we first observe that since is totally bounded in , it is bounded by , and so is finite. let be a sequence of positive real numbers so that and . We argue inductively that for each natural number , there is a subsequence of whose entries are all contained in an open ball of radius , and moreover that if then is a subsequence of .  Indeed, for the base case , we observe that is contained in an open ball of radius around any of its points, so we may take for all indices .  For the inductive step, suppose for the inductive hypothesis that for some natural number the sequence has a subsequence whose entries are all contained in an open ball of radius . Since is totally bounded, it is contained in a union of finitely many open balls of radius . The pigeonhole principle implies that infinitely many entries of lie in one of these open balls. Removing all other entries, we obtain a further subsequence whose entries all lie in a ball of radius .  Consider the subsequence of defined by the formula . We will show that this subsequence is Cauchy. Indeed, let be a positive real number. Since converges to , there is a natural number so that for all indices . Indeed, since is a subsequence of , its entries all lie in an open ball of radius . In particular, the entries of have distance strictly less than from each other. Thus is Cauchy.   "
},
{
  "id": "theorem-heine-borel-theorem",
  "level": "2",
  "url": "section-completeness-and-compactness.html#theorem-heine-borel-theorem",
  "type": "Theorem",
  "number": "3.6.22",
  "title": "Heine-Borel Theorem.",
  "body": " Heine-Borel Theorem  Eduard Heine; Émile Borel   A subset of a metric space is compact if and only if it is complete and totally bounded.    Let be a subset of a metric space , and suppose first that is compact. If is a Cauchy sequence of points , then compactness implies that has a subsequence which converges to a limit in . implies that converges to this limit. Since every Cauchy sequence in converges to a limit in , is complete.  Now suppose for a contradiction that is not totally bounded in . Then there exists a positive real number so that is not contained in any finite union of open balls of radius . In particular, there is a sequence of points so that for all indices . Since is compact, this sequence has a subsequence which converges to some point . In particular, there is a natural number so that for all indices . We observe that , so that . This contradiction implies that is totally bounded in .  Conversely, now suppose that is complete and totally bounded in . Then implies that every sequence of points in has a Cauchy subsequence, and completeness implies that such a subsequence converges to a limit in . Thus every sequence of points in has a subsequence which converges to a limit in ; that is, is compact.   "
},
{
  "id": "corollary-compact-subsets-of-euclidean-space",
  "level": "2",
  "url": "section-completeness-and-compactness.html#corollary-compact-subsets-of-euclidean-space",
  "type": "Corollary",
  "number": "3.6.23",
  "title": "Compact subsets of Euclidean space.",
  "body": " Compact subsets of Euclidean space   A subset of Euclidean space is compact if and only if it is closed and bounded.    The implies that a subset of Euclidean space is compact if and only if it is complete and totally bounded. Since Euclidean space is complete by , implies that a subset of Euclidean space is complete if and only if it is closed. Moreover, implies that a subset of Euclidean space is totally bounded if and only if it is bounded.   "
},
{
  "id": "exercise-cauchy-sequences-3",
  "level": "2",
  "url": "section-completeness-and-compactness.html#exercise-cauchy-sequences-3",
  "type": "Exercise",
  "number": "3.6.4.1",
  "title": "",
  "body": "  Consider the sequence of real numbers defined by the formula . Determine whether or not the sequence is a Cauchy sequence in the real line .    The sequence is a Cauchy sequence in the real line .    Let be a positive real number. The Archimedean principle implies that for some natural number . We observe that for all indices , and so the sequence is Cauchy.   "
},
{
  "id": "exercise-cauchy-sequences-4",
  "level": "2",
  "url": "section-completeness-and-compactness.html#exercise-cauchy-sequences-4",
  "type": "Exercise",
  "number": "3.6.4.2",
  "title": "",
  "body": "  Consider the sequence of real numbers defined by the formula . Determine whether or not the sequence is a Cauchy sequence in the discrete metric space .    The sequence is not a Cauchy sequence in the discrete metric space .    Since for all indices , for all such indices. In particular, there is no natural number so that for all indices . Thus the sequence is not a Cauchy sequence in the discrete metric space .   "
},
{
  "id": "exercise-cauchy-sequences-5",
  "level": "2",
  "url": "section-completeness-and-compactness.html#exercise-cauchy-sequences-5",
  "type": "Exercise",
  "number": "3.6.4.3",
  "title": "",
  "body": "  Consider the sequence of bounded real-valued functions defined by the formula . Determine whether or not the sequence is a Cauchy sequence in the metric space .    The sequence is a Cauchy sequence in the metric space .    "
},
{
  "id": "exercise-complete-subspaces-of-euclidean-space-3",
  "level": "2",
  "url": "section-completeness-and-compactness.html#exercise-complete-subspaces-of-euclidean-space-3",
  "type": "Exercise",
  "number": "3.6.4.4",
  "title": "",
  "body": "  Let be the subset of the Euclidean plane defined by . Determine whether is complete or incomplete in .     is incomplete in .    For any vector , the sequence defined by the formula is a Cauchy sequence which does not converge to a limit in , and so is not complete.   "
},
{
  "id": "exercise-complete-subspaces-of-euclidean-space-4",
  "level": "2",
  "url": "section-completeness-and-compactness.html#exercise-complete-subspaces-of-euclidean-space-4",
  "type": "Exercise",
  "number": "3.6.4.5",
  "title": "",
  "body": "  Let be the subset of -dimensional Euclidean space defined by . Determine whether is complete or incomplete in .     is complete in .     is the pre-image of the closed set of under the continuous map , and so is closed. Since is a closed subset of a complete metric space, it is complete.   "
},
{
  "id": "exercise-complete-subspaces-of-euclidean-space-5",
  "level": "2",
  "url": "section-completeness-and-compactness.html#exercise-complete-subspaces-of-euclidean-space-5",
  "type": "Exercise",
  "number": "3.6.4.6",
  "title": "",
  "body": "  Determine whether the irrational numbers is complete or incomplete in the real line .    The irrational numbers are incomplete in the real line .    For any irrational number , the sequence defined by the formula is a Cauchy sequence of irrational numbers which does not converge to a limit in , and so the irrational numbers are incomplete in the real line .   "
},
{
  "id": "exercise-compact-subspaces-of-euclidean-space-3",
  "level": "2",
  "url": "section-completeness-and-compactness.html#exercise-compact-subspaces-of-euclidean-space-3",
  "type": "Exercise",
  "number": "3.6.4.7",
  "title": "",
  "body": "  Let be the subset of the Euclidean plane defined by . Determine whether or not is compact in .     is not compact.    As shown in , is not complete in . Thus the implies that is not compact.   "
},
{
  "id": "exercise-compact-subspaces-of-euclidean-space-4",
  "level": "2",
  "url": "section-completeness-and-compactness.html#exercise-compact-subspaces-of-euclidean-space-4",
  "type": "Exercise",
  "number": "3.6.4.8",
  "title": "",
  "body": "  Let be the subset of -dimensional Euclidean space defined by . Determine whether or not is compact.     is compact.    As shown in , is complete in . Moreover, is bounded. Thus implies that is compact.   "
},
{
  "id": "exercise-compact-subspaces-of-euclidean-space-5",
  "level": "2",
  "url": "section-completeness-and-compactness.html#exercise-compact-subspaces-of-euclidean-space-5",
  "type": "Exercise",
  "number": "3.6.4.9",
  "title": "",
  "body": "  Determine whether or not the irrational numbers are compact.    The irrational numbers are not compact.    As shown in , the irrational numbers are not complete in the real line . Thus the implies that is not compact.   "
},
{
  "id": "exercise-compact-subspaces-of-euclidean-space-6",
  "level": "2",
  "url": "section-completeness-and-compactness.html#exercise-compact-subspaces-of-euclidean-space-6",
  "type": "Exercise",
  "number": "3.6.4.10",
  "title": "",
  "body": "  Let be real numbers, and suppose that . Determine whether or not the closed interval is compact.    The closed interval is compact.    Since the interval is closed in the real line , implies that is complete in . Moreover, is bounded. Thus implies that is compact.   "
},
{
  "id": "problem-completeness-of-discrete-metric-spaces",
  "level": "2",
  "url": "section-completeness-and-compactness.html#problem-completeness-of-discrete-metric-spaces",
  "type": "Problem",
  "number": "3.6.5.1",
  "title": "Completeness of discrete metric spaces.",
  "body": " Completeness of discrete metric spaces    Prove that any discrete metric space is complete.       Prove that any metric space with finite ground set is complete.       Consider the metric subspace of the real line with ground set . Argue that is a counterexample to the false claim that any metric space whose points are all isolated points is complete.     "
},
{
  "id": "problem-incompleteness-of-the-p-adic-metric",
  "level": "2",
  "url": "section-completeness-and-compactness.html#problem-incompleteness-of-the-p-adic-metric",
  "type": "Problem",
  "number": "3.6.5.2",
  "title": "Incompleteness of the <span class=\"process-math\">\\(p \\)<\/span>-adic metric.",
  "body": " Incompleteness of the -adic metric   Fix a prime number , and recall that the  -adic valuation  is a rational-valued function defined by the formula , where is the unique integer so that for some integers which are not divisible by .  Moreover, recall from (b) of that the -adic valuation induces an ultrametric called the  -adic metric defined by the formula .     Consider the sequence of rational numbers defined by the formula . Show that is a Cauchy sequence in .       Consider the sequence of rational numbers defined by the formula . Show that is a Cauchy sequence in .      While the Cauchy sequence converges in the -adic metric to a rational number, the Cauchy sequence diverges and therefore is a witness to the incompleteness of .   "
},
{
...
},
{
  ...
  "number": "4.1.11",
  "title": "Neighborhood.",
  "body": " Neighborhood  neighborhood       the set of neighborhoods of          the set of neighborhoods of in          the set of neighborhoods of in      A subset of a topological space is called a neighborhood of a point in if for some open set in . The set of neighborhoods of in is variously denoted , , and\/or , depending on which information is clear from context.   "
},
{
  ...
  "number": "4.1.12",
  "title": "Neighborhoods.",
  "body": " Neighborhoods   Recall that the Sierpiński space is the topological space with ground set and topology . Thus is the only neighborhood of the point , but and are neighborhoods of the point ; that is,    "
},
{
...
{
  "id": "exercise-examples-of-topologies-3",
  "level": "2",
  "url": "section-introduction-to-topology.html#exercise-examples-of-topologies-3",
  "type": "Exercise",
  "number": "4.1.3.1",
  "title": "",
  "body": " cofinite  cofinite topology   A subset of a set is called cofinite in if the complement is finite. The cofinite topology  on is the collection of all cofinite subsets of along with the empty set . Verify that is a topology on .    We see that and is finite, so that is cofinite in . Thus , and so is non-degenerate.  Now let be an indexed family of subsets . Since unions with the empty set do not affect the value of the union, we may suppose that for all indices . So is finite for each index . (4) of implies that is an intersection of finite sets, and so is itself finite. Thus is cofinite in , and so is stable under unions.  Finally, let be a finite sequence of subsets . If for any index , then . So we may suppose that for all indices . In particular, is finite for each index . (4) of implies that is a finite union of finite sets, and so is itself finite. Thus is cofinite in , and so is stable under finite intersections.  Since is non-degenerate, stable under unions, and stable under finite intersections, it is a topology on .   "
},
{
  "id": "exercise-examples-of-topologies-4",
  "level": "2",
  "url": "section-introduction-to-topology.html#exercise-examples-of-topologies-4",
  "type": "Exercise",
  "number": "4.1.3.2",
  "title": "",
  "body": " cocountable  cocountable topology   A subset of a set is called cocountable in if the complement is countable. The cocountable topology  on is the collection of all cocountable subsets of along with the empty set . Verify that is a topology on .    We see that and is countable, so that is cocountable in . Thus , and so is non-degenerate.  Now let be an indexed family of subsets . Since unions with the empty set do not affect the value of the union, we may suppose that for all indices . So is countable for each index . (4) of implies that is an intersection of countable sets, and so is itself countable. Thus is cocountable in , and so is stable under unions.  Finally, let be a finite sequence of subsets . If for any index , then . So we may suppose that for all indices . In particular, is countable for each index . (4) of implies that is a finite union of countable sets, and so is itself countable. Thus is cocountable in , and so is stable under finite intersections.  Since is non-degenerate, stable under unions, and stable under finite intersections, it is a topology on .   "
},
{
  "id": "exercise-examples-of-topologies-5",
  "level": "2",
  "url": "section-introduction-to-topology.html#exercise-examples-of-topologies-5",
  "type": "Exercise",
  "number": "4.1.3.3",
  "title": "",
  "body": " particular point topology   Fix an element of a set . The particular point topology  on is the collection of all subsets of which contain along with the empty set . Verify that is a topology on .    We see that and , so that . Since , is non-degenerate.  Now let be an indexed family of subsets . If for all indices , then . Otherwise, if for some index , then for some index , and so . Thus is stable under unions.  Finally, let be a finite sequence of subsets . If for any index , then . So we may suppose that for all indices . In particular, for all indices , and so . Thus is stable under finite intersections.  Since is non-degenerate, stable under unions, and stable under finite intersections, it is a topology on .   "
},
{
  "id": "exercise-examples-of-topologies-6",
  "level": "2",
  "url": "section-introduction-to-topology.html#exercise-examples-of-topologies-6",
  "type": "Exercise",
  "number": "4.1.3.4",
  "title": "",
  "body": "  Fix an element of a set . The excluded point topology  on is the collection of all subsets of which do not contain along with itself. Verify that is a topology on .    We see that and , so that . Since , is non-degenerate.  Now let be an indexed family of subsets . If for some index , then . Otherwise, if for all indices , then for all indices , and so . Thus is stable under unions.  Finally, let be a finite sequence of subsets . If for all indices , then . So we may suppose that for all indices . In particular, for some index , and so . Thus is stable under finite intersections.  Since is non-degenerate, stable under unions, and stable under finite intersections, it is a topology on .   "
},
{
  "id": "exercise-computing-subspace-topologies-3",
  "level": "2",
  "url": "section-introduction-to-topology.html#exercise-computing-subspace-topologies-3",
  "type": "Exercise",
  "number": "4.1.3.5",
  "title": "",
  "body": "  Let be a subset of a set . Verify that if is equipped with the cofinite topology, then the subspace topology induced on is the cofinite topology on .    Denote by and the cofinite topologies on and ; we aim to show that .  To that end, let be a subset of , and suppose first that . If , then . On the other hand, if , then is finite. We observe that is cofinite in , and so .  Conversely, now suppose that . Then for some . If , then . On the other hand, if , then is finite, and so (a) of implies that is finite, so that .  In summary, we have shown that if and only if . We conclude that .   "
},
{
  "id": "exercise-computing-subspace-topologies-4",
  "level": "2",
  "url": "section-introduction-to-topology.html#exercise-computing-subspace-topologies-4",
  "type": "Exercise",
  "number": "4.1.3.6",
  "title": "",
  "body": "  Let be a subset of a set . Verify that if is equipped with the cocountable topology, then the subspace topology induced on is the cocountable topology on .    Denote by and the cocountable topologies on and ; we aim to show that .  To that end, let be a subset of , and suppose first that . If , then . On the other hand, if , then is countable. We observe that is cocountable in , and so .  Conversely, now suppose that . Then for some . If , then . On the other hand, if , then is countable, and so (a) of implies that is countable, so that .  In summary, we have shown that if and only if . We conclude that .   "
},
{
  "id": "exercise-computing-subspace-topologies-5",
  "level": "2",
  "url": "section-introduction-to-topology.html#exercise-computing-subspace-topologies-5",
  "type": "Exercise",
  "number": "4.1.3.7",
  "title": "",
  "body": "  Let be a subset of a set , and fix a point . Verify that if is equipped with the particular point topology, then the subspace topology induced on is either the particular point topology if or the discrete topology if .    Denote by the particular point topology. First suppose that , and denote by the particular point topology; we aim to show that .  To that end, let be a subset of , and suppose first that . If , then . On the other hand, if , then , and so .  Now suppose that . Then for some . If , then . On the other hand, if , then , and so .  In summary, we have shown that if and only if . We conclude that .  Now suppose that . Then for any subset , (a) of implies that . We conclude that is the discrete topology on .   "
},
{
  "id": "exercise-computing-subspace-topologies-6",
  "level": "2",
  "url": "section-introduction-to-topology.html#exercise-computing-subspace-topologies-6",
  "type": "Exercise",
  "number": "4.1.3.8",
  "title": "",
  "body": "  Let be a subset of a set , and fix a point . Verify that if is equipped with the excluded point topology, then the subspace topology induced on is either the excluded point topology if or the indiscrete topology if .    Denote by the excluded point topology. First suppose that , and denote by the excluded point topology; we aim to show that .  To that end, let be a subset of , and suppose first that . If , then . On the other hand, if , then , and so .  Now suppose that . Then for some . If , then . On the other hand, if , then , and so .  In summary, we have shown that if and only if . We conclude that .  Now suppose that . Then for any subset , and so . We conclude that is the discrete topology on .   "
},
{
  "id": "problem-topologies-induced-by-p-metrics",
  "level": "2",
  "url": "section-introduction-to-topology.html#problem-topologies-induced-by-p-metrics",
  "type": "Problem",
  "number": "4.1.4.1",
  "title": "Topologies induced by <span class=\"process-math\">\\(p \\)<\/span>-metrics.",
  "body": " Topologies induced by -metrics   Show that for all , the -metrics induce the same metric topology on -dimensional real coordinate space .    You may find (b) of useful.    "
},
{
  "id": "problem-closed-sets-in-subspaces",
  "level": "2",
  "url": "section-introduction-to-topology.html#problem-closed-sets-in-subspaces",
  "type": "Problem",
  "number": "4.1.4.2",
  "title": "Closed sets in subspaces.",
  "body": " Closed sets in subspaces   Let be a subset of a topological space . Prove that a subset is closed in if and only if for some closed set in .    "
},
{
  "id": "problem-monotonicity-of-interior-closure-and-boundary",
  "level": "2",
  "url": "section-introduction-to-topology.html#problem-monotonicity-of-interior-closure-and-boundary",
  "type": "Problem",
  "number": "4.1.4.3",
  "title": "Monotonicity of interior, closure, and boundary.",
  "body": " Monotonicity of interior, closure, and boundary   Fix subsets of a topological space .     Prove that if , then and .       Give concrete examples so that but and .       Give concrete examples so that but .     "
},
{
  "id": "section-comparison-and-topology",
  "level": "1",
  "url": "section-comparison-and-topology.html",
  "type": "Section",
  "number": "4.2",
  "title": "Comparison and Topology",
  "body": " Comparison and Topology   In this section, we will explore the relationships between topology and order theory. We will explore the order-theoretic properties of the partial order on the set of topologies on a fixed set, and we will explore ways of generating topologies by specifying some (but not necessarily all) open sets. Finally we will explore some natural topologies inherited by any totally ordered set.    The Lattice of Topologies   We now investigate the structure of the collection of topologies on a given set. As a subcollection of the second power set, this collection of topologies inherits a partial order by containment . Much of this section will be devoted to exploring the order-theoretic properties of this partially ordered set. In particular, we will see that any collection of topologies has a unique supremum and infimum in this partially ordered set, and therefore that any collection of subsets of a set generates a unique topology.    Comparison of topologies  lattice of topologies  coarsening  strict coarsening  refinement  strict refinement       the lattice of topologies on      Denote by the set of topologies on a set . is partially ordered by containment, and the partially ordered set is called the lattice of topologies on .   Coarsening  For all topologies on , if , then we say that is coarser than , or equivalently that is a coarsening of .  Similarly, if , then we say that is strictly coarser than , or equivalently that is a strict coarsening of .    Refinement  For all topologies on , if , then we say that is finer than , or equivalently that is a refinement of .  Similarly, if , then we say that is strictly finer than , or equivalently that is a strict refinement of .       We will eventually see that any collection of topologies on a given set has a unique supremum and infimum in the partial order of topological comparison. More easy to see, however, is that the lattice of topologies on that set itself has a maximum and minimum; any topology is coarser than the discrete topology and finer than the indiscrete topology. This justifies our use of the top and bottom notation for the discrete topology and the indiscrete topology on a set ; in the lattice of topologies on , and are the maximum and minimum elements, respectively.   Bounds on topologies   The lattice of topologies on a set is bounded as a partially ordered set.    Consider the lattice of topologies on a set . We observe that for all topologies , and so is bounded from above and below.    As stated previously, we will show that any collection of topologies on a given set has a supremum and a infimum with respect to the partial order by containment. Since the intersection of a collection of topologies on a set is also a topology on that set, the existence of the greatest lower bound, or meet, of such a collection of topologies is easy to show.   Intersection of topologies   The intersection of a collection of topologies on a set is a topology.    Let be an indexed family of topologies on a set , and let be the intersection. Since for each index , ; that is, is non-degenerate.  Now let be an indexed family of sets . We observe that for all indices and , and so the stability of each under unions implies that for each index . Since this holds for all indices , ; that is, is stable under unions.  Finally let be a finite sequence of sets . We observe that for all indices and , and so the stability of each under finite intersection implies that for each index . Since this holds for all indices , ; that is, is stable under finite intersections.  Since is non-degenerate, stable under unions, and stable under finite intersections, it is a topology on .    Since the intersection of sets is the infimum in the partial order by containment, we will see that implies that the intersection of topologies is also the infimum in the partial order of topological comparison. That is, the intersection of a collection of topologies on a set is the finest common coarsening.  It might seem plausible that the dual notion of union might provide us with a supremum of such a collection with respect to topological comparison. Unfortunately, this is not the case; the union of topologies on a set is not, in general, also a topology on that set. However, there is a unique coarsest common refinement of such a collection. In order to show that this is the case, we need to investigate the ways in which collections of subsets of a set which may not themselves be topologies may generate a topology on that set.   Generated topology  subbase  generated topology       the topology generated by .     A collection of subsets of a set is called a subbase of a topology (and its elements are called subbasic open sets ) on if is the coarsest topology on which contains ; that is, is a subbase for if every topology on which contains is a refinement of .  Every collection of subsets of a set is a subbase of a unique topology on called the topology generated by .     Subbases   The following are examples of subbases of topologies:   The metric topology induced on a set by a metric is generated by the collection of metric open balls; that is, .    The indiscrete topology on a set is generated by the empty set ; that is, .    The discrete topology on a set is generated by the singleton sets ; that is, .    box  Let be an indexed family of topological spaces . The box topology is the topology on the Cartesian product of the ground sets generated by products of open sets.  We will see that if the index set is infinite, then the box topology described above is too fine to be of use. In , we will introduce the dual notions of initial and final topologies to give natural topologies on sets constructed from the ground sets of topological spaces, including via Cartesian products, disjoint unions, and quotients.       The concept of generated topologies will now allow us to show that the lattice of topologies on a given set is a complete lattice. That is, any collection of topologies has a unique least upper bound and a unique greatest lower bound.   Complete lattice of topologies   The lattice of topologies on a set is a complete lattice; that is, any collection of topologies on has a unique supremum and infimum.    Let be a collection of topologies on a set . Since the intersection is an infimum for in the partially ordered set , it is an infimum for in ; that is, this intersection is the finest common coarsening of the topologies in . It therefore suffices to show that has a supremum.  To that end, let be the topology on generated by the union of all topologies in . Then is a common refinement of each topology in . Moreover, if is a common refinement of each topology in , then it contains the union of all such topologies, and therefore is a refinement of . Thus is an infimum for in ; that is, this intersection is the coarsest common refinement of the topologies in .    So the lattice of topologies on any given set is in fact a complete lattice under the partial order of containment, where infima are intersections and suprema are the topologies generated by unions. The following alternative characterization of subbases of topology will be useful for specifying certain nice subbases.   Subbases of topology   A collection of subsets of a set is a subbase of a topology if and only if for every point in every open set , for some finite sequence of subbasic open sets .     You will have an opportunity to prove in the problems for this section. For the rest of this section, we will explore the properties of certain nice subbases of topologies known as bases , which can be said to capture the point-set topology of a given topological space.     Base of Topology   Text before the first subdivision.    Text after the last subdivision.     Order Topologies   Text before the first subdivision.    Text after the last subdivision.     Text after the last subsection.    Exercises   The following exercises are designed to help you practice basic techniques and verify\/solidify your retention of the core ideas presented in each section. Answering these questions will typically involve applying known ideas in familiar ways, recalling information presented in this section, and building fluency with the basic tools of the course. You should approach these exercises as opportunities to make sure you can recognize, recall, and use the fundamental concepts accurately and efficiently before you are asked to answer questions which require complex problem-solving strategies.    Comparison of topologies on    Compare the following topologies on the real numbers : the indiscrete topology ; the discrete topology ; the Euclidean topology ; the cofinite topology ; the cocountable topology ; the particular point topology specified with particular point ; and the excluded point topology specified with excluded point .    We observe that all comparison relationships can be summarized by .     Unions of topologies   Give an example of two topologies on a set whose union is not a topology on that set.    Let , and consider the topologies . Then is not a topology on , since it is not stable under unions. Indeed, , but .     From subbase to topology   Let , and consider the collection be the collection of subsets of defined by . Compute explicitly the topology on generated by .    The topology on generated by is .     Short answers and\/or in-depth solutions to these exercises are available in . However, these are provided in order to check your work. If you are unable to answer the above questions, revisit the content of this section rather than spoiling the answers.     Problems   In contrast with the exercises, solving the following problems will require you to engage more deeply with the course material. These questions may require you to analyze a situation, connect multiple ideas, or develop a logical argument from first principles. Many problems will involve writing proofs, explaining your reasoning clearly, and justifying each step. You should expect these to be less routine and more exploratory: there may not be an obvious starting point, and persistence is part of the task .    Monotonicity of generated topologies   Fix collections of subsets of a set . Show that if , then .      Topological comparison and interior\/closure   Fix two topologies on a set , and suppose that .     For a subset , compare its interior in the topological space with its interior in the topological space .       For a subset , compare its closure in the topological space with its closure in the topological space .       Subbase of the Euclidean topology   Consider the collections and of upwards- and downwards pointing rays in the real numbers , respectively; that is, . Show that is a subbase of the Euclidean topology on .    First argue that every open set in the real line is a union of bounded open intervals. What do finite intersections of subbasic open sets (elements of ) look like?      In-depth solutions to these problems are available in . However, these are provided in order to check your work. If you are unable to solve the above problems, discuss your questions with classmates and\/or instructor rather than spoiling the answers.    "
},
{
  "id": "definition-comparison-of-topologies",
  "level": "2",
  "url": "section-comparison-and-topology.html#definition-comparison-of-topologies",
  "type": "Definition",
  "number": "4.2.1",
  "title": "Comparison of topologies.",
  "body": " Comparison of topologies  lattice of topologies  coarsening  strict coarsening  refinement  strict refinement       the lattice of topologies on      Denote by the set of topologies on a set . is partially ordered by containment, and the partially ordered set is called the lattice of topologies on .   Coarsening  For all topologies on , if , then we say that is coarser than , or equivalently that is a coarsening of .  Similarly, if , then we say that is strictly coarser than , or equivalently that is a strict coarsening of .    Refinement  For all topologies on , if , then we say that is finer than , or equivalently that is a refinement of .  Similarly, if , then we say that is strictly finer than , or equivalently that is a strict refinement of .      "
},
{
  "id": "lemma-bounds-on-topologies",
  "level": "2",
  "url": "section-comparison-and-topology.html#lemma-bounds-on-topologies",
  "type": "Lemma",
  "number": "4.2.2",
  "title": "Bounds on topologies.",
  "body": " Bounds on topologies   The lattice of topologies on a set is bounded as a partially ordered set.    Consider the lattice of topologies on a set . We observe that for all topologies , and so is bounded from above and below.   "
},
{
  "id": "lemma-intersection-of-topologies",
  "level": "2",
  "url": "section-comparison-and-topology.html#lemma-intersection-of-topologies",
  "type": "Lemma",
  "number": "4.2.3",
  "title": "Intersection of topologies.",
  "body": " Intersection of topologies   The intersection of a collection of topologies on a set is a topology.    Let be an indexed family of topologies on a set , and let be the intersection. Since for each index , ; that is, is non-degenerate.  Now let be an indexed family of sets . We observe that for all indices and , and so the stability of each under unions implies that for each index . Since this holds for all indices , ; that is, is stable under unions.  Finally let be a finite sequence of sets . We observe that for all indices and , and so the stability of each under finite intersection implies that for each index . Since this holds for all indices , ; that is, is stable under finite intersections.  Since is non-degenerate, stable under unions, and stable under finite intersections, it is a topology on .   "
},
{
  "id": "definition-generated-topology",
  "level": "2",
  "url": "section-comparison-and-topology.html#definition-generated-topology",
  "type": "Definition",
  "number": "4.2.4",
  "title": "Generated topology.",
  "body": " Generated topology  subbase  generated topology       the topology generated by .     A collection of subsets of a set is called a subbase of a topology (and its elements are called subbasic open sets ) on if is the coarsest topology on which contains ; that is, is a subbase for if every topology on which contains is a refinement of .  Every collection of subsets of a set is a subbase of a unique topology on called the topology generated by .   "
},
{
  "id": "example-subbases",
  "level": "2",
  "url": "section-comparison-and-topology.html#example-subbases",
  "type": "Example",
  "number": "4.2.5",
  "title": "Subbases.",
  "body": " Subbases   The following are examples of subbases of topologies:   The metric topology induced on a set by a metric is generated by the collection of metric open balls; that is, .    The indiscrete topology on a set is generated by the empty set ; that is, .    The discrete topology on a set is generated by the singleton sets ; that is, .    box  Let be an indexed family of topological spaces . The box topology is the topology on the Cartesian product of the ground sets generated by products of open sets.  We will see that if the index set is infinite, then the box topology described above is too fine to be of use. In , we will introduce the dual notions of initial and final topologies to give natural topologies on sets constructed from the ground sets of topological spaces, including via Cartesian products, disjoint unions, and quotients.      "
},
{
  "id": "theorem-complete-lattice-of-topologies",
  "level": "2",
  "url": "section-comparison-and-topology.html#theorem-complete-lattice-of-topologies",
  "type": "Theorem",
  "number": "4.2.6",
  "title": "Complete lattice of topologies.",
  "body": " Complete lattice of topologies   The lattice of topologies on a set is a complete lattice; that is, any collection of topologies on has a unique supremum and infimum.    Let be a collection of topologies on a set . Since the intersection is an infimum for in the partially ordered set , it is an infimum for in ; that is, this intersection is the finest common coarsening of the topologies in . It therefore suffices to show that has a supremum.  To that end, let be the topology on generated by the union of all topologies in . Then is a common refinement of each topology in . Moreover, if is a common refinement of each topology in , then it contains the union of all such topologies, and therefore is a refinement of . Thus is an infimum for in ; that is, this intersection is the coarsest common refinement of the topologies in .   "
},
{
  "id": "lemma-subbases-of-topology",
  "level": "2",
  "url": "section-comparison-and-topology.html#lemma-subbases-of-topology",
  "type": "Lemma",
  "number": "4.2.7",
  "title": "Subbases of topology.",
  "body": " Subbases of topology   A collection of subsets of a set is a subbase of a topology if and only if for every point in every open set , for some finite sequence of subbasic open sets .   "
},
{
  "id": "subsection-the-lattice-of-topologies-16-1",
  "level": "2",
  "url": "section-comparison-and-topology.html#subsection-the-lattice-of-topologies-16-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "bases "
...
{
  "id": "exercise-comparison-of-topologies-on-r",
  "level": "2",
  "url": "section-comparison-and-topology.html#exercise-comparison-of-topologies-on-r",
  "type": "Exercise",
  "number": "4.2.4.1",
  "title": "Comparison of topologies on <span class=\"process-math\">\\(\\R \\)<\/span>.",
  "body": " Comparison of topologies on    Compare the following topologies on the real numbers : the indiscrete topology ; the discrete topology ; the Euclidean topology ; the cofinite topology ; the cocountable topology ; the particular point topology specified with particular point ; and the excluded point topology specified with excluded point .    We observe that all comparison relationships can be summarized by .   "
},
{
  "id": "exercise-unions-of-topologies",
  "level": "2",
  "url": "section-comparison-and-topology.html#exercise-unions-of-topologies",
  "type": "Exercise",
  "number": "4.2.4.2",
  "title": "Unions of topologies.",
  "body": " Unions of topologies   Give an example of two topologies on a set whose union is not a topology on that set.    Let , and consider the topologies . Then is not a topology on , since it is not stable under unions. Indeed, , but .   "
},
{
  "id": "exercise-from-subbase-to-topology",
  "level": "2",
  "url": "section-comparison-and-topology.html#exercise-from-subbase-to-topology",
  "type": "Exercise",
  "number": "4.2.4.3",
  "title": "From subbase to topology.",
  "body": " From subbase to topology   Let , and consider the collection be the collection of subsets of defined by . Compute explicitly the topology on generated by .    The topology on generated by is .   "
},
{
  "id": "problem-monotonicity-of-generated-topologies",
  "level": "2",
  "url": "section-comparison-and-topology.html#problem-monotonicity-of-generated-topologies",
  "type": "Problem",
  "number": "4.2.5.1",
  "title": "Monotonicity of generated topologies.",
  "body": " Monotonicity of generated topologies   Fix collections of subsets of a set . Show that if , then .    "
},
{
  "id": "problem-topological-comparison-and-interior-closure",
  "level": "2",
  "url": "section-comparison-and-topology.html#problem-topological-comparison-and-interior-closure",
  "type": "Problem",
  "number": "4.2.5.2",
  "title": "Topological comparison and interior\/closure.",
  "body": " Topological comparison and interior\/closure   Fix two topologies on a set , and suppose that .     For a subset , compare its interior in the topological space with its interior in the topological space .       For a subset , compare its closure in the topological space with its closure in the topological space .     "
},
{
  "id": "problem-subbase-of-the-euclidean-topology",
  "level": "2",
  "url": "section-comparison-and-topology.html#problem-subbase-of-the-euclidean-topology",
  "type": "Problem",
  "number": "4.2.5.3",
  "title": "Subbase of the Euclidean topology.",
  "body": " Subbase of the Euclidean topology   Consider the collections and of upwards- and downwards pointing rays in the real numbers , respectively; that is, . Show that is a subbase of the Euclidean topology on .    First argue that every open set in the real line is a union of bounded open intervals. What do finite intersections of subbasic open sets (elements of ) look like?    "
},
{
...
},
{
  ...
  "body": " The cocountable topology  cocountable topology   The cocountable topology  on a set is the set . One can verify that is a topology on for which all convergent sequences are eventually constant. We have seen in that the discrete topology has the same property. Moreover, if is uncountable, then is not the discrete topology on .  In particular, if is uncountable, then sequential convergence in the cocountable topology on is precisely the same as sequential convergence in the discrete topology on , even though these topologies are quite different.   "
},
{
...
{
  "id": "definition-limit-isolated-point",
  "level": "2",
  "url": "section-convergence-and-separation.html#definition-limit-isolated-point",
  "type": "Definition",
  "number": "4.3.10",
  "title": "Limit\/isolated points.",
  "body": " Limit\/isolated points  accumulation  limit point  accumulation point  isolation  isolated point   Consider a subset of a topological space .   Limit point  We say that  accumulates at a point if for all neighborhoods of in . In this case, is called a limit point (or accumulation point ) of in . The set of such limit points of in is called the derived set of and is denoted .    Isolated point  We say that a point is isolated from if for some neighborhood of in . If this condition holds for some point , then is called an isolated point of .      "
},
{
  "id": "proposition-net-characterization-of-limit-points",
  "level": "2",
  "url": "section-convergence-and-separation.html#proposition-net-characterization-of-limit-points",
  "type": "Proposition",
  "number": "4.3.11",
  "title": "Net characterization of limit points.",
  "body": " Net characterization of limit points   Let be a subset of a topological space . A point is a limit point of in if and only if some net of points converges to .    First suppose that is a limit point of in . Then for each neighborhood of in , there is a point . The allows us to choose an indexed family of points so that for all indices .  We observe that for each neighborhood of in , for all indices . Thus converges downwards to .  Conversely, suppose that is isolated from . Then for some neighborhood of in . Let be a net of points . We observe that for all indices , and so does not converge to .   "
},
{
...
},
{
  ...
  "body": " Topological distinguishability  topological distinguishability   space  Kolmogorov space   Let be a topological space.   Topological indistinguishability  Two points are called topologically indistinguishable if they have precisely the same neighborhoods ; that is, and are topologically indistinguishable if for any subset , is a neighborhood of in if and only if is a neighborhood of in .    Topological distinguishability  Two points are called topologically distinguishable if ; that is, and are topologically indistinguishable if there is some subset which is a neighborhood of one of and but not a neighborhood of the other.   A topological space is called   or Kolmogorov if all pairs of distinct points in are topologically distinguishable.   "
},
{
...
{
  "id": "exercise-net-convergence-3",
  "level": "2",
  "url": "section-convergence-and-separation.html#exercise-net-convergence-3",
  "type": "Exercise",
  "number": "4.3.3.1",
  "title": "",
  "body": "  Let be the collection of finite subsets of the natural numbers ; that is, . is partially ordered and directed both upwards and downwards by containment . Consider the net of real numbers defined by the formula . Verify that converges upwards to .    Let be a neighborhood of in . Then for some positive real number . The Archimedean principle implies that for some natural number . Let . We observe that for all indices , , and so . We conclude that the net converges upwards to .   "
},
{
  "id": "exercise-net-convergence-4",
  "level": "2",
  "url": "section-convergence-and-separation.html#exercise-net-convergence-4",
  "type": "Exercise",
  "number": "4.3.3.2",
  "title": "",
  "body": "  Let be a net of points in a topological space indexed by the neighborhood system . Verify that if for all indices , then converges downwards to .    Let be a neighborhood of in . Since for all indices , the net converges downwards to .   "
},
{
  "id": "exercise-separation-axioms-3",
  "level": "2",
  "url": "section-convergence-and-separation.html#exercise-separation-axioms-3",
  "type": "Exercise",
  "number": "4.3.3.3",
  "title": "",
  "body": "  Is an indiscrete topological space  ? Is it ? Is it ?    If has at most one element, then the indiscrete topological space is , , and . However, if has at least two elements, then the indiscrete topological space is neither , , nor .   "
},
{
  "id": "exercise-separation-axioms-4",
  "level": "2",
  "url": "section-convergence-and-separation.html#exercise-separation-axioms-4",
  "type": "Exercise",
  "number": "4.3.3.4",
  "title": "",
  "body": "  Is discrete topological space  ? Is it ? Is it ?    The discrete topological space is , , and .   "
},
{
  "id": "exercise-separation-axioms-5",
  "level": "2",
  "url": "section-convergence-and-separation.html#exercise-separation-axioms-5",
  "type": "Exercise",
  "number": "4.3.3.5",
  "title": "",
  "body": "  Is Euclidean space  ? Is it ? Is it ?    Euclidean space is , , and .   "
},
{
  "id": "exercise-separation-axioms-6",
  "level": "2",
  "url": "section-convergence-and-separation.html#exercise-separation-axioms-6",
  "type": "Exercise",
  "number": "4.3.3.6",
  "title": "",
  "body": "  Let be the cofinite topology on a set . Is the topological space  ? Is it ? Is it ?    The topological space is , , and if is finite. However, if is infinite, then the topological space is only and .   "
},
{
  "id": "exercise-separation-axioms-7",
  "level": "2",
  "url": "section-convergence-and-separation.html#exercise-separation-axioms-7",
  "type": "Exercise",
  "number": "4.3.3.7",
  "title": "",
  "body": "  Let be the cocountable topology on a set . Is the topological space  ? Is it ? Is it ?    The topological space is , , and if is countable. However, if is uncountable, then the topological space is only and .   "
},
{
  "id": "exercise-separation-axioms-8",
  "level": "2",
  "url": "section-convergence-and-separation.html#exercise-separation-axioms-8",
  "type": "Exercise",
  "number": "4.3.3.8",
  "title": "",
  "body": "  Let be a particular point topology on a set . Is the topological space  ? Is it ? Is it ?    The topological space is , , and if has at most one element. However, if has at least two elements, then the topological space is only .   "
},
{
  "id": "exercise-separation-axioms-9",
  "level": "2",
  "url": "section-convergence-and-separation.html#exercise-separation-axioms-9",
  "type": "Exercise",
  "number": "4.3.3.9",
  "title": "",
  "body": "  Let be an excluded point topology on a set . Is the topological space  ? Is it ? Is it ?    The topological space is , , and if has at most one element. However, if has at least two elements, then the topological space is only .   "
},
{
  "id": "problem-net-convergence-and-subspaces",
  "level": "2",
  "url": "section-convergence-and-separation.html#problem-net-convergence-and-subspaces",
  "type": "Problem",
  "number": "4.3.4.1",
  "title": "Net convergence and subspaces.",
  "body": " Net convergence and subspaces   Let be a subset of a topological space , and consider a net of points indexed by an upwards directed set . Prove that for a point , converges upwards to as a net in if and only if it converges upwards to as a net in .    What are the neighborhoods of in the subspace ?    "
},
{
  "id": "problem-points-of-closure",
  "level": "2",
  "url": "section-convergence-and-separation.html#problem-points-of-closure",
  "type": "Problem",
  "number": "4.3.4.2",
  "title": "Points of closure.",
  "body": " Points of closure   Let be a subset of a topological space .     Prove that a point is a point of closure of in if and only if a net of points in converges to .       Prove that .      We remark that the union need not be a disjoint union; that is, and its derived set might have significant overlap.   "
},
{
  "id": "problem-non-hausdorff-spaces",
  "level": "2",
  "url": "section-convergence-and-separation.html#problem-non-hausdorff-spaces",
  "type": "Problem",
  "number": "4.3.4.3",
  "title": "Non-Hausdorff spaces.",
  "body": " Non-Hausdorff spaces   Let be a topological space. For two points , let be the binary relation on the Cartesian product defined by the following condition: if and only if both and .     Show that is partially ordered and directed both upwards and downwards by .       Prove that if is not Hausdorff, then there is a convergent net of points in which does not have a unique limit.     "
},
{
  ...
  "number": "4.4",
  "title": "Continuity",
  "body": " Continuity   In this section, we extend the notion of continuity from the specific setting of maps between metric spaces to the broader setting of maps between topological spaces. As in metric geometry, a map between topological spaces is continuous at a point if the images of nearby points are nearby to the image of that point. We will see several characterizations of topological continuity which are analogous to the metric characterizations.    Map Convergence and Continuity at a Point   Other than the - characterization, all other characterizations of continuity for maps between metric spaces provided in may be generalized to maps between topological spaces. While the neighborhood characterization is identical, we have already seen that the point-set topology of general topological spaces requires us to consider nets of points rather than sequences. So the sequential characterization of continuity will become the net characterization. We begin with the convergence of maps between topological spaces.    Topological map convergence  map convergence  map divergence  limit   A map from a topological space to a topological space  converges at a point to a point if for all neighborhoods of in , there is a neighborhood of in so that for all points .  Such a map is called convergent at a point if it converges at to some point in and divergent at otherwise if it does not converge at to any point in . If such a map converges at a point to a unique point , then this unique point is called a limit , and is denoted .    As with previous generalizations of concepts introduced in , the convergence of a map between metric spaces is equivalent to its convergence as a map between the underlying metrizable topological spaces.   Map convergence for metrizable spaces   Let be a map from a metric space to a metric space . For any points and , converges at to as a map between metric spaces if and only if converges at to as a map from the metrizable space to the metrizable space .    First suppose that converges at to as a map between metric spaces, and let be a neighborhood of in . Then for some positive real number . Since converges at to , there is some positive real number so that for all points so that . Let , and note that is a neighborhood of in . Moreover, for all points , if , then , so that . We conclude that converges at to as a map between metrizable spaces.  Conversely, now suppose that converges at to as a map between metrizable spaces, and let be a positive real number. Then is a neighborhood of in , and so there is some neighborhood so that for all points .  Since is a neighborhood of , for some positive real number . In particular, for all points , if , then , and so ; that is, . We conclude that converges at to as a map between metric spaces.    Since the convergence of maps between metrizable topological spaces is precisely the convergence of maps between metric spaces, we already have extensive experience working with the convergence of such maps. For example, the convergence of maps between metrizable spaces may be characterized in terms of sequential convergence. However, only one of the implications relevant for such a characterization holds for topological spaces in general.   Sequential characterization of map convergence   Let be a map from a topological space to a topological space . If converges at a point to a point , then for all sequences of points converging to , the sequence converges to .    Let be a sequence of points converging to , and consider a neighborhood of in . Since converges at to , for some neighborhood of in . Moreover, since converges to , there is some natural number so that for all indices .  We observe that and so for all indices . Thus the sequence converges to .    One might hope to find a converse to as exists for metric spaces, but unfortunately no such result holds for topological spaces in general. However, we can obtain a full characterization of map convergence by considering the convergence of arbitrary nets instead of only sequences.   Net characterization of map convergence   Let be a map from a topological space to a topological space , and fix points and . Then the following are equivalent:   The map converges to at .    For every net of points indexed by an upwards directed set , if converges upwards to , then converges upwards to .    For every net of points indexed by a downwards directed set , if converges downwards to , then converges downwards to .       First suppose that converges at to , and let be a net of points indexed by an upwards directed set which converges upwards to . Let be a neighborhood of in . Since converges at to , for some neighborhood of in .  Moreover, since converges upwards to , there is an index so that for all indices . In particular, , so that for all indices . We conclude that converges upwards to .  Now suppose that for every net of points indexed by an upwards directed set , if converges upwards to , then converges upwards to . Let be a net of points indexed by a downwards directed set which converges downwards to .  Let and be the opposite of , as described in . Since is downwards directed, is upwards directed. Moreover, one can check that the downwards convergence of as a net indexed by implies the upwards convergence of as a net indexed by . In particular, converges upwards to as a net indexed by , which again implies that converges downwards to as a net indexed by .  Finally, we wish to prove that if for every net of points indexed by a downwards directed set , if converges downwards to , then converges downwards to , then converges to at . We will prove the contrapositive implication.  To that end, suppose that does not converge at to . Then there is a neighborhood of in so that for every neighborhood of in , there is some point so that . Recall that the set of neighborhoods of in is partially ordered and directed downwards by containment . The allows us to find an indexed family of points so that for each index , but .  Since for all indices , the net cannot converge downwards to . It therefore suffices to show that the net converges downwards to . To that end, let be a neighborhood of in . Then for all indices ; that is, the net converges downwards to .    We will use to construct an analogue for the sequential characterization of continuity at a point.   Characterizations of topological continuity   Let be a map from a topological space to a topological space . For a point , the following are equivalent:   Convergence characterization of continuity  The map converges at to .    Neighborhood characterization of continuity  The pre-image of any neighborhood of in is a neighborhood of in .    Net characterization of continuity  For every net of points indexed by an upwards directed set , if converges upwards to , then converges upwards to .       First suppose that converges at to , and let be a neighborhood of in . Then for some neighborhood of in . We observe that as well, so that . Since is a neighborhood of in , for some open set in . Therefore, , and so the pre-image is a neighborhood of in .  Now suppose that the pre-image of every neighborhood of in is a neighborhood of in , and let be a net of points indexed by an upwards directed set which converges upwards to . Let be a neighborhood of in .  Since is a neighborhood of in , there is an index so that for all indices . In particular, for all indices , and so the net converges upwards to .  Finally, now suppose that for every net of points indexed by an upwards directed set , if converges upwards to , then converges upwards to . Then implies that converges at to .     Continuity  continuity  discontinuity       the space of continuous maps from to          the space of continuous real-valued functions on      Let be a map from a topological space to a topological space .   Continuity   is continuous at a point if it satisfies any (and therefore all) of the conditions in . is continuous if it is continuous at every point .    Discontinuity   is discontinuous at a point if it does not satisfies any of the conditions in . is discontinuous if it is discontinuous at some point .   The set of continuous maps from to is denoted . In the special case , the set of continuous, real-valued functions on a topological space is denoted .     Continuity at a point  particular point topology   Let be the real-valued function defined piecewise by the formula . While is not continuous at when the domain and codomain are equipped with the Euclidean topology , we can construct a topology on so that is a continuous map from to .  Specifically, let . One can check that is a topology on called the particular point topology . We see that if is a neighborhood of in , then . In either case, is an open neighborhood of in the particular point topology, and so is continuous at as a map from to .    Many of the standard properties of continuous maps between metric spaces also apply to continuous maps between topological spaces. For example, continuity is preserved by composition.   Composition of continuous maps   Let and be maps between topological spaces , , and . If for some point , is continuous at and is continuous at , then the composition is continuous at .    Let be a neighborhood of in . Since is continuous at , the pre-image is a neighborhood of in . Moreover, since is continuous at , the pre-image . Thus the neighborhood characterization of continuity in implies that is continuous at .     Composition of continuous maps   The composition of continuous maps between topological spaces is continuous.         As previously mentioned, we can recover the sequential characterization of map convergence (and therefore the sequential characterization of continuity at a point) by imposing additional hypotheses on the spaces in question. For example, to establish a sequential characterization of convergence of maps from a topological space to a topological space , it is sufficient to require that be first-countable.     Continuous Maps between Topological Spaces   While we have seen characterizations of continuity at a point, we will be more interested in maps which are continuous everywhere. For the remainder of this section, we will explore the properties of such maps, and define maximal and minimal topologies with respect to continuity. To begin, we observe that holds for maps between topological spaces.    Open set characterization of continuity   A map from a topological space to a topological space is continuous if and only if the pre-image of every open set in is open in .    Suppose first that is continuous, and let be an open subset of . Let . Since is open, it is a neighborhood of , and so the neighborhood characterization of continuity in implies that is a neighborhood of . Since this is true of every point , is open in .  Conversely, now suppose that the pre-image of every open set in is an open set in , and consider a point and a neighborhood of in . Then for some open set , and so is open in . We observe that , and so is a neighborhood of . The neighborhood characterization of continuity in now implies that is continuous at . Since this holds for all points , is continuous.     Continuity   The following are examples of continuous and discontinuous maps between topological spaces:   Any map from a discrete topological space to a topological space is continuous.    Any map from a topological space to an indiscrete topological space is continuous.    Any constant map from a topological space to a topological space is continuous.    Any non-constant map from an indiscrete topological space to a topological space with at least two points is discontinuous.       We will use the observations of together with as a platform for building topologies in such a way to guarantee the continuity of certain maps. First, however, we will require the following corollary:   Continuity, coarsening, and refinement   Let be a continuous map from a topological space to a topological space . Then for any refinement of and any coarsening of , is continuous as a map from to .    Let be an open set in . Since , , and so the continuity of as a map from to implies that is an open set in . Since , .  In summary, we have shown that for any open set , the pre-image is open. Thus is a continuous map from to .    To summarize , we note that it is easy for a map to be continuous if the topology on its domain is very fine and the topology on its codomain is very coarse. For the same reason, it is very hard for a map to be continuous if the topology on its domain is very coarse and the topology on its codomain is very fine. We exploit this tension to define topologies which are just coarse enough or just fine enough to make certain maps continuous.   Final topology  final topology   The final topology on a set induced by an indexed family of maps from topological spaces to a set is the finest topology on so that each map is continuous. Concretely, the final topology induced on by is defined by .     Quotient topology  quotient topology   Let be the quotient map associated to an equivalence relation on a topological space . Concretely, sends a point to its equivalence class . The quotient topology on is the final topology induced by ; that is, the quotient topology is the finest topology on so that is continuous. When is equipped with the quotient topology, the resulting topological space is called a quotient space .    The final topology on a set can also be characterized in terms of the continuity of outgoing maps.   Continuous maps from final spaces   Let be the final topology on a set induced by an indexed family of maps from topological spaces to . Then a map from to a topological space is continuous if and only if the composition is continuous for all indices .    If is continuous, then is continuous for each index by . Thus it suffices to prove the converse implication. To that end, suppose that is continuous for each index , and let be an open set in . We wish to show that is open in .  To that end, we note that is open in for each index , and so is open in . We conclude by the that is continuous.    Dual to the notion of a final topology is that of an initial topology. While final topologies are as fine as possible while requiring the continuity of some incoming maps, initial topologies are as coarse as possible while requiring the continuity of some outgoing maps.   Initial topology  initial topology   The initial topology on a set induced by an indexed family of maps from a set to topological spaces is the coarsest topology on so that each map is continuous. Concretely, the initial topology induced on by is .     Initial topologies   The following are examples of initial topologies:   The subspace topology on a subset of a topological space is the initial topology induced by the inclusion map .    product topology  The product topology on a Cartesian product of topological spaces is the initial topology induced by the indexed family of projection maps .       The initial topology on a set can also be characterized in terms of the continuity of incoming maps. The proof of this characterization is slightly harder than the analogous result for final topologies, since in this case we have a subbase for the initial topology rather than an explicit description.   Continuous maps to initial spaces   Let be the initial topology on a set induced by an indexed family of maps from to topological spaces . Then a map from a topological space to is continuous if and only if the composition is continuous for all indices .    If is continuous, then is continuous for each index by . Thus it suffices to prove the converse implication. To that end, suppose that is continuous for each index , and let be an open set in . Then implies that for some indices and open sets . Since topologies are stable under finite intersections and arbitrary unions, is open in , and so is continuous.     Typically, the above examples of final and initial topologies are implicitly assumed, so that unless otherwise specified, quotient maps, subset inclusions, and projection maps of Cartesian products are continuous.     In the next section, we will explore how compactness generalizes to topological spaces. In particular, we will use compactness to construct a subbase for a natural topology on the set of continuous maps from a topological space to a topological space .    Exercises   The following exercises are designed to help you practice basic techniques and verify\/solidify your retention of the core ideas presented in each section. Answering these questions will typically involve applying known ideas in familiar ways, recalling information presented in this section, and building fluency with the basic tools of the course. You should approach these exercises as opportunities to make sure you can recognize, recall, and use the fundamental concepts accurately and efficiently before you are asked to answer questions which require complex problem-solving strategies.     Short answers and\/or in-depth solutions to these exercises are available in . However, these are provided in order to check your work. If you are unable to answer the above questions, revisit the content of this section rather than spoiling the answers.     Problems   In contrast with the exercises, solving the following problems will require you to engage more deeply with the course material. These questions may require you to analyze a situation, connect multiple ideas, or develop a logical argument from first principles. Many problems will involve writing proofs, explaining your reasoning clearly, and justifying each step. You should expect these to be less routine and more exploratory: there may not be an obvious starting point, and persistence is part of the task .     In-depth solutions to these problems are available in . However, these are provided in order to check your work. If you are unable to solve the above problems, discuss your questions with classmates and\/or instructor rather than spoiling the answers.    "
},
{
  "id": "section-continuity-2-1",
  "level": "2",
  "url": "section-continuity.html#section-continuity-2-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "continuous "
...
{
  "id": "definition-topological-map-convergence",
  "level": "2",
  "url": "section-continuity.html#definition-topological-map-convergence",
  "type": "Definition",
  "number": "4.4.1",
  "title": "Topological map convergence.",
  "body": " Topological map convergence  map convergence  map divergence  limit   A map from a topological space to a topological space  converges at a point to a point if for all neighborhoods of in , there is a neighborhood of in so that for all points .  Such a map is called convergent at a point if it converges at to some point in and divergent at otherwise if it does not converge at to any point in . If such a map converges at a point to a unique point , then this unique point is called a limit , and is denoted .   "
},
{
  "id": "lemma-map-convergence-for-metrizable-spaces",
  "level": "2",
  "url": "section-continuity.html#lemma-map-convergence-for-metrizable-spaces",
  "type": "Lemma",
  "number": "4.4.2",
  "title": "Map convergence for metrizable spaces.",
  "body": " Map convergence for metrizable spaces   Let be a map from a metric space to a metric space . For any points and , converges at to as a map between metric spaces if and only if converges at to as a map from the metrizable space to the metrizable space .    First suppose that converges at to as a map between metric spaces, and let be a neighborhood of in . Then for some positive real number . Since converges at to , there is some positive real number so that for all points so that . Let , and note that is a neighborhood of in . Moreover, for all points , if , then , so that . We conclude that converges at to as a map between metrizable spaces.  Conversely, now suppose that converges at to as a map between metrizable spaces, and let be a positive real number. Then is a neighborhood of in , and so there is some neighborhood so that for all points .  Since is a neighborhood of , for some positive real number . In particular, for all points , if , then , and so ; that is, . We conclude that converges at to as a map between metric spaces.   "
},
{
  "id": "proposition-sequential-characterization-of-map-convergence",
  "level": "2",
  "url": "section-continuity.html#proposition-sequential-characterization-of-map-convergence",
  "type": "Proposition",
  "number": "4.4.3",
  "title": "Sequential “characterization” of map convergence.",
  "body": " Sequential characterization of map convergence   Let be a map from a topological space to a topological space . If converges at a point to a point , then for all sequences of points converging to , the sequence converges to .    Let be a sequence of points converging to , and consider a neighborhood of in . Since converges at to , for some neighborhood of in . Moreover, since converges to , there is some natural number so that for all indices .  We observe that and so for all indices . Thus the sequence converges to .   "
},
{
  "id": "theorem-net-characterization-of-map-convergence",
  "level": "2",
  "url": "section-continuity.html#theorem-net-characterization-of-map-convergence",
  "type": "Theorem",
  "number": "4.4.4",
  "title": "Net characterization of map convergence.",
  "body": " Net characterization of map convergence   Let be a map from a topological space to a topological space , and fix points and . Then the following are equivalent:   The map converges to at .    For every net of points indexed by an upwards directed set , if converges upwards to , then converges upwards to .    For every net of points indexed by a downwards directed set , if converges downwards to , then converges downwards to .       First suppose that converges at to , and let be a net of points indexed by an upwards directed set which converges upwards to . Let be a neighborhood of in . Since converges at to , for some neighborhood of in .  Moreover, since converges upwards to , there is an index so that for all indices . In particular, , so that for all indices . We conclude that converges upwards to .  Now suppose that for every net of points indexed by an upwards directed set , if converges upwards to , then converges upwards to . Let be a net of points indexed by a downwards directed set which converges downwards to .  Let and be the opposite of , as described in . Since is downwards directed, is upwards directed. Moreover, one can check that the downwards convergence of as a net indexed by implies the upwards convergence of as a net indexed by . In particular, converges upwards to as a net indexed by , which again implies that converges downwards to as a net indexed by .  Finally, we wish to prove that if for every net of points indexed by a downwards directed set , if converges downwards to , then converges downwards to , then converges to at . We will prove the contrapositive implication.  To that end, suppose that does not converge at to . Then there is a neighborhood of in so that for every neighborhood of in , there is some point so that . Recall that the set of neighborhoods of in is partially ordered and directed downwards by containment . The allows us to find an indexed family of points so that for each index , but .  Since for all indices , the net cannot converge downwards to . It therefore suffices to show that the net converges downwards to . To that end, let be a neighborhood of in . Then for all indices ; that is, the net converges downwards to .   "
},
{
  "id": "theorem-characterizations-of-topological-continuity",
  "level": "2",
  "url": "section-continuity.html#theorem-characterizations-of-topological-continuity",
  "type": "Theorem",
  "number": "4.4.5",
  "title": "Characterizations of topological continuity.",
  "body": " Characterizations of topological continuity   Let be a map from a topological space to a topological space . For a point , the following are equivalent:   Convergence characterization of continuity  The map converges at to .    Neighborhood characterization of continuity  The pre-image of any neighborhood of in is a neighborhood of in .    Net characterization of continuity  For every net of points indexed by an upwards directed set , if converges upwards to , then converges upwards to .       First suppose that converges at to , and let be a neighborhood of in . Then for some neighborhood of in . We observe that as well, so that . Since is a neighborhood of in , for some open set in . Therefore, , and so the pre-image is a neighborhood of in .  Now suppose that the pre-image of every neighborhood of in is a neighborhood of in , and let be a net of points indexed by an upwards directed set which converges upwards to . Let be a neighborhood of in .  Since is a neighborhood of in , there is an index so that for all indices . In particular, for all indices , and so the net converges upwards to .  Finally, now suppose that for every net of points indexed by an upwards directed set , if converges upwards to , then converges upwards to . Then implies that converges at to .   "
},
{
  "id": "definition-topological-continuity",
  "level": "2",
  "url": "section-continuity.html#definition-topological-continuity",
  "type": "Definition",
  "number": "4.4.6",
  "title": "Continuity.",
  "body": " Continuity  continuity  discontinuity       the space of continuous maps from to          the space of continuous real-valued functions on      Let be a map from a topological space to a topological space .   Continuity   is continuous at a point if it satisfies any (and therefore all) of the conditions in . is continuous if it is continuous at every point .    Discontinuity   is discontinuous at a point if it does not satisfies any of the conditions in . is discontinuous if it is discontinuous at some point .   The set of continuous maps from to is denoted . In the special case , the set of continuous, real-valued functions on a topological space is denoted .   "
},
{
  "id": "example-continuity-at-a-point",
  "level": "2",
  "url": "section-continuity.html#example-continuity-at-a-point",
  "type": "Example",
  "number": "4.4.7",
  "title": "Continuity at a point.",
  "body": " Continuity at a point  particular point topology   Let be the real-valued function defined piecewise by the formula . While is not continuous at when the domain and codomain are equipped with the Euclidean topology , we can construct a topology on so that is a continuous map from to .  Specifically, let . One can check that is a topology on called the particular point topology . We see that if is a neighborhood of in , then . In either case, is an open neighborhood of in the particular point topology, and so is continuous at as a map from to .   "
},
{
  "id": "proposition-composition-of-continuous-maps-topological",
  "level": "2",
  "url": "section-continuity.html#proposition-composition-of-continuous-maps-topological",
  "type": "Proposition",
  "number": "4.4.8",
  "title": "Composition of continuous maps.",
  "body": " Composition of continuous maps   Let and be maps between topological spaces , , and . If for some point , is continuous at and is continuous at , then the composition is continuous at .    Let be a neighborhood of in . Since is continuous at , the pre-image is a neighborhood of in . Moreover, since is continuous at , the pre-image . Thus the neighborhood characterization of continuity in implies that is continuous at .   "
},
{
  "id": "corollary-composition-of-continuous-maps",
  "level": "2",
  "url": "section-continuity.html#corollary-composition-of-continuous-maps",
  "type": "Corollary",
  "number": "4.4.9",
  "title": "Composition of continuous maps.",
  "body": " Composition of continuous maps   The composition of continuous maps between topological spaces is continuous.       "
},
{
  "id": "proposition-open-set-characterization-of-continuity",
  "level": "2",
  "url": "section-continuity.html#proposition-open-set-characterization-of-continuity",
  "type": "Proposition",
  "number": "4.4.10",
  "title": "Open set characterization of continuity.",
  "body": " Open set characterization of continuity   A map from a topological space to a topological space is continuous if and only if the pre-image of every open set in is open in .    Suppose first that is continuous, and let be an open subset of . Let . Since is open, it is a neighborhood of , and so the neighborhood characterization of continuity in implies that is a neighborhood of . Since this is true of every point , is open in .  Conversely, now suppose that the pre-image of every open set in is an open set in , and consider a point and a neighborhood of in . Then for some open set , and so is open in . We observe that , and so is a neighborhood of . The neighborhood characterization of continuity in now implies that is continuous at . Since this holds for all points , is continuous.   "
},
{
  "id": "example-continuity",
  "level": "2",
  "url": "section-continuity.html#example-continuity",
  "type": "Example",
  "number": "4.4.11",
  "title": "Continuity.",
  "body": " Continuity   The following are examples of continuous and discontinuous maps between topological spaces:   Any map from a discrete topological space to a topological space is continuous.    Any map from a topological space to an indiscrete topological space is continuous.    Any constant map from a topological space to a topological space is continuous.    Any non-constant map from an indiscrete topological space to a topological space with at least two points is discontinuous.      "
...
{
  "id": "lemma-continuity-coarsening-and-refinement",
  "level": "2",
  "url": "section-continuity.html#lemma-continuity-coarsening-and-refinement",
  "type": "Corollary",
  "number": "4.4.12",
  "title": "Continuity, coarsening, and refinement.",
  "body": " Continuity, coarsening, and refinement   Let be a continuous map from a topological space to a topological space . Then for any refinement of and any coarsening of , is continuous as a map from to .    Let be an open set in . Since , , and so the continuity of as a map from to implies that is an open set in . Since , .  In summary, we have shown that for any open set , the pre-image is open. Thus is a continuous map from to .   "
},
{
  "id": "definition-final-topology",
  "level": "2",
  "url": "section-continuity.html#definition-final-topology",
  "type": "Definition",
  "number": "4.4.13",
  "title": "Final topology.",
  "body": " Final topology  final topology   The final topology on a set induced by an indexed family of maps from topological spaces to a set is the finest topology on so that each map is continuous. Concretely, the final topology induced on by is defined by .   "
},
{
  "id": "example-quotient-topology",
  "level": "2",
  "url": "section-continuity.html#example-quotient-topology",
  "type": "Example",
  "number": "4.4.14",
  "title": "Quotient topology.",
  "body": " Quotient topology  quotient topology   Let be the quotient map associated to an equivalence relation on a topological space . Concretely, sends a point to its equivalence class . The quotient topology on is the final topology induced by ; that is, the quotient topology is the finest topology on so that is continuous. When is equipped with the quotient topology, the resulting topological space is called a quotient space .   "
},
{
  "id": "lemma-continuous-maps-from-final-spaces",
  "level": "2",
  "url": "section-continuity.html#lemma-continuous-maps-from-final-spaces",
  "type": "Lemma",
  "number": "4.4.15",
  "title": "Continuous maps from final spaces.",
  "body": " Continuous maps from final spaces   Let be the final topology on a set induced by an indexed family of maps from topological spaces to . Then a map from to a topological space is continuous if and only if the composition is continuous for all indices .    If is continuous, then is continuous for each index by . Thus it suffices to prove the converse implication. To that end, suppose that is continuous for each index , and let be an open set in . We wish to show that is open in .  To that end, we note that is open in for each index , and so is open in . We conclude by the that is continuous.   "
},
{
  "id": "definition-initial-topology",
  "level": "2",
  "url": "section-continuity.html#definition-initial-topology",
  "type": "Definition",
  "number": "4.4.16",
  "title": "Initial topology.",
  "body": " Initial topology  initial topology   The initial topology on a set induced by an indexed family of maps from a set to topological spaces is the coarsest topology on so that each map is continuous. Concretely, the initial topology induced on by is .   "
},
{
  "id": "example-initial-topologies",
  "level": "2",
  "url": "section-continuity.html#example-initial-topologies",
  "type": "Example",
  "number": "4.4.17",
  "title": "Initial topologies.",
  "body": " Initial topologies   The following are examples of initial topologies:   The subspace topology on a subset of a topological space is the initial topology induced by the inclusion map .    product topology  The product topology on a Cartesian product of topological spaces is the initial topology induced by the indexed family of projection maps .      "
},
{
  "id": "lemma-continuous-maps-to-initial-spaces",
  "level": "2",
  "url": "section-continuity.html#lemma-continuous-maps-to-initial-spaces",
  "type": "Lemma",
  "number": "4.4.18",
  "title": "Continuous maps to initial spaces.",
...
},
{
  ...
  "body": " Differentiability   The fundamental object in differential calculus is the differentiable function and its derivative . In this section, we formally differentiability as a property of real-valued functions defined on subsets of the real line . We will develop techniques of differentiation, compile a list of differentiable functions, and compare the regularities of various functions.    Introduction to Differentiation   A real-valued function defined on a subset of the real line is called differentiable at a point if it can be well-approximated by a linear function in a neighborhood of that point. In this case, the derivative of the function gives the slope of this local linear approximation.    Differentiability  differentiability  derivative       the derivative of at          the derivative of      A real-valued function on a subset is called differentiable at a point if there is a real number so that for all positive real numbers , there is a positive real number so that for all points so that ; that is, is differentiable at if is a limit point of in and . In this case, the above limit is called the derivative of at and is denoted .  The differentiability locus  of is the subset of comprised by the points at which is differentiable. The function is called differentiable if . The derivative of is the real-valued function .     Differentiability   The following are examples of differentiable functions:   Let be a constant function defined by the formula for some real number , and consider a point . Since for any , for all points , is differentiable at , with derivative . Since was chosen arbitrarily, has differentiability locus , so that is differentiable.    Let be a linear function defined by the formula for some real numbers , and consider a point . Since for any positive real number , for all points , is differentiable at , with derivative . Since was chosen arbitrarily, has differentiability locus , so that is differentiable.    Let be the quadratic function defined by the formula , and consider a point . For any positive real number , let . We observe that for all points so that . We conclude that is differentiable at , with derivative . Since was chosen arbitrarily, has differentiability locus , so that is differentiable.       Extensions of the above examples can be used to compute the derivatives of a plethora of differentiable functions. Specifically, we will shortly see how to compute the derivative of a complicated function by breaking it down into smaller differentiable pieces. First, however, we observe that differentiability at a point is a stronger condition than continuity at that point.   Differentiability implies continuity   A real-valued function on a subset is continuous at every point .    Let be a real-valued function on a subset , and suppose that is differentiable at a point . Then there is a positive real number so that for all points so that . Let be a positive real number, and let . We observe that for any point so that , and so we conclude that is continuous at .    While differentiable functions are continuous, continuous functions need not be differentiable. For example, the absolute value function is continuous, but is not differentiable at the point .  We now observe that differentiability is preserved under the pointwise addition, subtraction, and multiplication of functions.   Arithmetic of differentiable functions     Addition of differentiable functions  Let be real-valued functions on a subset . For a point , if and are differentiable at , then is differentiable at , and .    Multiplication of differentiable functions  Let be real-valued functions on a subset . For a point , if and are differentiable at , then is differentiable at , and .        Let be a positive real number. Since and are differentiable at , there are positive real numbers so that for all points so that , and for all points so that . Let , and note that for all points so that . Thus is differentiable at , with derivative .    Because utilizing the - characterization of convergence is slightly more complicated than it's worth, we compute the derivative directly. Since and are differentiable at , and . Moreover, implies that is continuous at , so that . We conclude that . Thus is differentiable at , with derivative .      In fact, modest hypotheses on the denominator of a quotient of differentiable functions implies that such a quotient is also differentiable. You will have an opportunity to prove this quotient rule in the problems for this section. First, however, we observe the following immediate corollary of :   Differentiability of polynomials   Polynomial functions are differentiable at every real number.    We can combine functions in more ways than just pointwise arithmetic. One of the fundamental such operations is composition. Here we see that the composition of two differentiable functions is also differentiable.   Chain rule   Let and be real-valued functions on subsets , and suppose that . If is differentiable at a point and is differentiable at , then is differentiable at , with derivative .    We first argue that . Indeed, let be a positive real number. Since is differentiable at , there is a positive real number so that for all points so that . Since is differentiable at , it is continuous at by , and so there is a positive real number so that for all points so that . In particular, if , then . We conclude that .  We now observe that , and so is differentiable at , with derivative .     The is one of the most important foundational results in differential calculus, and has important generalizations to the differentiability of maps between higher-dimensional Euclidean spaces.     Regularity and Smoothness   For real-valued functions which are differentiable at many points in their domain, we can view the derivative not just as a number, but rather as a real-valued function which itself may be subject to analysis. A natural question in this setting is whether we can differentiate this derivative itself. This is not always possible; the derivative of a differentiable function need not itself be differentiable. In fact, such a derivative need not even be continuous. Differentiable functions whose derivative is itself differentiable are called twice differentiable , and the derivative of the derivative of such a function is called its second derivative . Continuing in such a manner, we may speak of  -times differentiable functions and their  th derivatives .    Higher order differentiability  differentiability locus  derivative  differentiability       the -times differentiability locus of          the th derivative of      Fix a real-valued function on a subset . The higher order differentiability properties of are defined recursively as follows:    is -times differentiable, its  -times differentiability locus is , and its  th derivative is .    Suppose that has -times differentiability locus and th derivative . The -times differentiability locus is the differentiability locus . The  st derivative  of is the derivative . Then is  -times differentiable if is differentiable at every point of ; that is, is -times differentiable if .        Higher order differentiability   The following are examples of higher order derivatives of functions:   Since polynomial functions are differentiable, and their derivatives are polynomials (which are also differentiable), every polynomial function is -times differentiable for all .    Fix a natural number , and consider the power function defined by the formula . Once we have explored the calculus of power series in , we will see that is -times differentiable but not -times differentiable.       As previously mentioned, the derivative of a differentiable function need not be continuous. Requiring continuity in the derivatives of a function yield the concept of regularity . Intuitively, the regularity of a function determines how many of its derivatives are continuous.   Regularity  regularity  continuous differentiability       the set of -times continuously differentiable functions on      A real-valued function on a subset is called -times continuously differentiable if it is -times differentiable and its th derivative is continuous. The set of -times continuously differentiable functions on is denoted .     Regularity   The following are examples of functions of different regularity:   Since polynomial functions are continuous and differentiable, and their derivatives are polynomials (which are also continuous and differentiable), every polynomial function is -times continuously differentiable for all .    Fix a natural number , and consider the power function defined by the formula . Once we have explored the calculus of power series in , we will see that is -times continuously differentiable but not -times differentiable; that is .    Let be the real-valued function defined piecewise by the formula . Then is differentiable but not continuously differentiable.        Monotonicity of regularity classes   For any subset , . Moreover, the intersection is non-empty.    Let be a natural number, and suppose that ; that is, is a -times continuously differentiable real-valued function on . Since is -times differentiable, it is -times differentiable, and its th derivative is itself differentiable. implies that is continuous at every point .  On the other hand, if , then is an isolated point of , and therefore is automatically continuous at . Thus is continuous, and so is -times continuously differentiable.  We have already seen that constant functions are continuously differentiable, and their derivatives are also constant functions. Thus any constant function is -times continuously differentiable for all , and so .     Smoothness  smooth       the set of smooth functions on      A real-valued function on a subset is called smooth if it is -times continuously differentiable for all . The set of smooth functions on is denoted .     There are several interesting topologies on the space of -times differentiable functions on a subset In general, the uniform convergence of a sequence of continuously differentiable functions does not imply the uniform convergence of its derivatives. However, if the derivative converge uniformly, then the uniform limit of the functions is differentiable, and its derivative is the uniform limit of the derivatives.     Text after the last subsection.    Exercises   The following exercises are designed to help you practice basic techniques and verify\/solidify your retention of the core ideas presented in each section. Answering these questions will typically involve applying known ideas in familiar ways, recalling information presented in this section, and building fluency with the basic tools of the course. You should approach these exercises as opportunities to make sure you can recognize, recall, and use the fundamental concepts accurately and efficiently before you are asked to answer questions which require complex problem-solving strategies.    Locus of differentiability   Determine the differentiability locus of the given function.     Determine the differentiability locus of the real-valued function defined by the formula .     has differentiability locus ; that is, is differentiable.    We first show that is differentiable at . Let be a positive real number, and consider the positive real number . We observe that for all so that . Thus is differentiable at , with derivative .  At any point , is a composition of differentiable functions and , and so is also differentiable. We conclude that has differentiability locus ; that is, is differentiable.      Determine the differentiability locus of the real-valued function defined piecewise by the formula .     has differentiability locus ; that is, is differentiable.    We first show that is differentiable at . Let be a positive real number, and consider the positive real number . We observe that for all points . Similarly, for all points . Thus is differentiable at , with derivative .  At any point , is locally equal to a polynomial and therefore differentiable. We conclude that has differentiability locus ; that is, is differentiable.      Computing derivatives   Compute formulas for the derivatives of the given differentiable functions.     Compute a formula for the derivative of the differentiable function defined by the formula .     has derivative given by the formula .      Compute a formula for the derivative of the differentiable function defined by the formula .     has derivative given by the formula .      Compute a formula for the derivative of the differentiable function defined by the formula .     has derivative given by the formula .      Short answers and\/or in-depth solutions to these exercises are available in . However, these are provided in order to check your work. If you are unable to answer the above questions, revisit the content of this section rather than spoiling the answers.     Problems   In contrast with the exercises, solving the following problems will require you to engage more deeply with the course material. These questions may require you to analyze a situation, connect multiple ideas, or develop a logical argument from first principles. Many problems will involve writing proofs, explaining your reasoning clearly, and justifying each step. You should expect these to be less routine and more exploratory: there may not be an obvious starting point, and persistence is part of the task .    The quotient rule   In this problem, we complete to address the quotient of two differentiable functions.   quotient rule    Let be the function defined by the formula . Show that is differentiable, with derivative        Let be a non-zero real-valued function defined on a subset . Prove that if is differentiable at a point , then is differentiable at , with derivative .       Let be real-valued functions defined on a subset . Prove that if for all points and and are differentiable at a point , then is differentiable at , with derivative .      The result proven in (c) above is usually called the quotient rule .     The power rule  power rule   For each real number , consider the power function defined by the formula .     Prove that for each natural number , is differentiable, with derivative .    Either prove the desired result by induction on or use the binomial theorem.       Show that for any integer , is differentiable, with derivative .       Show that for any rational number that if is differentiable, then its derivative is .    Write for some integers with , and differentiate the equation .      The rules proven above also hold for irrational powers, although we will require additional machinery in order to prove this final extension. In general, this is called the power rule .     Derivatives of trigonometric functions   Text before the parts.     Compute the limits and .    You may wish to use the Pythagorean theorem , which holds for all , and\/or the bounds , which hold for all so that .       Show that the trigonometric function is differentiable, with derivative .    You may wish to use the sum of angles formula , which holds for all real numbers .       Show that the trigonometric function is differentiable, with derivative .    You may wish to use the trigonometric identities and , which holds for all .       Show that the trigonometric function is differentiable, with derivative .       In-depth solutions to these problems are available in . However, these are provided in order to check your work. If you are unable to solve the above problems, discuss your questions with classmates and\/or instructor rather than spoiling the answers.    "
},
{
  "id": "section-differentiability-2-1",
  "level": "2",
  "url": "section-differentiability.html#section-differentiability-2-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "differentiable function derivative "
},
{
  "id": "subsection-introduction-to-differentiation-2-1",
  "level": "2",
  "url": "section-differentiability.html#subsection-introduction-to-differentiation-2-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "differentiable derivative "
},
{
  "id": "definition-differentiability",
  "level": "2",
  "url": "section-differentiability.html#definition-differentiability",
  "type": "Definition",
  "number": "5.1.1",
  "title": "Differentiability.",
  "body": " Differentiability  differentiability  derivative       the derivative of at          the derivative of      A real-valued function on a subset is called differentiable at a point if there is a real number so that for all positive real numbers , there is a positive real number so that for all points so that ; that is, is differentiable at if is a limit point of in and . In this case, the above limit is called the derivative of at and is denoted .  The differentiability locus  of is the subset of comprised by the points at which is differentiable. The function is called differentiable if . The derivative of is the real-valued function .   "
},
{
  "id": "example-differentiability",
  "level": "2",
  "url": "section-differentiability.html#example-differentiability",
  "type": "Example",
  "number": "5.1.2",
  "title": "Differentiability.",
  "body": " Differentiability   The following are examples of differentiable functions:   Let be a constant function defined by the formula for some real number , and consider a point . Since for any , for all points , is differentiable at , with derivative . Since was chosen arbitrarily, has differentiability locus , so that is differentiable.    Let be a linear function defined by the formula for some real numbers , and consider a point . Since for any positive real number , for all points , is differentiable at , with derivative . Since was chosen arbitrarily, has differentiability locus , so that is differentiable.    Let be the quadratic function defined by the formula , and consider a point . For any positive real number , let . We observe that for all points so that . We conclude that is differentiable at , with derivative . Since was chosen arbitrarily, has differentiability locus , so that is differentiable.      "
},
{
  "id": "lemma-differentiability-implies-continuity",
  "level": "2",
  "url": "section-differentiability.html#lemma-differentiability-implies-continuity",
  "type": "Lemma",
  "number": "5.1.3",
  "title": "Differentiability implies continuity.",
  "body": " Differentiability implies continuity   A real-valued function on a subset is continuous at every point .    Let be a real-valued function on a subset , and suppose that is differentiable at a point . Then there is a positive real number so that for all points so that . Let be a positive real number, and let . We observe that for any point so that , and so we conclude that is continuous at .   "
},
{
  "id": "proposition-arithmetic-of-differentiable-functions",
  "level": "2",
  "url": "section-differentiability.html#proposition-arithmetic-of-differentiable-functions",
  "type": "Proposition",
  "number": "5.1.4",
  "title": "Arithmetic of differentiable functions.",
  "body": " Arithmetic of differentiable functions     Addition of differentiable functions  Let be real-valued functions on a subset . For a point , if and are differentiable at , then is differentiable at , and .    Multiplication of differentiable functions  Let be real-valued functions on a subset . For a point , if and are differentiable at , then is differentiable at , and .        Let be a positive real number. Since and are differentiable at , there are positive real numbers so that for all points so that , and for all points so that . Let , and note that for all points so that . Thus is differentiable at , with derivative .    Because utilizing the - characterization of convergence is slightly more complicated than it's worth, we compute the derivative directly. Since and are differentiable at , and . Moreover, implies that is continuous at , so that . We conclude that . Thus is differentiable at , with derivative .     "
},
{
  "id": "subsection-introduction-to-differentiation-10",
  "level": "2",
  "url": "section-differentiability.html#subsection-introduction-to-differentiation-10",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "quotient rule "
...
{
  "id": "corollary-differentiability-of-polynomials",
  "level": "2",
  "url": "section-differentiability.html#corollary-differentiability-of-polynomials",
  "type": "Corollary",
  "number": "5.1.5",
  "title": "Differentiability of polynomials.",
  "body": " Differentiability of polynomials   Polynomial functions are differentiable at every real number.   "
},
{
  "id": "theorem-chain-rule",
  "level": "2",
  "url": "section-differentiability.html#theorem-chain-rule",
  "type": "Theorem",
  "number": "5.1.6",
  "title": "Chain rule.",
  "body": " Chain rule   Let and be real-valued functions on subsets , and suppose that . If is differentiable at a point and is differentiable at , then is differentiable at , with derivative .    We first argue that . Indeed, let be a positive real number. Since is differentiable at , there is a positive real number so that for all points so that . Since is differentiable at , it is continuous at by , and so there is a positive real number so that for all points so that . In particular, if , then . We conclude that .  We now observe that , and so is differentiable at , with derivative .   "
},
{
  "id": "subsection-regularity-and-smoothness-2-1",
  "level": "2",
  "url": "section-differentiability.html#subsection-regularity-and-smoothness-2-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "twice differentiable second derivative  -times differentiable  th derivatives "
},
{
  "id": "definition-higher-order-differentiability",
  "level": "2",
  "url": "section-differentiability.html#definition-higher-order-differentiability",
  "type": "Definition",
  "number": "5.1.7",
  "title": "Higher order differentiability.",
  "body": " Higher order differentiability  differentiability locus  derivative  differentiability       the -times differentiability locus of          the th derivative of      Fix a real-valued function on a subset . The higher order differentiability properties of are defined recursively as follows:    is -times differentiable, its  -times differentiability locus is , and its  th derivative is .    Suppose that has -times differentiability locus and th derivative . The -times differentiability locus is the differentiability locus . The  st derivative  of is the derivative . Then is  -times differentiable if is differentiable at every point of ; that is, is -times differentiable if .      "
},
{
  "id": "example-higher-order-differentiability",
  "level": "2",
  "url": "section-differentiability.html#example-higher-order-differentiability",
  "type": "Example",
  "number": "5.1.8",
  "title": "Higher order differentiability.",
  "body": " Higher order differentiability   The following are examples of higher order derivatives of functions:   Since polynomial functions are differentiable, and their derivatives are polynomials (which are also differentiable), every polynomial function is -times differentiable for all .    Fix a natural number , and consider the power function defined by the formula . Once we have explored the calculus of power series in , we will see that is -times differentiable but not -times differentiable.      "
},
{
  "id": "subsection-regularity-and-smoothness-5",
  "level": "2",
  "url": "section-differentiability.html#subsection-regularity-and-smoothness-5",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "regularity "
...
{
  "id": "definition-regularity",
  "level": "2",
  "url": "section-differentiability.html#definition-regularity",
  "type": "Definition",
  "number": "5.1.9",
  "title": "Regularity.",
  "body": " Regularity  regularity  continuous differentiability       the set of -times continuously differentiable functions on      A real-valued function on a subset is called -times continuously differentiable if it is -times differentiable and its th derivative is continuous. The set of -times continuously differentiable functions on is denoted .   "
},
{
  "id": "example-regularity",
  "level": "2",
  "url": "section-differentiability.html#example-regularity",
  "type": "Example",
  "number": "5.1.10",
  "title": "Regularity.",
  "body": " Regularity   The following are examples of functions of different regularity:   Since polynomial functions are continuous and differentiable, and their derivatives are polynomials (which are also continuous and differentiable), every polynomial function is -times continuously differentiable for all .    Fix a natural number , and consider the power function defined by the formula . Once we have explored the calculus of power series in , we will see that is -times continuously differentiable but not -times differentiable; that is .    Let be the real-valued function defined piecewise by the formula . Then is differentiable but not continuously differentiable.      "
},
{
  "id": "lemma-monotonicity-of-regularity-classes",
  "level": "2",
  "url": "section-differentiability.html#lemma-monotonicity-of-regularity-classes",
  "type": "Lemma",
  "number": "5.1.11",
  "title": "Monotonicity of regularity classes.",
  "body": " Monotonicity of regularity classes   For any subset , . Moreover, the intersection is non-empty.    Let be a natural number, and suppose that ; that is, is a -times continuously differentiable real-valued function on . Since is -times differentiable, it is -times differentiable, and its th derivative is itself differentiable. implies that is continuous at every point .  On the other hand, if , then is an isolated point of , and therefore is automatically continuous at . Thus is continuous, and so is -times continuously differentiable.  We have already seen that constant functions are continuously differentiable, and their derivatives are also constant functions. Thus any constant function is -times continuously differentiable for all , and so .   "
},
{
  "id": "definition-smoothness",
  "level": "2",
  "url": "section-differentiability.html#definition-smoothness",
  "type": "Definition",
  "number": "5.1.12",
  "title": "Smoothness.",
  "body": " Smoothness  smooth       the set of smooth functions on      A real-valued function on a subset is called smooth if it is -times continuously differentiable for all . The set of smooth functions on is denoted .   "
},
{
  "id": "exercise-locus-of-differentiability-3",
  "level": "2",
  "url": "section-differentiability.html#exercise-locus-of-differentiability-3",
  "type": "Exercise",
  "number": "5.1.3.1",
  "title": "",
  "body": "  Determine the differentiability locus of the real-valued function defined by the formula .     has differentiability locus ; that is, is differentiable.    We first show that is differentiable at . Let be a positive real number, and consider the positive real number . We observe that for all so that . Thus is differentiable at , with derivative .  At any point , is a composition of differentiable functions and , and so is also differentiable. We conclude that has differentiability locus ; that is, is differentiable.   "
},
{
  "id": "exercise-locus-of-differentiability-4",
  "level": "2",
  "url": "section-differentiability.html#exercise-locus-of-differentiability-4",
  "type": "Exercise",
  "number": "5.1.3.2",
  "title": "",
  "body": "  Determine the differentiability locus of the real-valued function defined piecewise by the formula .     has differentiability locus ; that is, is differentiable.    We first show that is differentiable at . Let be a positive real number, and consider the positive real number . We observe that for all points . Similarly, for all points . Thus is differentiable at , with derivative .  At any point , is locally equal to a polynomial and therefore differentiable. We conclude that has differentiability locus ; that is, is differentiable.   "
},
{
  "id": "exercise-computing-derivatives-3",
  "level": "2",
  "url": "section-differentiability.html#exercise-computing-derivatives-3",
  "type": "Exercise",
  "number": "5.1.3.3",
  "title": "",
  "body": "  Compute a formula for the derivative of the differentiable function defined by the formula .     has derivative given by the formula .   "
},
{
  "id": "exercise-computing-derivatives-4",
  "level": "2",
  "url": "section-differentiability.html#exercise-computing-derivatives-4",
  "type": "Exercise",
  "number": "5.1.3.4",
  "title": "",
  "body": "  Compute a formula for the derivative of the differentiable function defined by the formula .     has derivative given by the formula .   "
...
{
  "id": "exercise-computing-derivatives-5",
  "level": "2",
  "url": "section-differentiability.html#exercise-computing-derivatives-5",
  "type": "Exercise",
  "number": "5.1.3.5",
  "title": "",
  "body": "  Compute a formula for the derivative of the differentiable function defined by the formula .     has derivative given by the formula .   "
...
{
  "id": "problem-the-quotient-rule",
  "level": "2",
  "url": "section-differentiability.html#problem-the-quotient-rule",
  "type": "Problem",
  "number": "5.1.4.1",
  "title": "The quotient rule.",
  "body": " The quotient rule   In this problem, we complete to address the quotient of two differentiable functions.   quotient rule    Let be the function defined by the formula . Show that is differentiable, with derivative        Let be a non-zero real-valued function defined on a subset . Prove that if is differentiable at a point , then is differentiable at , with derivative .       Let be real-valued functions defined on a subset . Prove that if for all points and and are differentiable at a point , then is differentiable at , with derivative .      The result proven in (c) above is usually called the quotient rule .   "
},
{
  "id": "problem-the-power-rule",
  "level": "2",
  "url": "section-differentiability.html#problem-the-power-rule",
  "type": "Problem",
  "number": "5.1.4.2",
  "title": "The power rule.",
  "body": " The power rule  power rule   For each real number , consider the power function defined by the formula .     Prove that for each natural number , is differentiable, with derivative .    Either prove the desired result by induction on or use the binomial theorem.       Show that for any integer , is differentiable, with derivative .       Show that for any rational number that if is differentiable, then its derivative is .    Write for some integers with , and differentiate the equation .      The rules proven above also hold for irrational powers, although we will require additional machinery in order to prove this final extension. In general, this is called the power rule .   "
},
{
  "id": "problem-derivatives-of-trigonometric-functions",
  "level": "2",
  "url": "section-differentiability.html#problem-derivatives-of-trigonometric-functions",
  "type": "Problem",
  "number": "5.1.4.3",
  "title": "Derivatives of trigonometric functions.",
  "body": " Derivatives of trigonometric functions   Text before the parts.     Compute the limits and .    You may wish to use the Pythagorean theorem , which holds for all , and\/or the bounds , which hold for all so that .       Show that the trigonometric function is differentiable, with derivative .    You may wish to use the sum of angles formula , which holds for all real numbers .       Show that the trigonometric function is differentiable, with derivative .    You may wish to use the trigonometric identities and , which holds for all .       Show that the trigonometric function is differentiable, with derivative .     "
},
{
  "id": "section-analysis-of-critical-points",
  "level": "1",
  "url": "section-analysis-of-critical-points.html",
  "type": "Section",
  "number": "5.2",
  "title": "Analysis of Critical Points",
  "body": " Analysis of Critical Points   In this section, we analyze the local behavior of a real-valued function based on differentiability criteria and\/or the value of the derivative. We will make a distinction between critical points and regular points and begin to develop tools to analyze the local behavior near each. Finally, we will also classify certain kinds of critical points based on several derivative tests.    Local Behavior Near Critical Points   The local behavior of a differentiable real-valued function near a point can be very different depending on the sign of the derivative at that point. We will make the distinction between critical points , where the derivative vanishes (or does not exist), and regular points , where the derivative is non-zero.    Critical\/regular point\/value  critical point  stationary point  critical value  regular point  regular value   Let be a real-valued function defined on an interval .   Critical point\/value  A point is called a critical point of if either , in which case is called a stationary (critical) point of , or is not differentiable at .  A real number is called a critical value of if for some critical point .    Regular point\/value  A point is called a regular point of if it is not a critical point of ; that is, is a regular point of if is differentiable at and .  A real number is called a regular value of if it is not a critical value of ; that is, is a regular value of if the pre-image consists only of regular points of .        Critical\/regular points\/values   The following are examples of critical points, regular points, critical values, and regular values of functions:   Let be the function defined piecewise by the formula . Then every point is a critical point of . If , then is a stationary critical point of , and if , then is a point of non-differentiability. Thus , , and are critical values of , and all other real numbers are regular values.    Let be the function defined piecewise by the formula . Then is the only critical point of , and it is a point of non-differentiability. Thus is a critical value of , and all other real numbers are regular values.    Let be the function defined by the formula . Then is the only critical point of , and it is a stationary critical point. Thus is a critical value of , and all other real numbers are regular values.       One use of the critical points of a real-valued function is in finding local extrema , which occur when the restriction of a function to a neighborhood attains an extreme value on that neighborhood.   Local extrema  local maximum  local minimum  local extremum   Let be a real-valued function defined on a subset .   Local maximum   attains a local maximum at a point if there is a neighborhood of in so that for all points .    Local minimum   attains a local minimum at a point if there is a neighborhood of in so that for all points .    Local extremum is a generic term which encompasses both local maxima and local minima.    In fact, local extrema may only be attained at critical points. If a local extremum is attained at a point of non-differentiability, then this is clear. However, if the local extremum is attained at a point at which the function is differentiable, then this point is a stationary critical point. This is the content of the following lemma:   Local extrema are stationary points   Let be a real-valued function defined on a subset which attains a local extremum at a point so that every neighborhood of contains points strictly greater than and points strictly less than . If is differentiable at , then ; that is, is a stationary point of .    By contraposition. Suppose first that is positive. Then since is positive, there is some positive real number so that for all points so that . In particular, for all points so that , so that and have the same sign; that is, so long as and , then if and if . In particular, since the neighborhood contains points strictly less than and contains points strictly larger than , we conclude that does not achieve a maximum or minimum at .  Similarly, now suppose that is negative. Then since is positive, there is some positive real number so that for all points so that . In particular, for all points so that , so that and have opposite sign; that is, so long as and , then if and if . In particular, since the neighborhood contains points strictly less than and contains points strictly larger than , we conclude that does not achieve a maximum or minimum at .    A famous consequence of due to Fermat occurs when we suppose the domain of the function in question is an interval. In this case, interior points automatically satisfy the hypothesis of .   Interior extremum theorem  Pierre de Fermat   Let be a real-valued function defined on an interval which attains a local extremum at an interior point . If is differentiable at , then ; that is, is a stationary point of .    Any neighborhood of an interior point contains an open interval containing , and therefore contains points strictly greater than and points strictly less than . The desired conclusion now follows immediately from .    One use of the is to prove the following theorem of Darboux, which establishes an intermediate value property for the derivative of a differentiable function even if said function is not continuously differentiable .   Darboux's theorem  Jean-Gaston Darboux   Let be a real-valued function defined on an interval . If is differentiable, then is an interval.    Let , and suppose that . There are three cases; either , , or . If , then vacuously contains all points between and .  Suppose that , let , and consider the function defined by the formula . We observe that and . In particular, the argument in the proof of implies that does not attain a minimum value at or .  However, since is continuous on the compact interval , it attains a minimum value on . Thus attains a minimum value at some interior point , and so the implies that ; that is, , so that .  Now suppose that , let , and consider the function defined by the formula . We observe that and . In particular, the argument in the proof of implies that does not attain a maximum value at or .  However, since is continuous on the compact interval , it attains a maximum value on . Thus attains a maximum value at some interior point , and so the implies that ; that is, , so that .    We have thus far only explored the local behavior of functions near critical points, but there is also much to say of the behavior of a function near regular points. For example, if a differentiable function has only regular points, then it is strictly monotone.   Monotonicity in the absence of critical points   Let be a differentiable function defined on an interval . If does not have any critical points in , then is strictly monotone.    If takes both positive and negative values on , then implies that for some point . This contradiction implies that either takes only positive values on or takes only negative values on . We will prove the result in the case for all points ; the case for all is similar.  Let , and suppose that . Since is continuous on the compact interval , it attains a maximum value on . If attains a maximum value at an interior point , then the implies that . This contradiction implies that attains a maximum value at one of the endpoints or .  Since and any neighborhood of in contains points which are strictly larger than , the argument in the proof of implies that does not attain a maximum value at . Thus we conclude that . Since and were chosen arbitrarily, is increasing.     Stronger versions of are usually called the inverse function theorem , and establish the differentiability of the left inverse of any such function. You will have an opportunity to explore these additional conclusions in the problems for this section.     Classification of Critical Points   While the previous results of this section are quite strong, we wish to establish even stronger characterizations of real-valued functions based on local analyses near critical points. However, these analyses will require us to consider critical points which are not themselves limits of critical points.    Isolated critical point  isolated critical point   Let be a real-valued function defined on a subset . A critical point of is called isolated if there is a neighborhood so that consists only of regular points of .     Isolated critical points   The following are examples and non-examples of isolated critical points of real-valued functions:   Let be the continuous real-valued function defined piecewise by the formula . Then has infinitely many non-zero isolated critical points which accumulate at the critical point .    Let be the continuous real-valued function defined by the formula . Then has one critical point at , which is isolated.    Let be the continuous real-valued function defined by the formula . Then has three critical points at , , and , each of which is isolated.       If is an isolated critical point of a continuous real-valued function defined on an interval, then continuity and implies that the restrictions of to some intervals and are strictly monotone. This can be used to detect the presence of a local extremum.   First derivative test   Let be a continuous real-valued function defined on an interval . If is an interior point of in and an isolated critical point of , then attains a local extremum at if and only if the derivative changes sign at .    Under these assumptions, the previous discussion applies; there is some positive real number so that so that does not change sign on the intervals and .  If takes only positive values on both and , then the continuity of and implies that the restrictions of to the intervals and are increasing. We conclude that the restriction of to is increasing, so that for all sufficiently small positive real numbers . In particular, this implies that cannot attain a local extremum at .  If takes only negative values on both and , then the continuity of and implies that the restrictions of to the intervals and are decreasing. We conclude that the restriction of to is decreasing, so that for all sufficiently small positive real numbers . In particular, this implies that cannot attain a local extremum at .  If takes only positive values on and only negative values on , then the continuity of and implies that the restrictions of to the intervals and are increasing and decreasing, respectively. We conclude that for all sufficiently small positive real numbers . In particular, this implies that attains a local maximum at .  Finally, if takes only negative values on and only positive values on , then the continuity of and implies that the restrictions of to the intervals and are decreasing and increasing, respectively. We conclude that for all sufficiently small positive real numbers . In particular, this implies that attains a local minimum at .     Using the first derivative test   Let be a positive integer, and consider the real-valued function defined by the formula . Recall that is differentiable, with derivative . If , then for all points , and so has no critical points. However, if , then vanishes precisely when ; that is, is the only critical point of , and it is a stationary critical point.  We observe that if is even, then when and when . The implies that attains a local minimum at in this case.  On the other hand, if is odd, then when . The implies that does not attain a local extremum at in this case.    When a real-valued function is twice-differentiable at an isolated stationary critical point, its second derivative can also sometimes be used to detect the presence of local extrema at critical points.   Second derivative test   Let be a differentiable real-valued function defined on an interval . Suppose that is an interior point of in and an isolated stationary critical point of at which is twice-differentiable. If , then attains a local minimum at , and if , then attains a local maximum at .    Since is a stationary critical point of , . Since is a positive real number, there is a positive real number so that and for all points . If , then , and so and have the same sign; that is, takes only negative values on and takes only positive values on . The now implies that attains a local minimum at .  On the other hand, if , then , and so and have opposite signs; that is, takes only positive values on and takes only negative values on . The now implies that attains a local maximum at .     Using the second derivative test   Let , and consider the real-valued function defined by the formula . Recall that is differentiable, with derivative and second derivative . We have seen in that has a single stationary critical point at .  We observe that if , then , and so the implies that attains a local minimum at in this case. However, if , then , and so the is inconclusive.     So we see that the may not be conclusive even for functions whose isolated critical points may be analyzed by the .     In the next section, we will relate the instantaneous rate of change of a differentiable function (that is, its derivative) to its average rate of change via the mean value theorem .    Exercises   The following exercises are designed to help you practice basic techniques and verify\/solidify your retention of the core ideas presented in each section. Answering these questions will typically involve applying known ideas in familiar ways, recalling information presented in this section, and building fluency with the basic tools of the course. You should approach these exercises as opportunities to make sure you can recognize, recall, and use the fundamental concepts accurately and efficiently before you are asked to answer questions which require complex problem-solving strategies.    Examples of critical points   Give examples of differentiable functions which satisfy the given conditions.     Give an example of a differentiable function which has a single critical point which is not a local extremum.    The function has a single critical point which is not a local extremum.      Give an example of a differentiable function which attains a local minimum but no global minima.    The function attains a local minimum at but no global minima.      Give an example of a differentiable function which has no critical points.    The function has no critical points      Give an example of a differentiable function which has exactly one critical point but no global extrema.    The function has a critical point at but no global extrema.      Give an example of a differentiable function which attains infinitely many global extrema.    The function attains a global extremum at each integer multiple of .      Short answers and\/or in-depth solutions to these exercises are available in . However, these are provided in order to check your work. If you are unable to answer the above questions, revisit the content of this section rather than spoiling the answers.     Problems   In contrast with the exercises, solving the following problems will require you to engage more deeply with the course material. These questions may require you to analyze a situation, connect multiple ideas, or develop a logical argument from first principles. Many problems will involve writing proofs, explaining your reasoning clearly, and justifying each step. You should expect these to be less routine and more exploratory: there may not be an obvious starting point, and persistence is part of the task .    Misconceptions about critical points   Let be a real-valued function defined on an interval . The following statements are false! Explain why the statement is incorrect, and disprove them by providing a counterexample.     If attains a local extremum at a point , then is a critical point of .       If a point is a critical point of , then attains a local extremum at .       If a point is a regular point of , then has a neighborhood in on which is monotone.        is strictly monotone if and only if has no critical points.       In-depth solutions to these problems are available in . However, these are provided in order to check your work. If you are unable to solve the above problems, discuss your questions with classmates and\/or instructor rather than spoiling the answers.    "
},
{
  "id": "section-analysis-of-critical-points-2-1",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#section-analysis-of-critical-points-2-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "critical points regular points "
},
{
  "id": "subsection-local-behavior-near-critical-points-2-1",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#subsection-local-behavior-near-critical-points-2-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "critical points regular points "
...
{
  "id": "definition-critical-regular-point-value",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#definition-critical-regular-point-value",
  "type": "Definition",
  "number": "5.2.1",
  "title": "Critical\/regular point\/value.",
  "body": " Critical\/regular point\/value  critical point  stationary point  critical value  regular point  regular value   Let be a real-valued function defined on an interval .   Critical point\/value  A point is called a critical point of if either , in which case is called a stationary (critical) point of , or is not differentiable at .  A real number is called a critical value of if for some critical point .    Regular point\/value  A point is called a regular point of if it is not a critical point of ; that is, is a regular point of if is differentiable at and .  A real number is called a regular value of if it is not a critical value of ; that is, is a regular value of if the pre-image consists only of regular points of .      "
},
{
  "id": "example-critical-regular-points-values",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#example-critical-regular-points-values",
  "type": "Example",
  "number": "5.2.2",
  "title": "Critical\/regular points\/values.",
  "body": " Critical\/regular points\/values   The following are examples of critical points, regular points, critical values, and regular values of functions:   Let be the function defined piecewise by the formula . Then every point is a critical point of . If , then is a stationary critical point of , and if , then is a point of non-differentiability. Thus , , and are critical values of , and all other real numbers are regular values.    Let be the function defined piecewise by the formula . Then is the only critical point of , and it is a point of non-differentiability. Thus is a critical value of , and all other real numbers are regular values.    Let be the function defined by the formula . Then is the only critical point of , and it is a stationary critical point. Thus is a critical value of , and all other real numbers are regular values.      "
},
{
  "id": "subsection-local-behavior-near-critical-points-5",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#subsection-local-behavior-near-critical-points-5",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "local extrema "
},
{
  ...
  "level": "2",
  "url": "section-analysis-of-critical-points.html#definition-local-extrema",
  "type": "Definition",
  "number": "5.2.3",
  "title": "Local extrema.",
  "body": " Local extrema  local maximum  local minimum  local extremum   Let be a real-valued function defined on a subset .   Local maximum   attains a local maximum at a point if there is a neighborhood of in so that for all points .    Local minimum   attains a local minimum at a point if there is a neighborhood of in so that for all points .    Local extremum is a generic term which encompasses both local maxima and local minima.   "
},
{
  "id": "lemma-local-extrema-are-stationary-points",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#lemma-local-extrema-are-stationary-points",
  "type": "Lemma",
  "number": "5.2.4",
  "title": "Local extrema are stationary points.",
  "body": " Local extrema are stationary points   Let be a real-valued function defined on a subset which attains a local extremum at a point so that every neighborhood of contains points strictly greater than and points strictly less than . If is differentiable at , then ; that is, is a stationary point of .    By contraposition. Suppose first that is positive. Then since is positive, there is some positive real number so that for all points so that . In particular, for all points so that , so that and have the same sign; that is, so long as and , then if and if . In particular, since the neighborhood contains points strictly less than and contains points strictly larger than , we conclude that does not achieve a maximum or minimum at .  Similarly, now suppose that is negative. Then since is positive, there is some positive real number so that for all points so that . In particular, for all points so that , so that and have opposite sign; that is, so long as and , then if and if . In particular, since the neighborhood contains points strictly less than and contains points strictly larger than , we conclude that does not achieve a maximum or minimum at .   "
},
{
  "id": "theorem-interior-extremum-theorem",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#theorem-interior-extremum-theorem",
  "type": "Theorem",
  "number": "5.2.5",
  "title": "Interior extremum theorem.",
  "body": " Interior extremum theorem  Pierre de Fermat   Let be a real-valued function defined on an interval which attains a local extremum at an interior point . If is differentiable at , then ; that is, is a stationary point of .    Any neighborhood of an interior point contains an open interval containing , and therefore contains points strictly greater than and points strictly less than . The desired conclusion now follows immediately from .   "
},
{
  "id": "theorem-darbouxs-theorem",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#theorem-darbouxs-theorem",
  "type": "Theorem",
  "number": "5.2.6",
  "title": "Darboux’s theorem.",
  "body": " Darboux's theorem  Jean-Gaston Darboux   Let be a real-valued function defined on an interval . If is differentiable, then is an interval.    Let , and suppose that . There are three cases; either , , or . If , then vacuously contains all points between and .  Suppose that , let , and consider the function defined by the formula . We observe that and . In particular, the argument in the proof of implies that does not attain a minimum value at or .  However, since is continuous on the compact interval , it attains a minimum value on . Thus attains a minimum value at some interior point , and so the implies that ; that is, , so that .  Now suppose that , let , and consider the function defined by the formula . We observe that and . In particular, the argument in the proof of implies that does not attain a maximum value at or .  However, since is continuous on the compact interval , it attains a maximum value on . Thus attains a maximum value at some interior point , and so the implies that ; that is, , so that .   "
},
{
  "id": "corollary-monotonicity-in-the-absence-of-critical-points",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#corollary-monotonicity-in-the-absence-of-critical-points",
  "type": "Corollary",
  "number": "5.2.7",
  "title": "Monotonicity in the absence of critical points.",
  "body": " Monotonicity in the absence of critical points   Let be a differentiable function defined on an interval . If does not have any critical points in , then is strictly monotone.    If takes both positive and negative values on , then implies that for some point . This contradiction implies that either takes only positive values on or takes only negative values on . We will prove the result in the case for all points ; the case for all is similar.  Let , and suppose that . Since is continuous on the compact interval , it attains a maximum value on . If attains a maximum value at an interior point , then the implies that . This contradiction implies that attains a maximum value at one of the endpoints or .  Since and any neighborhood of in contains points which are strictly larger than , the argument in the proof of implies that does not attain a maximum value at . Thus we conclude that . Since and were chosen arbitrarily, is increasing.   "
},
{
  "id": "subsection-local-behavior-near-critical-points-15-1",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#subsection-local-behavior-near-critical-points-15-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "inverse function theorem "
},
{
  "id": "definition-isolated-critical-point",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#definition-isolated-critical-point",
  "type": "Definition",
  "number": "5.2.8",
  "title": "Isolated critical point.",
  "body": " Isolated critical point  isolated critical point   Let be a real-valued function defined on a subset . A critical point of is called isolated if there is a neighborhood so that consists only of regular points of .   "
},
{
  "id": "example-isolated-critical-points",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#example-isolated-critical-points",
  "type": "Example",
  "number": "5.2.9",
  "title": "Isolated critical points.",
  "body": " Isolated critical points   The following are examples and non-examples of isolated critical points of real-valued functions:   Let be the continuous real-valued function defined piecewise by the formula . Then has infinitely many non-zero isolated critical points which accumulate at the critical point .    Let be the continuous real-valued function defined by the formula . Then has one critical point at , which is isolated.    Let be the continuous real-valued function defined by the formula . Then has three critical points at , , and , each of which is isolated.      "
},
{
  "id": "theorem-first-derivative-test",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#theorem-first-derivative-test",
  "type": "Theorem",
  "number": "5.2.10",
  "title": "First derivative test.",
  "body": " First derivative test   Let be a continuous real-valued function defined on an interval . If is an interior point of in and an isolated critical point of , then attains a local extremum at if and only if the derivative changes sign at .    Under these assumptions, the previous discussion applies; there is some positive real number so that so that does not change sign on the intervals and .  If takes only positive values on both and , then the continuity of and implies that the restrictions of to the intervals and are increasing. We conclude that the restriction of to is increasing, so that for all sufficiently small positive real numbers . In particular, this implies that cannot attain a local extremum at .  If takes only negative values on both and , then the continuity of and implies that the restrictions of to the intervals and are decreasing. We conclude that the restriction of to is decreasing, so that for all sufficiently small positive real numbers . In particular, this implies that cannot attain a local extremum at .  If takes only positive values on and only negative values on , then the continuity of and implies that the restrictions of to the intervals and are increasing and decreasing, respectively. We conclude that for all sufficiently small positive real numbers . In particular, this implies that attains a local maximum at .  Finally, if takes only negative values on and only positive values on , then the continuity of and implies that the restrictions of to the intervals and are decreasing and increasing, respectively. We conclude that for all sufficiently small positive real numbers . In particular, this implies that attains a local minimum at .   "
},
{
  "id": "example-using-the-first-derivative-test",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#example-using-the-first-derivative-test",
  "type": "Example",
  "number": "5.2.11",
  "title": "Using the first derivative test.",
  "body": " Using the first derivative test   Let be a positive integer, and consider the real-valued function defined by the formula . Recall that is differentiable, with derivative . If , then for all points , and so has no critical points. However, if , then vanishes precisely when ; that is, is the only critical point of , and it is a stationary critical point.  We observe that if is even, then when and when . The implies that attains a local minimum at in this case.  On the other hand, if is odd, then when . The implies that does not attain a local extremum at in this case.   "
},
{
  "id": "corollary-second-derivative-test",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#corollary-second-derivative-test",
  "type": "Corollary",
  "number": "5.2.12",
  "title": "Second derivative test.",
  "body": " Second derivative test   Let be a differentiable real-valued function defined on an interval . Suppose that is an interior point of in and an isolated stationary critical point of at which is twice-differentiable. If , then attains a local minimum at , and if , then attains a local maximum at .    Since is a stationary critical point of , . Since is a positive real number, there is a positive real number so that and for all points . If , then , and so and have the same sign; that is, takes only negative values on and takes only positive values on . The now implies that attains a local minimum at .  On the other hand, if , then , and so and have opposite signs; that is, takes only positive values on and takes only negative values on . The now implies that attains a local maximum at .   "
},
{
  "id": "example-using-the-second-derivative-test",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#example-using-the-second-derivative-test",
  "type": "Example",
  "number": "5.2.13",
  "title": "Using the second derivative test.",
  "body": " Using the second derivative test   Let , and consider the real-valued function defined by the formula . Recall that is differentiable, with derivative and second derivative . We have seen in that has a single stationary critical point at .  We observe that if , then , and so the implies that attains a local minimum at in this case. However, if , then , and so the is inconclusive.   "
},
{
  "id": "section-analysis-of-critical-points-5-1",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#section-analysis-of-critical-points-5-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
{
  "id": "exercise-examples-of-critical-points-3",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#exercise-examples-of-critical-points-3",
  "type": "Exercise",
  "number": "5.2.3.1",
  "title": "",
  "body": "  Give an example of a differentiable function which has a single critical point which is not a local extremum.    The function has a single critical point which is not a local extremum.   "
},
{
  "id": "exercise-examples-of-critical-points-4",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#exercise-examples-of-critical-points-4",
  "type": "Exercise",
  "number": "5.2.3.2",
  "title": "",
  "body": "  Give an example of a differentiable function which attains a local minimum but no global minima.    The function attains a local minimum at but no global minima.   "
},
{
  "id": "exercise-examples-of-critical-points-5",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#exercise-examples-of-critical-points-5",
  "type": "Exercise",
  "number": "5.2.3.3",
  "title": "",
  "body": "  Give an example of a differentiable function which has no critical points.    The function has no critical points   "
},
{
  "id": "exercise-examples-of-critical-points-6",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#exercise-examples-of-critical-points-6",
  "type": "Exercise",
  "number": "5.2.3.4",
  "title": "",
  "body": "  Give an example of a differentiable function which has exactly one critical point but no global extrema.    The function has a critical point at but no global extrema.   "
},
{
  "id": "exercise-examples-of-critical-points-7",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#exercise-examples-of-critical-points-7",
  "type": "Exercise",
  "number": "5.2.3.5",
  "title": "",
  "body": "  Give an example of a differentiable function which attains infinitely many global extrema.    The function attains a global extremum at each integer multiple of .   "
},
{
  "id": "problem-misconceptions-about-critical-points",
  "level": "2",
  "url": "section-analysis-of-critical-points.html#problem-misconceptions-about-critical-points",
  "type": "Problem",
  "number": "5.2.4.1",
  "title": "Misconceptions about critical points.",
  "body": " Misconceptions about critical points   Let be a real-valued function defined on an interval . The following statements are false! Explain why the statement is incorrect, and disprove them by providing a counterexample.     If attains a local extremum at a point , then is a critical point of .       If a point is a critical point of , then attains a local extremum at .       If a point is a regular point of , then has a neighborhood in on which is monotone.        is strictly monotone if and only if has no critical points.     "
},
{
  ...
  "body": " Applications of the Mean Value Theorem   In this section, we introduce one of the fundamental results of differential calculus. The mean value theorem relates the instantaneous rate of change of a differentiable function given by its derivative and its average rate of change given by a difference quotient. While not immediately obvious, the mean value theorem has several deep consequences for the study of differentiable functions. We will derive the mean value theorem and exhibit several of these corollaries.    Average and Instantaneous Rates of Change   The average rate of change of a real-valued function defined on some subset which contains a closed interval is the difference quotient . In , we saw that the derivative of a differentiable function is a limit of such difference quotients; as such, the derivative is often interpreted as an instantaneous rate of change . The mean value theorem relates these two rates of change under some mild hypotheses on .   Before we explore the in its full generality, we prove a special case. The proof of the will rely on the following supporting lemma:   Rolle's lemma  Michel Rolle   Let be a real-valued function defined on a closed interval . If is continuous on the closed interval and differentiable on the open interval and , then for some point .    Let , and suppose that . Then since is positive, there is some positive real number so that for all points so that . In particular, for all points so that , so that and have the same sign; that is, so long as and , then if and if . In particular, we conclude that does not achieve a maximum or minimum at .  Similarly, now suppose that . Then since is positive, there is some positive real number so that for all points so that . In particular, for all points so that , so that and have opposite sign; that is, so long as and , then if and if . In particular, we conclude that does not achieve a maximum or minimum at .  However, is closed and bounded, and hence compact by . Since is continuous and is compact, implies that is compact and hence also closed and bounded. In particular, attains a maximum and a minimum .  To summarize our argument so far, we have shown that does not achieve a maximum or minimum value at any point where its derivative is non-zero, but must achieve a maximum and minimum value somewhere in . If achieves a maximum or a minimum value at an interior point , then the above argument shows that . Otherwise, attains both its maximum and minimum values at the endpoints and . In this case, must be constant because . In particular, this implies that for all points .    We remark that the details of the above proof of are necessary even if we have an understanding of the analysis of critical points and local extrema. Indeed, the result often taught in calculus classes that for a function , if is differentiable at a point and , then is increasing near is false; such a conclusion actually requires continuous differentiability near .   Mean value theorem  Joseph-Louis Lagrange   Let be a real-valued function defined on a closed interval . If is continuous on the closed interval and differentiable on the open interval , then for some point .    Let be the real-valued function defined by the formula . Then is continuous by and differentiable on the open interval by , with derivative . Moreover, . now implies that for some interior point ; that is, .     Hypotheses of the    The following examples demonstrate that each of the hypotheses of the is necessary:   Let be the real-valued function defined piecewise by the formula . Then is continuous on and differentiable on the interior , but for all interior points . This example shows that must be defined on a closed interval in order to guarantee the conclusions of the .    Let be the real-valued function defined piecewise by the formula . Then is continuous on and differentiable on the interior , but for all interior points . This example shows that must be continuous on the closed interval in order to guarantee the conclusions of the .    Let be the real-valued function defined by the formula . Then is continuous on and differentiable at every interior point except , but for all interior points . This example shows that must be differentiable on the open interval in order to guarantee the conclusions of the .        While all of the hypotheses of the are necessary in order to guarantee its conclusion, they are sufficient as written. Indeed, a function need not be differentiable at the endpoints or in order to apply the ; for an example, see the function defined by the formula , which is differentiable on the open interval but not at the endpoints and yet satisfies the conclusions of the .     Beyond the Mean Value Theorem    and the are two of a constellation of similar results relating the derivatives of some differentiable functions to the average rates of change of said functions. Particularly useful among these results is the following lemma, from which the and more can be derived:    Generalized mean value theorem   Let be real-valued functions defined on a closed interval , and consider the real-valued function defined by the formula . If , , and are continuous on the closed interval and differentiable on the interior , then is differentiable on , and for some interior point .    We observe that is a linear combination of , , and , and so is continuous on by and is differentiable on by . Moreover, and are determinants of matrices with repeated rows, and so by a standard result of linear algebra. Alternatively, one can verify that by direct computation. In any case, now implies that for some interior point .    One can use to construct another proof of the , but its real strength is in its generality. We include the following alternative proof of for completeness:   Alternative proof of the   Let be the real-valued functions defined by the formulas and , and consider the function defined as in . Then , and so . implies that there is some interior point so that . We observe that .    relates the derivatives of three differentiable functions, and the deals with the derivative of just one differentiable function. Intermediate between these results is the following result, which relates the derivatives of two differentiable functions:   Extended mean value theorem  Augustin-Louis Cauchy   Let be real-valued functions defined on a closed interval . If and are continuous on the closed interval and differentiable on the open interval , then for some point .    Let be the constant real-valued function defined by the formula , and consider the function defined as in . Then , and so . implies that there is some interior point so that . We observe that .    We will use the to compute certain limits taking indeterminate form.   L'Hôpital's rule  Guillaume de l'Hôpital   Let be real-valued functions on an open interval , and consider an interior point . Suppose that either or . If and are differentiable on , for all , and for some extended real number , then     Since is continuous on and for all , attains no local extrema on . In particular, is strictly monotone on and on . Without loss of generality, we may shrink the interval so that for all points .  For each point , let denote the interval with endpoints and , including but not ; that is, . Moreover, let be the extended real-valued functions defined by the formulas . We observe that since , .  Consider distinct points . Since the restriction of to either or is strictly monotone and therefore injective, . Moreover, the restrictions of and to the closed interval between and satisfy the hypotheses of , and so for some point lying strictly between and . In particular, we conclude that for all and distinct points .  We first suppose that . Then the above argument implies that for all and . Taking a limit as , we obtain . The and now implies that .  We now suppose that . Then the above argument implies that for all and . Taking limits as , we observe that . We conclude that for all . The and now implies that .     While different in formulation than the other -like results covered in this section, also relates the values of some differentiable functions with the values of their derivatives.     In the next section, we will explore approximations of multiply-differentiable functions by higher order polynomials.    Exercises   The following exercises are designed to help you practice basic techniques and verify\/solidify your retention of the core ideas presented in each section. Answering these questions will typically involve applying known ideas in familiar ways, recalling information presented in this section, and building fluency with the basic tools of the course. You should approach these exercises as opportunities to make sure you can recognize, recall, and use the fundamental concepts accurately and efficiently before you are asked to answer questions which require complex problem-solving strategies.    Hypotheses and conclusions of the    Determine whether the given function satisfies the hypotheses and conclusions of the .     Let be the real-valued function defined on the interval by the formula .     Determine whether or not satisfies the hypotheses of the .    Yes, satisfies the hypotheses of the .      Determine whether or not satisfies the conclusions of the . If so, find a point for which .    Yes, satisfies the conclusions of the . We may choose .       Let be the real-valued function defined on the interval by the formula .     Determine whether or not satisfies the hypotheses of the .    No, does not satisfy the hypotheses of the .      Determine whether or not satisfies the conclusions of the . If so, find a point for which .    Yes, satisfies the conclusions of the . We may choose .       Short answers and\/or in-depth solutions to these exercises are available in . However, these are provided in order to check your work. If you are unable to answer the above questions, revisit the content of this section rather than spoiling the answers.     Problems   In contrast with the exercises, solving the following problems will require you to engage more deeply with the course material. These questions may require you to analyze a situation, connect multiple ideas, or develop a logical argument from first principles. Many problems will involve writing proofs, explaining your reasoning clearly, and justifying each step. You should expect these to be less routine and more exploratory: there may not be an obvious starting point, and persistence is part of the task .    Functions with the same derivative    Let be a differentiable real-valued function defined on an interval . Prove that if for all , then is constant.       Let be differentiable real-valued functions defined on an interval . Prove that if for all , then and differ by a constant.      The above result will be important in , where we will use it to compare two anti-derivatives of the same continuous function on a closed interval.     Differentiability and Lipschitz continuity   Let be a differentiable real-valued function defined on an interval . Prove that is Lipschitz continuous if and only if its derivative is bounded.      In-depth solutions to these problems are available in . However, these are provided in order to check your work. If you are unable to solve the above problems, discuss your questions with classmates and\/or instructor rather than spoiling the answers.    "
},
{
  "id": "section-applications-of-the-mean-value-theorem-2-1",
  "level": "2",
  "url": "section-applications-of-the-mean-value-theorem.html#section-applications-of-the-mean-value-theorem-2-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "mean value theorem "
...
{
  "id": "subsection-average-and-instantaneous-rates-of-change-2-1",
  "level": "2",
  "url": "section-applications-of-the-mean-value-theorem.html#subsection-average-and-instantaneous-rates-of-change-2-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "average rate of change instantaneous rate of change mean value theorem "
},
{
  "id": "lemma-rolles-lemma",
  "level": "2",
  "url": "section-applications-of-the-mean-value-theorem.html#lemma-rolles-lemma",
  "type": "Lemma",
  "number": "5.3.1",
  "title": "Rolle’s lemma.",
  "body": " Rolle's lemma  Michel Rolle   Let be a real-valued function defined on a closed interval . If is continuous on the closed interval and differentiable on the open interval and , then for some point .    Let , and suppose that . Then since is positive, there is some positive real number so that for all points so that . In particular, for all points so that , so that and have the same sign; that is, so long as and , then if and if . In particular, we conclude that does not achieve a maximum or minimum at .  Similarly, now suppose that . Then since is positive, there is some positive real number so that for all points so that . In particular, for all points so that , so that and have opposite sign; that is, so long as and , then if and if . In particular, we conclude that does not achieve a maximum or minimum at .  However, is closed and bounded, and hence compact by . Since is continuous and is compact, implies that is compact and hence also closed and bounded. In particular, attains a maximum and a minimum .  To summarize our argument so far, we have shown that does not achieve a maximum or minimum value at any point where its derivative is non-zero, but must achieve a maximum and minimum value somewhere in . If achieves a maximum or a minimum value at an interior point , then the above argument shows that . Otherwise, attains both its maximum and minimum values at the endpoints and . In this case, must be constant because . In particular, this implies that for all points .   "
},
{
  "id": "theorem-mean-value-theorem",
  "level": "2",
  "url": "section-applications-of-the-mean-value-theorem.html#theorem-mean-value-theorem",
  "type": "Theorem",
  "number": "5.3.2",
  "title": "Mean value theorem.",
  "body": " Mean value theorem  Joseph-Louis Lagrange   Let be a real-valued function defined on a closed interval . If is continuous on the closed interval and differentiable on the open interval , then for some point .    Let be the real-valued function defined by the formula . Then is continuous by and differentiable on the open interval by , with derivative . Moreover, . now implies that for some interior point ; that is, .   "
},
{
  "id": "example-hypotheses-of-the-mean-value-theorem",
  "level": "2",
  "url": "section-applications-of-the-mean-value-theorem.html#example-hypotheses-of-the-mean-value-theorem",
  "type": "Example",
  "number": "5.3.3",
  "title": "Hypotheses of the Mean value theorem.",
  "body": " Hypotheses of the    The following examples demonstrate that each of the hypotheses of the is necessary:   Let be the real-valued function defined piecewise by the formula . Then is continuous on and differentiable on the interior , but for all interior points . This example shows that must be defined on a closed interval in order to guarantee the conclusions of the .    Let be the real-valued function defined piecewise by the formula . Then is continuous on and differentiable on the interior , but for all interior points . This example shows that must be continuous on the closed interval in order to guarantee the conclusions of the .    Let be the real-valued function defined by the formula . Then is continuous on and differentiable at every interior point except , but for all interior points . This example shows that must be differentiable on the open interval in order to guarantee the conclusions of the .      "
},
{
  "id": "lemma-generalized-mean-value-theorem",
  "level": "2",
  "url": "section-applications-of-the-mean-value-theorem.html#lemma-generalized-mean-value-theorem",
  "type": "Lemma",
  "number": "5.3.4",
  "title": "Generalized mean value theorem.",
  "body": " Generalized mean value theorem   Let be real-valued functions defined on a closed interval , and consider the real-valued function defined by the formula . If , , and are continuous on the closed interval and differentiable on the interior , then is differentiable on , and for some interior point .    We observe that is a linear combination of , , and , and so is continuous on by and is differentiable on by . Moreover, and are determinants of matrices with repeated rows, and so by a standard result of linear algebra. Alternatively, one can verify that by direct computation. In any case, now implies that for some interior point .   "
},
{
  "id": "proof-alternative-proof-of-the-mean-value-theorem",
  "level": "2",
  "url": "section-applications-of-the-mean-value-theorem.html#proof-alternative-proof-of-the-mean-value-theorem",
  "type": "Proof",
  "number": "5.3.2.1",
  "title": "Alternative proof of the Mean value theorem.",
  "body": " Alternative proof of the   Let be the real-valued functions defined by the formulas and , and consider the function defined as in . Then , and so . implies that there is some interior point so that . We observe that .  "
},
{
  "id": "theorem-extended-mean-value-theorem",
  "level": "2",
  "url": "section-applications-of-the-mean-value-theorem.html#theorem-extended-mean-value-theorem",
  "type": "Theorem",
  "number": "5.3.5",
  "title": "Extended mean value theorem.",
  "body": " Extended mean value theorem  Augustin-Louis Cauchy   Let be real-valued functions defined on a closed interval . If and are continuous on the closed interval and differentiable on the open interval , then for some point .    Let be the constant real-valued function defined by the formula , and consider the function defined as in . Then , and so . implies that there is some interior point so that . We observe that .   "
},
{
  "id": "corollary-lhopitals-rule",
  "level": "2",
  "url": "section-applications-of-the-mean-value-theorem.html#corollary-lhopitals-rule",
  "type": "Corollary",
  "number": "5.3.6",
...
{
  "id": "exercise-hypotheses-and-conclusions-of-the-mean-value-theorem-3",
  "level": "2",
  "url": "section-applications-of-the-mean-value-theorem.html#exercise-hypotheses-and-conclusions-of-the-mean-value-theorem-3",
  "type": "Exercise",
  "number": "5.3.3.1",
  "title": "",
  "body": "  Let be the real-valued function defined on the interval by the formula .     Determine whether or not satisfies the hypotheses of the .    Yes, satisfies the hypotheses of the .      Determine whether or not satisfies the conclusions of the . If so, find a point for which .    Yes, satisfies the conclusions of the . We may choose .    "
},
{
  "id": "exercise-hypotheses-and-conclusions-of-the-mean-value-theorem-4",
  "level": "2",
  "url": "section-applications-of-the-mean-value-theorem.html#exercise-hypotheses-and-conclusions-of-the-mean-value-theorem-4",
  "type": "Exercise",
  "number": "5.3.3.2",
  "title": "",
  "body": "  Let be the real-valued function defined on the interval by the formula .     Determine whether or not satisfies the hypotheses of the .    No, does not satisfy the hypotheses of the .      Determine whether or not satisfies the conclusions of the . If so, find a point for which .    Yes, satisfies the conclusions of the . We may choose .    "
},
{
  "id": "problem-functions-with-the-same-derivative",
  "level": "2",
  "url": "section-applications-of-the-mean-value-theorem.html#problem-functions-with-the-same-derivative",
  "type": "Problem",
  "number": "5.3.4.1",
  "title": "Functions with the same derivative.",
  "body": " Functions with the same derivative    Let be a differentiable real-valued function defined on an interval . Prove that if for all , then is constant.       Let be differentiable real-valued functions defined on an interval . Prove that if for all , then and differ by a constant.      The above result will be important in , where we will use it to compare two anti-derivatives of the same continuous function on a closed interval.   "
},
{
  "id": "problem-differentiability-and-lipschitz-continuity",
  "level": "2",
  "url": "section-applications-of-the-mean-value-theorem.html#problem-differentiability-and-lipschitz-continuity",
  "type": "Problem",
  "number": "5.3.4.2",
  "title": "Differentiability and Lipschitz continuity.",
  "body": " Differentiability and Lipschitz continuity   Let be a differentiable real-valued function defined on an interval . Prove that is Lipschitz continuous if and only if its derivative is bounded.    "
},
{
...
{
  "id": "problem-taylor-approximations-of-non-analytic-functions",
  "level": "2",
  "url": "section-higher-order-approximations.html#problem-taylor-approximations-of-non-analytic-functions",
  "type": "Problem",
  "number": "5.4.4.1",
  "title": "Taylor approximations of non-analytic functions.",
  "body": " Taylor approximations of non-analytic functions   Let be the real-valued function defined piecewise by the formula .     Prove that as for all natural numbers .       Prove that as for all natural numbers .    Make the substitution .       Compute all Taylor approximations of near .       Let be a smooth function defined on an interval . If there is a point so that for all positive integers , can we conclude that is constant on a neighborhood of ?       is our first example of a smooth function which is not analytic . We will explore these functions more in .   "
},
{
...
{
  "id": "subsection-riemann-integrability-4",
  "level": "2",
  "url": "section-riemann-sums-and-integrals.html#subsection-riemann-integrability-4",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "upper lower Darboux integrals Darboux integrability Riemann integrability "
},
{
...
},
{
  ...
  "number": "6.1.11",
  "title": "Riemann integrability.",
  "body": " Riemann integrability   The following are examples and non-examples of Riemann integrable functions and their Riemann integrals:   Let be a real number, and consider the constant function defined by the formula . We observe that for all tagged partitions of , where and . This implies that ; that is, is Riemann integrable, with Riemann integral .    Let be the real-valued function defined piecewise on the closed interval by the formula . Let be a partition of . Then there is a unique index so that . We observe that the restriction of to every other subinterval of the partition is constant, so that . This implies that is Riemann integrable, although this method does not necessarily directly compute the Riemann integral .    More generally, let be a non-decreasing real-valued function defined on a closed interval . We observe that for any partition of . This implies that is Riemann integrable, but the above computation doesn't necessarily aid in the computation of the Riemann integral .    Let be the real-valued function defined piecewise by the formula . Since any closed interval contains both rational and irrational numbers, and for any partition of . We conclude that is not Riemann integrable.      "
},
{
  ...
  "body": " Continuity implies integrability   Let be a real-valued function defined on a closed interval . If is continuous, then it is Riemann integrable.    Let . We will show that . To that end, the implies that is uniformly continuous on . Let be a positive real number. Then there is a positive real number so that for all points so that . We observe that for any partition of of mesh . In particular, for all positive real numbers , and so . implies that is Riemann integrable.   "
},
{
  ...
  "body": " Linearity of Riemann integration   Let be a real linear combination of real-valued functions defined on a closed interval . If each is Riemann integrable, then so too is , and .    We observe that for each tagged partition of , , where and . Thus .   "
},
{
  "id": "lemma-riemann-integration-and-comparison",
  "level": "2",
  "url": "section-riemann-sums-and-integrals.html#lemma-riemann-integration-and-comparison",
  "type": "Lemma",
  "number": "6.1.14",
  "title": "Riemann integration and comparison.",
  "body": " Riemann integration and comparison   Let be real-valued functions defined on a closed interval . If and are Riemann integrable and for all points , then .    We observe that for all tagged partitions of , , where and . Thus .   "
},
{
  "id": "theorem-uniform-convergence-and-riemann-integrability",
  "level": "2",
  "url": "section-riemann-sums-and-integrals.html#theorem-uniform-convergence-and-riemann-integrability",
  "type": "Theorem",
  "number": "6.1.15",
  "title": "Uniform convergence and Riemann integrability.",
  "body": " Uniform convergence and Riemann integrability   Let be a uniformly convergent sequence of real-valued functions defined on a closed interval . If each is Riemann integrable, then the uniform limit is Riemann integrable, with Riemann integral .    We first argue that the sequence of Riemann integrals is Cauchy. To that end, let . Since converges uniformly to , there is some natural number so that for all indices and points . In particular, for all indices and points , and so for all indices and tagged partitions of , where and .  Let be a tagged partition of so that . Thus . We conclude that is Cauchy.   implies that this sequence converges to a real number . We will show that . To that end, let be a positive real number. Since converges uniformly to , there is some natural number so that for all indices and points . In particular, for all indices and tagged partitions of . Since converges to , there is a natural number so that for all indices . Fix . Then there is some tagged partition of so that for all refinements . We observe that for all refinements . Thus the net converges to , and so is Riemann integrable, with Riemann integral .   "
},
{
  "id": "problem-more-on-integrability-and-continuity",
  "level": "2",
  "url": "section-riemann-sums-and-integrals.html#problem-more-on-integrability-and-continuity",
  "type": "Problem",
  "number": "6.1.4.1",
  "title": "More on integrability and continuity.",
  "body": " More on integrability and continuity   Let be a Riemann integrable real-valued function defined on a closed interval. Prove that is Riemann integrable for any continuous real-valued function .    Use the on an appropriate restriction of .    "
},
{
  ...
  "body": " The Fundamental Theorems of Calculus   In this section, we develop the two fundamental theorems of calculus . These theorems bridge the two halves of calculus: differential calculus and integral calculus. We will see that differentiation and integration are related operations, and in a sense inverse to each other.    Differentiating Riemann Integrals   Before we establish the first fundamental theorem of calculus , we will require two further properties of the Riemann integral. The first establishes a relation between integrals of the same function on adjacent closed intervals, and the second is an integral analogue of the triangle inequality.    Chasles' relation  Michel Chasles   Let be a real-valued function defined on a closed interval . If is Riemann integrable, then for any interior point , the restrictions of to the closed intervals and are Riemann integrable, and .    Since is Riemann integrable, there is a partition of so that for all refinements . Without loss of generality, we may suppose that for some index . Consider the partitions and of and . We will show that the existence of and implies the Riemann integrability of the restrictions of to and , respectively.  Indeed, note that for any refinement , we may form a refinement by appending as a suffix. We observe that . Similarly, for any refinement , we may form a refinement by appending as a prefix. We observe that . Thus the restrictions of to and are Riemann integrable. It remains only to show that , where .  To that end, we observe that since is bounded, there is a positive real number so that for all points . Note that for any positive real number , there is a positive real number so that for all tagged partitions of of mesh , for all tagged partitions of of mesh , and . Let be a tagged partition of of mesh , where and . If for some index , then consider the tagged partitions and of and , respectively, where . These tagged partitions both have mesh less than , and so .  On the other hand, if for all indices , then there is a unique index so that . We may find a refinement of solely by adding in and choosing tags and in the new subintervals and . We observe that the above computation applies to , and so .  In summary, we have shown that for all positive real numbers , there is some positive real number so that for all tagged partitions of of mesh . We conclude that .    So far, we have only allowed the notation when . provides a way to extend this to allow for any real numbers and . For example, if , then we define . Similarly, if , then we define . You will have an opportunity in the problems for this section to prove that with these extensions of notation for Riemann integrals, still holds.  While the following result may seem innocuous at first glance, it is a deeply useful estimate which reflects the geometric structure of the vector space of Riemann integrable functions.   Triangle inequality for Riemann integrals   Let be a real-valued function defined on a closed interval . If is Riemann integrable, then is Riemann integrable, and .    Since the function defined by the formula , the composition is Riemann integrable by . Since for all points , by . Similarly, for all points , and so by . Thus .    We are now ready to prove the first fundamental theorem of calculus . This foundational result establishes a relationship between differentiation integration: by integrating a continuous function , we may obtain a differentiable function whose derivative is .   First fundamental theorem of calculus  first fundamental theorem of calculus   Let be a continuous real-valued function defined on a closed interval , and consider the real-valued function defined by the formula . Then is differentiable on the open interval , and for all interior points .    Let be a positive real number. Since is continuous at , there is a positive real number so that for all points so that . We observe that and the together imply that for all points so that . Thus is differentiable at , with derivative .     anti-derivative  primitive  Thus integrating a continuous function yields a differentiable function whose derivative is the integrand; we call such a function an anti-derivative (or primitive ) of the integrand.     Integrating Derivatives   Text before the first subdivision.    Second fundamental theorem of calculus   Let be a real-valued function defined on a closed interval . If is continuously differentiable, then .    Let be the real-valued function defined on by the formula . Then the implies that is differentiable on , with derivative for all interior points .     Text after the last subdivision.     Text after the last subsection.    Exercises   The following exercises are designed to help you practice basic techniques and verify\/solidify your retention of the core ideas presented in each section. Answering these questions will typically involve applying known ideas in familiar ways, recalling information presented in this section, and building fluency with the basic tools of the course. You should approach these exercises as opportunities to make sure you can recognize, recall, and use the fundamental concepts accurately and efficiently before you are asked to answer questions which require complex problem-solving strategies.     Short answers and\/or in-depth solutions to these exercises are available in . However, these are provided in order to check your work. If you are unable to answer the above questions, revisit the content of this section rather than spoiling the answers.     Problems   In contrast with the exercises, solving the following problems will require you to engage more deeply with the course material. These questions may require you to analyze a situation, connect multiple ideas, or develop a logical argument from first principles. Many problems will involve writing proofs, explaining your reasoning clearly, and justifying each step. You should expect these to be less routine and more exploratory: there may not be an obvious starting point, and persistence is part of the task .     In-depth solutions to these problems are available in . However, these are provided in order to check your work. If you are unable to solve the above problems, discuss your questions with classmates and\/or instructor rather than spoiling the answers.    "
},
{
  "id": "section-the-fundamental-theorems-of-calculus-2-1",
  "level": "2",
  "url": "section-the-fundamental-theorems-of-calculus.html#section-the-fundamental-theorems-of-calculus-2-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
{
  "id": "subsection-differentiating-riemann-integrals-2-1",
  "level": "2",
  "url": "section-the-fundamental-theorems-of-calculus.html#subsection-differentiating-riemann-integrals-2-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "first fundamental theorem of calculus "
},
{
  "id": "lemma-chasles-relation",
  "level": "2",
  "url": "section-the-fundamental-theorems-of-calculus.html#lemma-chasles-relation",
  "type": "Lemma",
  "number": "6.2.1",
  "title": "Chasles’ relation.",
  "body": " Chasles' relation  Michel Chasles   Let be a real-valued function defined on a closed interval . If is Riemann integrable, then for any interior point , the restrictions of to the closed intervals and are Riemann integrable, and .    Since is Riemann integrable, there is a partition of so that for all refinements . Without loss of generality, we may suppose that for some index . Consider the partitions and of and . We will show that the existence of and implies the Riemann integrability of the restrictions of to and , respectively.  Indeed, note that for any refinement , we may form a refinement by appending as a suffix. We observe that . Similarly, for any refinement , we may form a refinement by appending as a prefix. We observe that . Thus the restrictions of to and are Riemann integrable. It remains only to show that , where .  To that end, we observe that since is bounded, there is a positive real number so that for all points . Note that for any positive real number , there is a positive real number so that for all tagged partitions of of mesh , for all tagged partitions of of mesh , and . Let be a tagged partition of of mesh , where and . If for some index , then consider the tagged partitions and of and , respectively, where . These tagged partitions both have mesh less than , and so .  On the other hand, if for all indices , then there is a unique index so that . We may find a refinement of solely by adding in and choosing tags and in the new subintervals and . We observe that the above computation applies to , and so .  In summary, we have shown that for all positive real numbers , there is some positive real number so that for all tagged partitions of of mesh . We conclude that .   "
},
{
  "id": "proposition-triangle-inequality-for-riemann-integrals",
  "level": "2",
  "url": "section-the-fundamental-theorems-of-calculus.html#proposition-triangle-inequality-for-riemann-integrals",
  "type": "Proposition",
  "number": "6.2.2",
  "title": "Triangle inequality for Riemann integrals.",
  "body": " Triangle inequality for Riemann integrals   Let be a real-valued function defined on a closed interval . If is Riemann integrable, then is Riemann integrable, and .    Since the function defined by the formula , the composition is Riemann integrable by . Since for all points , by . Similarly, for all points , and so by . Thus .   "
},
{
  "id": "subsection-differentiating-riemann-integrals-7",
  "level": "2",
  "url": "section-the-fundamental-theorems-of-calculus.html#subsection-differentiating-riemann-integrals-7",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "first fundamental theorem of calculus "
...
{
  "id": "theorem-first-fundamental-theorem-of-calculus",
  "level": "2",
  "url": "section-the-fundamental-theorems-of-calculus.html#theorem-first-fundamental-theorem-of-calculus",
  "type": "Theorem",
  "number": "6.2.3",
  "title": "First fundamental theorem of calculus.",
  "body": " First fundamental theorem of calculus  first fundamental theorem of calculus   Let be a continuous real-valued function defined on a closed interval , and consider the real-valued function defined by the formula . Then is differentiable on the open interval , and for all interior points .    Let be a positive real number. Since is continuous at , there is a positive real number so that for all points so that . We observe that and the together imply that for all points so that . Thus is differentiable at , with derivative .   "
},
{
  "id": "subsection-differentiating-riemann-integrals-9-3",
  "level": "2",
  "url": "section-the-fundamental-theorems-of-calculus.html#subsection-differentiating-riemann-integrals-9-3",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "anti-derivative primitive "
},
{
  "id": "theorem-second-fundamental-theorem-of-calculus",
  "level": "2",
  "url": "section-the-fundamental-theorems-of-calculus.html#theorem-second-fundamental-theorem-of-calculus",
  "type": "Theorem",
  "number": "6.2.4",
  "title": "Second fundamental theorem of calculus.",
  "body": " Second fundamental theorem of calculus   Let be a real-valued function defined on a closed interval . If is continuously differentiable, then .    Let be the real-valued function defined on by the formula . Then the implies that is differentiable on , with derivative for all interior points .   "
},
{
